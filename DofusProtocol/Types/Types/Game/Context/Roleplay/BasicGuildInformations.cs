...
namespace Stump.DofusProtocol.Types
{
    public class BasicGuildInformations : AbstractSocialGroupInfos
    {
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (guildId < 0)
                throw new Exception("Forbidden value on guildId = " + guildId +
                                    ", it doesn't respect the following condition : guildId < 0");
            guildName = reader.ReadUTF();
            guildLevel = reader.ReadSByte();
            if (guildLevel < 0 || guildLevel > 200)
                throw new Exception("Forbidden value on guildLevel = " + guildLevel +
                                    ", it doesn't respect the following condition : guildLevel < 0 || guildLevel > 200");
        }
    }
}