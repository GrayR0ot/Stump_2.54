...
namespace Stump.DofusProtocol.D2oClasses.Tools.D2i
{
    public class D2IFile
    {
        ...
        private readonly Dictionary<string, D2IEntry<string>>
            m_textEntries = new Dictionary<string, D2IEntry<string>>();
        ...
        private void Initialize()
        {
            using (var reader = new FastBigEndianReader(File.ReadAllBytes(FilePath)))
            {
                ...
                {
                    var key = reader.ReadInt(); //4 byte
                    var undiacritical = reader.ReadBoolean(); //1 byte
                    var dataPos = reader.ReadInt(); //4 byte
        ...
        public void Save(string uri)
        {
            using (var contentWriter = new BigEndianWriter(new StreamWriter(uri).BaseStream))
            {
                ...
                foreach (var entry in sortedEntries) headerWriter.WriteInt(entry.Key);
                ...
                /* write index at end */
                var indexData = headerWriter.Data;
