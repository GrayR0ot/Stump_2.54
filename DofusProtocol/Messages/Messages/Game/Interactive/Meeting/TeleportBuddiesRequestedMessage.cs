...
namespace Stump.DofusProtocol.Messages
{
    public class TeleportBuddiesRequestedMessage : Message
    {
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (dungeonId < 0)
                throw new Exception("Forbidden value on dungeonId = " + dungeonId +
                                    ", it doesn't respect the following condition : dungeonId < 0");
            inviterId = reader.ReadVarLong();
            if (inviterId < 0 || inviterId > 9007199254740990)
                throw new Exception("Forbidden value on inviterId = " + inviterId +
                                    ", it doesn't respect the following condition : inviterId < 0 || inviterId > 9007199254740990");
            ...
            for (var i = 0; i < limit; i++)
            {
                ...
                if (invalidBuddiesIds_[i] > 9007199254740990)
                    throw new Exception("Forbidden value on invalidBuddiesIds_[i] = " + invalidBuddiesIds_[i] +
                                        ", it doesn't respect the following condition : invalidBuddiesIds_[i] > 9007199254740990");
            }
