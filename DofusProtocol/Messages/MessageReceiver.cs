...
namespace Stump.DofusProtocol.Messages
{
    ...
    {
        private static readonly Dictionary<uint, MessageDefinition> MessagesDefinitions =
            new Dictionary<uint, MessageDefinition>();
        ...
        public static void Initialize()
        {
            ...
            foreach (var type in asm.GetTypes().Where(x => x.IsSubclassOf(typeof(Message))))
            {
                ...
                if (fieldId != null)
                {
                    ...
                    if (MessagesDefinitions.TryGetValue(id, out message))
                    {
                        ...
                        if (@override && message.Override)
                            throw new Exception(
                                string.Format(
                                    "MessageReceiver() => {0} and {1} override both the same message (id:{2})",
                                    message.MessageType, type, id));
        ...
        public static Message BuildMessage(uint id, IDataReader reader)
        {
            ...
            var message = definition.MessageConstructor();

            if (message == null)
                throw new MessageNotFoundException(string.Format("Constructors[{0}] (delegate {1}) does not exist", id,
                    MessagesDefinitions[id]));
