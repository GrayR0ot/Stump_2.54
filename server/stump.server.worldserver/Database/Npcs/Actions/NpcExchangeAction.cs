...
{
    [Discriminator("Exchange", typeof(NpcActionDatabase), typeof(NpcActionRecord))]
    internal class NpcExchangeAction : NpcActionDatabase
    {
        ...
        {
            get => Record.GetParameter<string>(0, true);
            set => Record.SetParameter(0, value);
        }

        public string ItemsToReceiveCSV
        {
            get => Record.GetParameter<string>(1, true);
            ...
            get
            {
                if (m_itemsToTake == null)
                {
                    ...
                    if (ItemsToTakeCSV.Contains("|"))
                    {
                        ...
                        {
                            var listItm = itm.FromCSV<string>(";").Select(x => new ItemList(int.Parse(x.Split(',')[1]),
                                ItemManager.Instance.TryGetTemplate(int.Parse(x.Split(',')[0])))).ToList();
                            m_itemsToTake.Add(listItm);
                        }
                    }
                    else
                    {
                        var listItm = ItemsToTakeCSV.FromCSV<string>(";").Select(x =>
                            new ItemList(int.Parse(x.Split(',')[1]),
                                ItemManager.Instance.TryGetTemplate(int.Parse(x.Split(',')[0])))).ToList();
                        m_itemsToTake.Add(listItm);
                    }
                }
        ...
        public List<List<ItemList>> ItemsToReceive
        {
            get
            {
                if (m_itemsToReceive == null)
                {
                    ...
                    {
                        var listItm = ItemsToReceiveCSV.FromCSV<string>(";").Select(x =>
                            new ItemList(int.Parse(x.Split(',')[1]),
                                ItemManager.Instance.TryGetTemplate(int.Parse(x.Split(',')[0])))).ToList();
                        m_itemsToReceive.Add(listItm);
                    }
        ...
        {
            var dialog = new NpcTrade(character, npc, ItemsToTake, ItemsToReceive);
            dialog.Open();
        }
    }
