...
namespace Stump.Server.WorldServer.Database.Npcs.Replies
{
    ...
    public class PvPSeekReply : NpcReply
    {
        ...
        private readonly TimedStack<Pair<int, int>> m_pvpSeekHistory = new TimedStack<Pair<int, int>>(60 * 5);
        ...
        {
            first.SendServerMessage(
                "Un adversaire a été trouvé: <b>" + second.Name + "</b> du rang <b>" + second.GetCharacterRankName() +
                " (" + second.CharacterRankExp + " CP)" + "</b>, lancement du combat..", Color.Chartreuse);
            second.SendServerMessage(
                "Un adversaire vous défi en mode DeadMatch! <b>" + first.Name + "</b> du rang <b>" +
                first.GetCharacterRankName() + " (" + first.CharacterRankExp + " CP)" + "</b>, lancement du combat..",
                Color.Chartreuse);
            ...
            {
                var preFight =
                    FightManager.Instance.CreateAgressionFight(m_pvpmap, second.AlignmentSide, first.AlignmentSide,
                        true);
                try
                {
                ...
                {
                    preFight.EndFight();
                    second.Teleport(Game.World.Instance.GetMap(100270593), second.Cell);
                    first.Teleport(Game.World.Instance.GetMap(100270593), first.Cell);
                }
            }
            else
            {
        ...
        private static void OnQueueBattleField(Character character, PvPSeekReply obj)
        {
            ...
            target = Game.World.Instance.GetCharacters(x => character.CanBattlefield(x)).RandomElementOrDefault();
            if (target != null && target.Account.Nickname == character.CharacterToSeekName && !character.IsGameMaster())
                target = null;
            obj.retryBattleField++;
            character.SendServerMessage("Recherche en cours d'adversaire... (" + obj.retryBattleField + " essai)",
                Color.Aqua);
            ...
            if (target != null)
            {
                character.SendServerMessage(
                    "Un adversaire a été trouvé pour le DeadMatch, mais vous étiez occupé, le combat a été annulé.",
                    Color.Red);
                return;
            }
            ...
            {
                character.SendServerMessage(
                    "Personne n'est actuellement disponible pour le DeadMatch, veuillez réessayer plus tard...",
                    Color.DarkOrange);
                return;
            }

            obj.BattlefieldTimer =
                new Timer(_ => OnQueueBattleField(character, obj), null, 1000 * 10, Timeout.Infinite);
        }
        ...
        public override bool Execute(Npc npc, Character character)
        {
            ...
            {
                character.SendServerMessage(
                    "Vous devez attendre de ne plus être banni pour chercher un combat sur le DeadMatch.",
                    Color.DarkOrange);
                return false;
            }
            ...
            retryBattleField = 0;
            character.SendServerMessage("Recherche d'adversaire pour le DeadMatch en cours... (60 secondes)",
                Color.Aqua);
            BattlefieldTimer = new Timer(_ => OnQueueBattleField(character, this), null, 1000 * 10, Timeout.Infinite);
