...
namespace Stump.Server.WorldServer.Database.Monsters
{
    ...
    public class MonsterGrade : IAssignedByD2O, ISaveIntercepter, IAutoGeneratedRecord
    {
        ...
        {
            get => m_template ?? (m_template = MonsterManager.Instance.GetTemplate(MonsterId));
            set
            {
                m_template = value;
                MonsterId = value.Id;
        ...
        public string StatsJSON
        {
            ...
            set
            {
                ...
                Stats = value == null
                    ? new Dictionary<PlayerFields, short>()
                    : value.FromJson<Dictionary<PlayerFields, short>>();
            }
        }

        [Ignore] public Dictionary<PlayerFields, short> Stats { get; set; }

        public List<MonsterSpell> SpellsTemplates => m_spellsTemplates ??
                                                     (m_spellsTemplates =
                                                         MonsterManager.Instance.GetMonsterGradeSpells(Id));

        public List<Spell> Spells
        {
            get { return m_spells ?? (m_spells = SpellsTemplates.Select(entry => new Spell(entry)).ToList()); }
        }
        ...
        public MonsterGrade DeepCopy()
        {
            var other = (MonsterGrade) MemberwiseClone();
            return other;
        }
    }
}