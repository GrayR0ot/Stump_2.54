...
namespace Stump.Server.WorldServer.Database.Tinsel
{
    public class TinselManager : DataManager<TinselManager>
    {
        ...
        public IReadOnlyDictionary<short, TitleRecord> Titles => new ReadOnlyDictionary<short, TitleRecord>(m_titles);

        public IReadOnlyDictionary<short, OrnamentRecord> Ornaments =>
            new ReadOnlyDictionary<short, OrnamentRecord>(m_ornaments);

        [Initialization(InitializationPass.Fourth)]
        public override void Initialize()
        {
            m_titles = Database.Fetch<TitleRecord>(TitleRelator.FetchQuery).ToDictionary(entry => (short) entry.Id);
            m_ornaments = Database.Fetch<OrnamentRecord>(OrnamentRelator.FetchQuery)
                .ToDictionary(entry => (short) entry.Id);
        }
    }
}