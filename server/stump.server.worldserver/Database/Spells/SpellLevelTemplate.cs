...
namespace Stump.Server.WorldServer.Database.Spells
{
    ...
    public class SpellLevelTemplate : IAssignedByD2O, ISaveIntercepter, IAutoGeneratedRecord
    {
        ...
        public string CriticalEffectsJson { get; set; }
        ...
        public List<EffectDice> Effects
        {
            get
            {
                ...
                var effectBaseList = new List<EffectBase>();
                var jArray = JArray.Parse(EffectsJson);
                foreach (JObject jsonObject in jArray)
                {
                    var effectBase = new EffectBase(
                ...
                //EffectManager.Instance.DeserializeEffectsFromJson(EffectsJson);
                var effectDiceList = new List<EffectDice>();
                foreach (var effectBase in effectBaseList)
                {
                    var effectDice = new EffectDice(effectBase.Id, effectBase.Value, effectBase.DiceNum,
        ...
        public List<EffectDice> CriticalEffects
        {
            get
            {
                ...
                var effectBaseList = new List<EffectBase>();
                var jArray = JArray.Parse(CriticalEffectsJson);
                foreach (JObject jsonObject in jArray)
                {
                    var effectBase = new EffectBase(
                        (string) jsonObject.GetValue("targetMask"),
        ...
        public void AssignFields(object d2oObject)
        {
            ...
            EffectsJson = EffectManager.Instance.SerializeEffectsToJson(spell.effects);
            CriticalEffectsJson = EffectManager.Instance.SerializeEffectsToJson(spell.criticalEffect);
        }
