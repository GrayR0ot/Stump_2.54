...
namespace Stump.Server.WorldServer.Database.World.Maps
{
    ...
    public class MapRecord : ISaveIntercepter, IAutoGeneratedRecord
    {
        ...
        private string m_compressedCellsJson;
        ...
        [Ignore] public MapPositionRecord Position { get; set; }

        [Ignore]
        public bool Outdoor
        {
            get => Position != null && Position.Outdoor;
            set
            {
                if (Position != null) Position.Outdoor = value;
            }
        }
        ...
        public int ClientTopNeighbourId { get; set; }

        public int ClientBottomNeighbourId { get; set; }

        public int ClientLeftNeighbourId { get; set; }

        public int ClientRightNeighbourId { get; set; }
        ...
        [NullString] public string BlueCellsCSV { get; set; }

        [NullString] public string RedCellsCSV { get; set; }
        ...
        public byte[] CompressedCells
        {
            ...
            set
            {
                ...
                {
                    Cells[j] = new Cell((int) Version);
                    Cells[j].Deserialize(uncompressedCells, i);
                }
            }
        }
        
        public String CompressedCellsJson
        {
            get => m_compressedCellsJson;
            set
            {
                m_compressedCellsJson = value;

                JArray jArray = JArray.Parse(m_compressedCellsJson);
                foreach (JObject jObject in jArray)
                {
                    Cell cell = new Cell(
                        (short)jObject.GetValue("Floor"),
                        (short)jObject.GetValue("Id"),
                        (byte)jObject.GetValue("Losmov"),
                        (byte)jObject.GetValue("MapChangeData"),
                        (uint)jObject.GetValue("MoveZone"),
                        (byte)jObject.GetValue("Speed"),
                        (int)Version
                    );
                    Cells.Add(cell);
                }
        ...
        [Ignore] public Cell[] Cells { get; private set; }
