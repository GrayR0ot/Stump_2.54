...
namespace Stump.Server.WorldServer.Database.World
{
    ...
    public class Cell
    {
        ...
        public bool NonWalkableDuringFight => versionMap >= 9 ? (LosMov & 2) != 0 : (LosMov & 4) >> 2 == 1;
        ...
        //    }
        //}
        public bool Red => versionMap >= 9 ? (LosMov & 32) != 0 : (LosMov & 8) >> 3 == 1;

        public bool Blue => versionMap >= 9 ? (LosMov & 16) != 0 : (LosMov & 16) >> 4 == 1;

        public bool FarmCell => versionMap >= 9 ? (LosMov & 128) != 0 : (LosMov & 32) >> 5 == 1;

        public bool Visible => versionMap >= 9 ? (LosMov & 64) != 0 : (LosMov & 64) >> 6 == 1;

        public bool NonWalkableDuringRP => versionMap >= 9 ? (LosMov & 4) != 0 : (LosMov & 128) >> 7 == 1;
        ...
        public void Deserialize(byte[] data, int index = 0)
        {
            ...
            Speed = data[index + 6];
            MoveZone = (uint) ((data[index + 7] << 24) | (data[index + 8] << 16) | (data[index + 9] << 8) |
                               data[index + 10]);
        }
    }
}