...
namespace Stump.Server.WorldServer.Database.Mounts
{
    public class MountTemplateRelator : OneToManyRelator<MountTemplate, MountBonus>
    {
        public static string FetchQuery =
            "SELECT * FROM mounts_templates LEFT JOIN mounts_bonus ON mounts_templates.Id = mounts_bonus.MountTemplateId";
    }
    ...
    public sealed class MountTemplate : IAssignedByD2O, IAutoGeneratedRecord, IOneToManyRecord1<MountBonus>
    {
        ...
        [Ignore] public string Name => m_name ?? (m_name = TextManager.Instance.GetText(NameId));
        ...
        public ItemTemplate ScrollItem => m_scrollItem ?? (m_scrollItem =
            ItemManager.Instance.TryGetTemplate(ScrollId == 0 ? DEFAULT_SCROLL_ITEM : (int) ScrollId));
        ...
        [Ignore] public List<MountBonus> Bonuses { get; set; } = new List<MountBonus>();
