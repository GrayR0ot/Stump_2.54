...
namespace Stump.Server.WorldServer.Database.Idols
{
    ...
    public sealed class IdolTemplate : IAssignedByD2O, IAutoGeneratedRecord, ISaveIntercepter
    {
        ...
        {
            get => m_synergyIdolsIdsCSV;
            set
            {
                m_synergyIdolsIdsCSV = value;
                m_synergyIdolsIds = !string.IsNullOrEmpty(m_synergyIdolsIdsCSV)
                    ? m_synergyIdolsIdsCSV.FromCSV<int>(",").ToList()
                    : new List<int>();
            }
        }
        ...
        {
            get => m_synergyIdolsCoefCSV;
            set
            {
                m_synergyIdolsCoefCSV = value;
                m_synergyIdolsCoef = !string.IsNullOrEmpty(m_synergyIdolsCoefCSV)
                    ? m_synergyIdolsCoefCSV.FromCSV<double>(";").ToList()
                    : new List<double>();
            }
        }
        ...
        {
            get => m_incompatibleMonstersCSV;
            set
            {
                m_incompatibleMonstersCSV = value;
                m_incompatibleMonsters = !string.IsNullOrEmpty(m_incompatibleMonstersCSV)
                    ? m_incompatibleMonstersCSV.FromCSV<int>(",").ToList()
                    : new List<int>();
            }
        }

        [NullString] public string TargetString { get; set; }
