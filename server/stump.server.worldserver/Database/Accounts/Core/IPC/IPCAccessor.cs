...
namespace Stump.Server.WorldServer.Core.IPC
{
    public class IPCAccessor : IPCEntity
    {
        ...
        {
            get => m_instance ?? (m_instance = new IPCAccessor());
            private set => m_instance = value;
        }

        public bool Running { get; set; }

        public SelfRunningTaskPool TaskPool { get; }
        ...
        private void OnClientDisconnected()
        {
            ...
            foreach (var request in Requests.Values) request.Cancel("world server - disconnected");
        ...
        private void ResumeReceive()
        {
            ...
            m_readArgs.SetBuffer(m_bufferSegment.Buffer.Array, m_bufferSegment.Offset + m_writeOffset,
                m_bufferSegment.Length - m_writeOffset);
            m_readArgs.Completed += ProcessReceive;

            var willRaiseEvent = Socket.ReceiveAsync(m_readArgs);
            if (!willRaiseEvent) ProcessReceive(this, m_readArgs);
        }
        ...
        protected virtual bool BuildMessage(BufferSegment buffer)
        {
            ...
            {
                logger.Error("Cannot build message. Length={0} LengthSize={3} RemainingLength={1} Data={2}",
                    m_messagePart.Length, m_remainingLength, m_messagePart.Data, m_messagePart.LengthBytesCount);
                throw;
            }
        ...
        {
            var clients = WorldServer.Instance
                .FindClients(client => client.Account != null && client.Account.Id == message.AccountId).ToArray();

            if (clients.Length > 1)
                logger.Error("Several clients connected on the same account ({0}). Disconnect them all",
                    message.AccountId);
