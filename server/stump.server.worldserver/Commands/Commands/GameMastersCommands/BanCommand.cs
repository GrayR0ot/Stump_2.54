...
namespace Stump.Server.WorldServer.Commands.Commands
{
    public class BanCommand : TargetCommand
    {
        public BanCommand()
        {
            ...
            AddParameter<string>("reason", "r", "Reason of ban", isOptional: true);
    ...
    public class BanHardwareIdCommand : CommandBase
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            IPCAccessor.Instance.SendRequest(message,
                ok =>
                {
                    World.Instance.ForEachCharacter(x => x.Account.LastHardwareId == hardwareId,
                        character => character.Client.Disconnect());
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            else
            {
                WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
                {
                    ...
                    IPCAccessor.Instance.SendRequest<AccountAnswerMessage>(message, reply => OnReply(trigger, reply),
                        error => trigger.ReplyError("Cannot get player {0} account : {1}", character.Name,
                            error.Message));
                });
            }
        }
    ...
    public class UnBanCommand : CommandBase
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
                ...
                WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
                {
                    ...
                    IPCAccessor.Instance.SendRequest<AccountAnswerMessage>(message,
                        reply => IPCAccessor.Instance.SendRequest(new UnBanAccountMessage(reply.Account.Login),
                            ...
                            error =>
                                trigger.ReplyError("Account {0} not unbanned : {1}", reply.Account.Login,
                                    error.Message)),
                        error =>
            ...
            else
            {
                ...
                player.SendServerMessage("Vous avez �t� lib�r� de prison.", Color.Red);
            }
        }
    }
