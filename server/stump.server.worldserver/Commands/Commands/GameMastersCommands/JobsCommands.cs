...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class JobLevelupCommand : TargetSubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            {
                jobTemplate = JobManager.Instance.GetJobTemplates()
                    .FirstOrDefault(x => x.Name.ToLower().Contains(jobStr.ToLower()));
            ...
            foreach (var target in GetTargets(trigger))
            {
                ...
                {
                    trigger.ReplyError(
                        $"{trigger.Bold(target.Name)} doesn't have job {trigger.Bold(jobTemplate.Name)}");
                    continue;
                }

                if (level + job.Level > ExperienceManager.Instance.HighestJobLevel)
                    job.Experience =
                        ExperienceManager.Instance.GetJobLevelExperience(ExperienceManager.Instance.HighestJobLevel);
                ...
                trigger.Reply(
                    $"{trigger.Bold(target.Name)} job {trigger.Bold(jobTemplate.Name)} is now level {job.Level}");
            }
        }
    }
}