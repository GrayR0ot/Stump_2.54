...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class LearnSpellCommand : TargetSubCommand
    {
        public LearnSpellCommand()
        {
            ...
            AddParameter("spell", "spell", "Given spell to learn",
                converter: ParametersConverter.SpellTemplateConverter);
    ...
    public class UnLearnSpellCommand : TargetSubCommand
    {
        public UnLearnSpellCommand()
        {
            ...
            AddParameter("spell", "spell", "Given spell to forget",
                converter: ParametersConverter.SpellTemplateConverter);
            AddTargetParameter(true);
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            var result = trigger.Get<bool>("keep")
                ? target.Spells.ForgetSpell(spell)
                : target.Spells.UnLearnSpell(spell);
            ...
            if (result)
                ...
                trigger.ReplyError("Spell {0} not unlearned. {1} may not have this spell", trigger.Bold(spell.Name),
                    trigger.Bold(target));
        }
    }
    ...
    public class LearnMonsterSpellCommand : TargetSubCommand
    {
        public LearnMonsterSpellCommand()
        {
            ...
            ParentCommandType = typeof(SpellsCommands);
            Description = "Learn the given spell";
            AddParameter("monster", "monster", "Target monster to learn spells",
                converter: ParametersConverter.MonsterTemplateConverter, isOptional: true);
            AddTargetParameter(true);
        }
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var spell in monster.Grades.FirstOrDefault().SpellsTemplates)
            {
                ...
                if (result != null)
                    trigger.Reply("'{0}' learned the spell '{1}'", trigger.Bold(target),
                        trigger.Bold(result.Template.Name));
                else
    ...
    public class SpellInfoCommand : SubCommand
    {
        ...
        public static void ExploreSpell(SpellTemplate spell, int level, bool critical, TriggerBase trigger)
        {
            ...
            trigger.ReplyBold("Level.SpellBreed = {0}, Level.HideEffects = {1}", levelTemplate.SpellBreed,
                levelTemplate.HideEffects);
            trigger.ReplyBold("");

            foreach (var effect in critical ? levelTemplate.CriticalEffects : levelTemplate.Effects)
            {
                trigger.ReplyBold("Effect \"{0}\" ({1}, {2})",
                    TextManager.Instance.GetText(effect.Template.DescriptionId), effect.EffectId,
                    (int) effect.EffectId);
                trigger.ReplyBold("DiceFace = {0}, DiceNum = {1}, Value = {2}", effect.DiceFace, effect.DiceNum,
                    effect.Value);
                trigger.ReplyBold("Hidden = {0}, Modificator = {1}, Random = {2}, Trigger = {3}, Delay = {4}",
                    effect.Hidden, effect.Modificator, effect.Random, effect.Trigger, effect.Delay);
                trigger.ReplyBold("ZoneShape = {0}, ZoneSize = {1}-{2}, Duration = {3}, Target = {4}, Group = {5}",
                    ...
                    effect.Group);
                trigger.ReplyBold("Template.Active = {0}, Template.BonusType = {1}, Template.Boost = {2}",
                    effect.Template.Active, effect.Template.BonusType, effect.Template.Boost);
                trigger.ReplyBold("Template.Category = {0}, Template.Characteristic = {1}, Template.ForceMinMax = {2}",
                    effect.Template.Category, effect.Template.Characteristic, effect.Template.ForceMinMax);
                trigger.ReplyBold("Template.Operator = {0}, Template.Id = {1}, Template.ShowInSet = {2}",
                    effect.Template.Operator, effect.Template.Id, effect.Template.ShowInSet);
                trigger.ReplyBold("Template.ShowInTooltip = {0}, Template.UseDice = {1}", effect.Template.ShowInTooltip,
                    effect.Template.UseDice);
                trigger.ReplyBold("");
            }
