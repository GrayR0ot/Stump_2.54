...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class InteractiveShowCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var io in ios)
            {
                ...
                character.Client.Send(new DebugHighlightCellsMessage(randomColor.ToArgb(),
                    new[] {(ushort) io.Cell.Id}));
                trigger.Reply(trigger.Color(
                    $"Identifier : {io.Id} - Template Id: {io.Template?.Id ?? 0} - CellId: {io.Cell.Id} - Animated - {io.Animated} - Element Id : {io.Spawn.ElementId}",
                    randomColor));
    ...
    public class InteractiveTeleportCommand : AddRemoveSubCommand
    {
        public InteractiveTeleportCommand()
        {
            ...
            RequiredRole = RoleEnum.Administrator;
            ParentCommandType = typeof(InteractivesCommands);
            AddParameter("templateId", "Interactive templateId",
                converter: ParametersConverter.InteractiveTemplateConverter);
    ...
    public class InteractivesStarsCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            if (map != null)
                foreach (var skill in map.GetInteractiveObjects().SelectMany(x => x.GetSkills())
                    .OfType<ISkillWithAgeBonus>())
                    skill.AgeBonus = (short) bonus;
            else if (subarea != null)
                foreach (var skill in subarea.Maps.SelectMany(x =>
                    x.GetInteractiveObjects().SelectMany(y => y.GetSkills()).OfType<ISkillWithAgeBonus>()))
                    skill.AgeBonus = (short) bonus;
            else
                foreach (var skill in World.Instance.GetMaps().SelectMany(x =>
                    x.GetInteractiveObjects().SelectMany(y => y.GetSkills()).OfType<ISkillWithAgeBonus>()))
                    skill.AgeBonus = (short) bonus;
