...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class MonsterSpawnCommand : SubCommand
    {
        public MonsterSpawnCommand()
        {
            ...
            AddParameter("monster", "m", "Monster template Id",
                converter: ParametersConverter.MonsterTemplateConverter);
            ...
            AddParameter("direction", "dir", "Direction", isOptional: true,
                converter: ParametersConverter.GetEnumConverter<DirectionsEnum>());
        }
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            if (trigger.IsArgumentDefined("map") && trigger.IsArgumentDefined("cell") &&
                trigger.IsArgumentDefined("direction"))
            {
    ...
    public class MonsterSpawnNextCommand : SubCommand
    {
        public MonsterSpawnNextCommand()
        {
            ...
            AddParameter("subarea", "subarea", "If defined spawn a monster on each map", isOptional: true,
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            {
                var i = subarea.Maps
                    .Select(subMap => subMap.SpawningPools.OfType<ClassicalSpawningPool>().FirstOrDefault())
    ...
    public class StaticEnableCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            var map = trigger.IsArgumentDefined("map")
                ? trigger.Get<Map>("map")
                : (trigger as GameTrigger).Character.Map;

            var spawns = map.MonsterSpawns.ToArray();
            map.DisableClassicalMonsterSpawns();

            foreach (var spawn in spawns) spawn.Map.RemoveMonsterSpawn(spawn);
    ...
    public class StaticDisableCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var spawn in spawns.Where(spawn => spawn.Map != null)) spawn.Map.AddMonsterSpawn(spawn);
            ...
            WorldServer.Instance.IOTaskPool.AddMessage(() =>
            {
                ...
                // do something else ?
                trigger.ReplyBold("{0} is not static anymore",
                    string.Format("{0}/{1}", map.Position.X, map.Position.Y));
            });
        }
    }
    ...
    public class StaticMonster : AddRemoveSubCommand
    {
        public StaticMonster()
        {
            ...
            AddParameter("direction", "d", "Given direction", isOptional: true,
                converter: ParametersConverter.DirectionConverter);
        ...
        public override void ExecuteAdd(TriggerBase trigger)
        {
            ...
            var cell = trigger.IsArgumentDefined("cell")
                ? trigger.Get<short>("cell")
                : ((GameTrigger) trigger).Character.Cell.Id;
            var direction = trigger.IsArgumentDefined("direction")
                ? trigger.Get<DirectionsEnum>("direction")
                : ((GameTrigger) trigger).Character.Direction;
