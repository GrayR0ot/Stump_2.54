...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class TransitionSetCommand : SubCommand
    {
        public TransitionSetCommand()
        {
            ...
            AddParameter("cell", "c", "The cell destination", isOptional: true,
                converter: ParametersConverter.CellConverter);
            AddParameter("from", "f", "The map to modify", isOptional: true,
    ...
    public class TransitionAddTriggerCommand : AddRemoveSubCommand
    {
        public TransitionAddTriggerCommand()
        {
            ...
            RequiredRole = RoleEnum.Administrator;
            ParentCommandType = typeof(TransitionsCommands);
            AddParameter("cellidsrc", "cellsrc", "Cell source", isOptional: true,
                converter: ParametersConverter.CellConverter);
        ...
        public override void ExecuteAdd(TriggerBase trigger)
        {
            ...
            {
                var cellIdSrc = trigger.IsArgumentDefined("cellidsrc")
                    ? character.Map.Cells[trigger.Get<short>("cellidsrc")]
                    : character.Cell;
                ...
                WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
                {
                    ...
                    trigger.ReplyBold("Add CellTrigger from map {0}({1}) to {2}({3})", record.MapId, record.CellId,
                        record.Parameter1, record.Parameter0);
                });
            }
        }
