...
namespace Stump.Server.WorldServer.Commands.Commands
{
    ...
    public class AIActionsInfoCommand : InGameSubCommand
    {
        ...
        public override void Execute(GameTrigger trigger)
        {
            ...
            {
                trigger.Reply("(#1) Spell {0} ({1}) :: {2}", cast.Spell.Template.Name, cast.Spell.Id,
                    SpellIdentifier.GetSpellCategories(cast.Spell));
                ...
                foreach (var impact in cast.Impacts) trigger.Reply(dumper.DumpElement(impact));
            }
        }
    }
    ...
    public class AIActionExecuteCommand : InGameSubCommand
    {
        ...
        public override void Execute(GameTrigger trigger)
        {
            ...
            var index = trigger.Get<int>("action");

            if (index >= aiFighter.Brain.SpellSelector.Possibilities.Count)
            {
                trigger.ReplyError("Specify an index between 0 and " +
                                   (aiFighter.Brain.SpellSelector.Possibilities.Count - 1));
                return;
            }
    ...
    public class AIShowMovesCommand : InGameSubCommand
    {
        ...
        public override void Execute(GameTrigger trigger)
        {
            ...
            if (trigger.IsArgumentDefined("los"))
                trigger.Reply(trigger.Color("Cast with LoS",
                    trigger.Character.HighlightCells(aiFighter.Brain.Environment.GetCellsWithLoS(enemy.Cell))));

            foreach (var spell in aiFighter.Spells.Values)
            {
                var cell = aiFighter.Brain.Environment.GetCellToCastSpell(new TargetCell(enemy.Cell), spell,
                    spell.CurrentSpellLevel.CastTestLos);
                if (cell != null)
