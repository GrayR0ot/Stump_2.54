...
namespace Stump.Server.WorldServer.Commands.Commands
{
    public class InfoMapCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            else if (trigger is GameTrigger) map = (trigger as GameTrigger).Character.Map;
            ...
            foreach (var actor in actors) trigger.ReplyBold("- {0} : {1}", actor.GetType().Name, actor);

            trigger.ReplyBold("SpawningPools ({0}) :", map.SpawningPools.Count);

            foreach (var pool in map.SpawningPools)
                trigger.ReplyBold("- {0} : State : {1}, Next : {2}s", pool.GetType().Name, pool.State,
                    pool.RemainingTime / 1000);
        }
    }
    ...
    public class InfoAreaCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            else if (trigger is GameTrigger) area = (trigger as GameTrigger).Character.Area;
            ...
            foreach (var group in groups.Take(5))
            {
                ...
                trigger.ReplyBold("- {0} {1} ms {2} entries {3} errors", group.Key, average, group.Count(),
                    errors.Count());
            }
        }
    }
    ...
    public class InfoCharacterCommand : TargetSubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            foreach (var character in GetTargets(trigger))
            {
                ...
                trigger.ReplyBold("Ip : {0}",
                    character.UserGroup.Role == RoleEnum.Administrator ? "127.0.0.1" : character.Client.IP);
                ...
                trigger.ReplyBold("Fight : {0}",
                    character.IsFighting()
                        ? character.Fight.Id.ToString(CultureInfo.InvariantCulture)
                        : "Not fighting");
    ...
    public class InfoFightCommand : SubCommand
    {
        public InfoFightCommand()
        {
            ...
            ParentCommandType = typeof(InfoCommand);
            AddParameter("fight", "f", "Gives informations about the given fight",
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var spectator in fight.Spectators) trigger.ReplyBold(" - {0}", spectator.Character);
        }
    }
    ...
    public class InfoIPCommand : SubCommand
    {
        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var character in characters)
                trigger.Reply(string.Format("Player: {0} - Account: {1}({2})", character.Name, character.Account.Login,
                    character.Account.LastHardwareId));
        }
    }
}