...
namespace Stump.Server.WorldServer.Game.Maps.Cells.Shapes.Set
{
    public class LozengeSet : Set
    {
        ...
        {
            for (var r = MinRadius; r <= Radius; r++)
            {
                ...
                // bottom right side
                for (var i = 0; i < r; i++) yield return new MapPoint(Center.X + (r - i), Center.Y - i);

                //bottom left side
                for (var i = 0; i < r; i++) yield return new MapPoint(Center.X - i, Center.Y - (r - i));
