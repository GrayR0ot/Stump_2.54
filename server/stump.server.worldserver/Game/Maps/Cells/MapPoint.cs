...
namespace Stump.Server.WorldServer.Game.Maps.Cells
{
    ...
    public class MapPoint
    {
        ...
        public DirectionsEnum OrientationToAdjacent(MapPoint point)
        {
            ...
            if (vector == VectorRight) return DirectionsEnum.DIRECTION_EAST;
            if (vector == VectorDownRight) return DirectionsEnum.DIRECTION_SOUTH_EAST;
            if (vector == VectorDown) return DirectionsEnum.DIRECTION_SOUTH;
            if (vector == VectorDownLeft) return DirectionsEnum.DIRECTION_SOUTH_WEST;
            if (vector == VectorLeft) return DirectionsEnum.DIRECTION_WEST;
            if (vector == VectorUpLeft) return DirectionsEnum.DIRECTION_NORTH_WEST;
            if (vector == VectorUp) return DirectionsEnum.DIRECTION_NORTH;
            if (vector == VectorUpRight) return DirectionsEnum.DIRECTION_NORTH_EAST;
        ...
        public DirectionsEnum OrientationTo(MapPoint point, bool diagonal = true)
        {
            ...
            var transformedAngle = angleInDegrees * (point.Y > m_y ? -1 : 1);
        ...
        public MapPoint[] GetCellsOnLineBetween(MapPoint destination)
        {
            ...
            var current = this;
            for (var i = 0; i < MapHeight * MapWidth / 2; i++)
            {
        ...
        {
            return new LineSet(new MapPoint(0), new MapPoint(13)).BelongToSet(this) ||
                   new LineSet(new MapPoint(14), new MapPoint(26)).BelongToSet(this) ||
                   new LineSet(new MapPoint(546), new MapPoint(559)).BelongToSet(this) ||
                   new LineSet(new MapPoint(532), new MapPoint(545)).BelongToSet(this) ||
                   new LineSet(new MapPoint(27), new MapPoint(559)).BelongToSet(this) ||
                   new LineSet(new MapPoint(13), new MapPoint(545)).BelongToSet(this) ||
                   new LineSet(new MapPoint(0), new MapPoint(532)).BelongToSet(this) ||
                   new LineSet(new MapPoint(14), new MapPoint(546)).BelongToSet(this);
        }
        ...
        public MapNeighbour GetMapNeighbour(Cell cell)
        {
            ...
            if (GetBorderCells(MapNeighbour.Bottom).Contains(cell))
                return MapNeighbour.Bottom;
            if (GetBorderCells(MapNeighbour.Right).Contains(cell))
                return MapNeighbour.Right;
            if (GetBorderCells(MapNeighbour.Left).Contains(cell)) return MapNeighbour.Left;
        ...
        public bool IsOnMapBorder(MapNeighbour mapNeighbour)
        {
            switch (mapNeighbour)
            {
                ...
                case MapNeighbour.Bottom:
                    return new LineSet(new MapPoint(546), new MapPoint(559)).BelongToSet(this) ||
                           new LineSet(new MapPoint(532), new MapPoint(545)).BelongToSet(this);
                case MapNeighbour.Right:
                    return new LineSet(new MapPoint(27), new MapPoint(559)).BelongToSet(this) ||
                           new LineSet(new MapPoint(13), new MapPoint(545)).BelongToSet(this);
                case MapNeighbour.Left:
                    return new LineSet(new MapPoint(0), new MapPoint(532)).BelongToSet(this) ||
                           new LineSet(new MapPoint(14), new MapPoint(546)).BelongToSet(this);
            }
        ...
        {
            var point = new MapPoint(cell.Id);
            point.SetFromCellId();
        ...
        public DirectionsEnum OrientationToX(MapPoint point, bool diagonal = false)
        {
            var dx = point.X - m_x;
            ...
            var angleInDegrees = angleInRadians * 180 / Math.PI;
            var transformedAngle = angleInDegrees;
        ...
        public DirectionsEnum OrientationToY(MapPoint point, bool diagonal = false)
        {
            var dx = 0;
            var dy = m_y - point.Y;
