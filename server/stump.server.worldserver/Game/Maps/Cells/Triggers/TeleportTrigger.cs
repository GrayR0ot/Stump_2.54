...
namespace Stump.Server.WorldServer.Game.Maps.Cells.Triggers
{
    ...
    public class TeleportTrigger : CellTrigger
    {
        ...
        {
            get => m_destinationCellId ?? (m_destinationCellId = Record.GetParameter<short>(0)).Value;
            set
            {
        ...
        {
            get => m_destinationMapId ?? (m_destinationMapId = Record.GetParameter<int>(1)).Value;
            set
            {
        ...
        {
            var map = World.Instance.GetMap(DestinationMapId);

            if (map == null)
                throw new Exception(string.Format("Cannot load CellTeleport id={0}, DestinationMapId {1} isn't found",
                    Record.Id, DestinationMapId));

            var cell = map.Cells[DestinationCellId];
        ...
        public override void Apply(Character character)
        {
            ...
            {
                character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_ERROR,
                    1); //Certaines conditions ne sont pas satisfaites
                return;
            }
