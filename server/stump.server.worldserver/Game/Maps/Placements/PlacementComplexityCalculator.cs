...
namespace Stump.Server.WorldServer.Game.Maps.Placements
{
    public class PlacementComplexityCalculator
    {
        ...
        public int Compute()
        {
            ...
            {
                distanceSum += groups.Where(entry => !exclusions.Contains(entry))
                    .Sum(entry => DistanceTo(entry.Center, group.Center));
        ...
        private List<Point> FindAllAdjacentsPoints(Point point, ICollection<Point> exclusions)
        {
            ...
            foreach (var adjacentPoint in GetAdjacentPoints(point).Where(entry => m_points.Contains(entry))
                .Where(adjacentPoint => !exclusions.Contains(adjacentPoint)))
            {
