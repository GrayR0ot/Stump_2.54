...
namespace Stump.Server.WorldServer.Game.Maps.Placements
{
    ...
    {
        [Variable] public static string PatternsDir = "./patterns";

        [Variable] public static byte SearchDeep = 5;

        [Variable] public static bool SortByComplexity = true;

        public List<PlacementPattern> m_patterns = new List<PlacementPattern>();
        private List<PlacementPattern> m_relativePatterns = new List<PlacementPattern>();

        [Initialization(InitializationPass.Fourth)]
        public void Initialize()
        {
            var temp_patterns = new List<PlacementPattern>();
            ...
            m_relativePatterns = temp_patterns.Where(x => x.Relative).ShuffleWithProbabilities(fixPatternsComplx)
                .ToList();
        }
        ...
        public void GeneratePattern(Map map, bool forceUpdate)
        {
            ...
            if (pattern != null)
            {
                map.Pattern = pattern;
                map.SetPlacements(
                    pattern.Blues.Select(x => map.Cells[MapPoint.CoordToCellId(x.X, x.Y)])
                        .Where(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight).ToArray(),
                    pattern.Reds.Select(x => map.Cells[MapPoint.CoordToCellId(x.X, x.Y)])
                        .Where(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight).ToArray());
            }
            else
            {
                foreach (var center in GetCellsCircle(map.Cells[300], map.Record, SearchDeep, 0))
                {
                    ...
                    pattern = m_relativePatterns.FirstOrDefault(x =>
                        x.TestPattern(centerPoint, map.Record) && x != map.Pattern);
                    ...
                    if (pattern != null)
                    {
                        ...
                        map.SetPlacements(
                            pattern.Blues
                                .Select(x =>
                                    map.Cells[MapPoint.CoordToCellId(x.X + centerPoint.X, x.Y + centerPoint.Y)])
                                .Where(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight).ToArray(),
                            pattern.Reds
                                .Select(x =>
                                    map.Cells[MapPoint.CoordToCellId(x.X + centerPoint.X, x.Y + centerPoint.Y)])
