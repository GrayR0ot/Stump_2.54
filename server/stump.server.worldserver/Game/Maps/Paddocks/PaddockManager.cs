...
namespace Stump.Server.WorldServer.Game.Maps.Paddocks
{
    public class PaddockManager : DataManager<PaddockManager>, ISaveable
    {
        ...
        public void Save()
        {
            ...
            foreach (var paddock in m_paddocks.Where(paddock =>
                paddock.Value.IsPublicPaddock() && paddock.Value.IsRecordDirty)) paddock.Value.Save(Database);
        }
        ...
        {
            m_paddocks = Database
                .Query<WorldMapPaddockRecord, MountRecord, WorldMapPaddockRecord>(new WorldMapPaddockRelator().Map,
                    WorldMapPaddockRelator.FetchQuery).Where(x => x.Map != null)
                .ToDictionary(entry => entry.Id, x => new Paddock(x));
