...
namespace Stump.Server.WorldServer.Game.Maps.Paddocks
{
    public class Paddock
    {
        ...
        public bool IsRecordDirty { get; private set; }

        public bool IsPublicPaddock()
        {
            return Guild == null;
        }
        ...
        {
            return IsPublicPaddock()
                ? character.GetPublicPaddockedMount(mountId)
                : PaddockedMounts.FirstOrDefault(x => x.Id == mountId);
        }
        ...
        {
            var properties = new PaddockInstancesInformations((ushort) MaxOutdoorMount, (ushort) MaxItems,
                IsPublicPaddock()
                    ? new PaddockBuyableInformations[0]
                    : new[] {new PaddockBuyableInformations((ulong) Price, Locked)});
