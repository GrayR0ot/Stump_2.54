...
namespace Stump.Server.WorldServer.Game.Maps.Spawns
{
    ...
    {
        [Variable(true)] public static int StaticSpawnsInterval = 3;

        private static readonly Logger logger = LogManager.GetCurrentClassLogger();

        private readonly Dictionary<MonsterGroup, MonsterStaticSpawn> m_groupsSpawn =
            new Dictionary<MonsterGroup, MonsterStaticSpawn>();
        ...
        public ReadOnlyCollection<MonsterStaticSpawn> Spawns => m_spawns.AsReadOnly();
        ...
        protected override MonsterGroup DequeueNextGroupToSpawn()
        {
            ...
            lock (m_locker)
            {
                if (m_spawnsQueue.Count == 0) return null;
