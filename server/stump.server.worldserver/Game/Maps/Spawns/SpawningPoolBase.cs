...
namespace Stump.Server.WorldServer.Game.Maps.Spawns
{
    ...
    public abstract class SpawningPoolBase
    {
        ...
        public int RemainingTime => State != SpawningPoolState.Running
            ? 0
            : (int) (SpawnTimer.NextTick - DateTime.Now).TotalMilliseconds;

        protected List<MonsterGroup> Spawns { get; set; }

        public int SpawnsCount => Spawns.Count;

        protected TimedTimerEntry SpawnTimer { get; set; }

        public SpawningPoolState State { get; private set; }
        ...
        public void StartAutoSpawn()
        {
            lock (this)
            {
                if (!Map.CanSpawnMonsters()) return;
