...
namespace Stump.Server.WorldServer.Game.Maps
{
    public class Area : IContextHandler
    {
        ...
        [Variable] public static readonly int DefaultUpdateDelay = 50;
        ...
        private readonly PriorityQueueB<TimedTimerEntry> m_timers =
            new PriorityQueueB<TimedTimerEntry>(new TimedTimerComparer());
        ...
        public SuperArea SuperArea { get; internal set; }

        public int ObjectCount => m_objects.Count;

        public int TimersCount => m_timers.Count;

        public List<IMessage> MessageQueue => m_messageQueue.ToList();

        public int MsgQueueCount => m_messageQueue.Count;
        ...
        public int CharacterCount => m_characters.Count;
        ...
        private void UpdateCallback(object state)
        {
            ...
            var updateDelta = (int) (updateStart - LastUpdateTime).TotalMilliseconds;
            ...
            try
            {
                ...
                foreach (var timer in m_pausedTimers.Where(timer => timer.Enabled)) m_timers.Push(timer);
                        ...
                        try
                        {
                            ...
                            if (BenchmarkManager.Enable && swMsg.Elapsed.TotalMilliseconds > 20)
                                processedMessages.Add(
                                    BenchmarkEntry.Create(timer.ToString(), swMsg.Elapsed, "area", Id));
                ...
                try
                {
                    ...
                    if (callbackTimeout < 0)
                    {
                        ...
                        logger.Debug("Area '{0}' update lagged ({1}ms) (msg:{2}ms, timers:{3}ms, timerProc:{4}/{5})",
                            this, (int) newUpdateDelta.TotalMilliseconds, messageProcessTime, timerProcessingTime,
                            timerProcessed, m_timers.Count);
                        foreach (var msg in processedMessages.OrderByDescending(x => x.Timestamp).Take(15))
                            logger.Debug(msg);

                        BenchmarkManager.Instance.AddRange(processedMessages.OrderByDescending(x => x.Timestamp)
                            .Take(15));
                    }
        ...
        public void CheckDC()
        {
            ...
            if (count > 0) logger.Warn("{0} disconnected characters removed from {1}", count, this);
        }
        ...
        public void SpawnMaps()
        {
            ...
            foreach (var map in Maps.Where(map => map.MonsterSpawnsCount > 0)) map.EnableClassicalMonsterSpawns();
        }
        ...
        public void CallOnAllCharacters(Action<Character> action)
        {
            ExecuteInContext(() =>
            {
                foreach (var chr in m_characters) action(chr);
            });
        }
        ...
        public void AddMonsterSpawn(MonsterSpawn spawn)
        {
            ...
            foreach (var subArea in SubAreas) subArea.AddMonsterSpawn(spawn);
        }
        ...
        public void RemoveMonsterSpawn(MonsterSpawn spawn)
        {
            ...
            foreach (var subArea in SubAreas) subArea.RemoveMonsterSpawn(spawn);
        }
