...
namespace Stump.Server.WorldServer.Game.Maps.Pathfinding
{
    ...
    {
        [Variable(true)] public static int SearchLimit = 500;

        [Variable(true)] public static int EstimateHeuristic = 1;
        ...
        public ICellsInformationProvider CellsInformationProvider { get; }
        ...
        public Path FindPath(MapPoint startPoint, MapPoint endPoint, bool diagonal, int movementPoints = -1)
        {
            var success = false;
            ...
            if (movementPoints == 0)
                return Path.GetEmptyPath(CellsInformationProvider.Map,
                    CellsInformationProvider.Map.Cells[startPoint.CellId]);
            ...
            if (movementPoints > 0 && closedList.Count + 1 > movementPoints)
                return new Path(CellsInformationProvider.Map,
                    closedList.Take(movementPoints + 1)
                        .Select(entry => CellsInformationProvider.Map.Cells[entry.Cell]));

            return new Path(CellsInformationProvider.Map,
                closedList.Select(entry => CellsInformationProvider.Map.Cells[entry.Cell]));
        }
        ...
        internal class ComparePfNodeMatrix : IComparer<short>
        {
            ...
            {
                if (m_matrix[a].F > m_matrix[b].F) return 1;
