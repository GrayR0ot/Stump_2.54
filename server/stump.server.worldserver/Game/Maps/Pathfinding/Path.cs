...
namespace Stump.Server.WorldServer.Game.Maps.Pathfinding
{
    public class Path
    {
        ...
        public Cell StartCell => m_cellsPath[0];

        public Cell EndCell => m_cellsPath[m_cellsPath.Length - 1];

        public ObjectPosition EndPathPosition => m_endPathPosition ??
                                                 (m_endPathPosition =
                                                     new ObjectPosition(Map, EndCell, GetEndCellDirection()));

        public int MPCost => m_cellsPath.Length - 1;
