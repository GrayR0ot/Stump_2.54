...
namespace Stump.Server.WorldServer.Game.Maps
{
    public class Map : WorldObjectsContext, ICharacterContainer
    {
        ...
        [Variable(true)] public static int AutoMoveActorMaxInverval = 40;

        [Variable(true)] public static int AutoMoveActorMinInverval = 20;

        [Variable(true)] public static int InteractiveUseAdditionalDelay = 500;
        ...
        public PlacementPattern Pattern { get; set; }
        ...
        public void UpdateFightPlacements(byte nbFighters = 0)
        {
            ...
            {
                BlueFightCells = new Cell[0];
                RedFightCells = new Cell[0];

                PlacementManager.Instance.GeneratePattern(this, false);
                //if (m_bluePlacement.Count() < nbFighters || m_bluePlacement.Count() == 0)

                if (BlueFightCells.Count() < 8 && BlueFightCells.Count() < 8)
                {
                    if (Record.BlueFightCells.Length == 0 || Record.RedFightCells.Length == 0)
                    {
                        if (Cells.Any(x => x.Red) && Cells.Any(x => x.Blue))
                        {
                            BlueFightCells = (from x in Cells
                                where x.Blue
                                select x).ToArray();
                            RedFightCells = (from x in Cells
                                where x.Red
                                select x).ToArray();
                        }
                        else if (Cells.Count(x => x.Walkable && !x.FarmCell && !x.NonWalkableDuringFight) > 16)
                        {
                            ...
                            var shape = enums
                                .Where(x => x != SpellShapeEnum.I && x != SpellShapeEnum.A && x != SpellShapeEnum.a)
                            ...
                            var fightcells = zone
                                .GetCells(GetRandomWalkableCell(x => !x.FarmCell && !x.NonWalkableDuringFight), this)
                                .Where(x => x.Walkable && !x.NonWalkableDuringRP && !interacCells.Contains(x) &&
                                            !x.NonWalkableDuringFight);
                            if (fightcells.Count() < 16)
                            {
                                ...
                                while (cells.Count != 16)
                                {
                                    ...
                                    if (!cells.Any(x => x.Id == tata))
                                        if (Cells[tata].Walkable && !Cells[tata].FarmCell &&
                                            !Cells[tata].NonWalkableDuringFight && !Cells[tata].NonWalkableDuringRP &&
                                            !interacCells.Contains(Cells[tata]))
                                            cells.Add(Cells[tata]);
                                }

                                BlueFightCells = new Cell[8];
                                RedFightCells = new Cell[8];

                                Array.Copy(cells.ToArray(), 0, BlueFightCells, 0, 8);
                                Array.Copy(cells.ToArray(), 8, RedFightCells, 0, 8);
                            }
                            else
                            {
                                var half = fightcells.Count() / 2;
                                var secondhalf = fightcells.Count() - half;
                                BlueFightCells = new Cell[half];
                                RedFightCells = new Cell[secondhalf];
                                Array.Copy(fightcells.ToArray(), 0, BlueFightCells, 0, half);
                                Array.Copy(fightcells.ToArray(), half, RedFightCells, 0, secondhalf);
                    ...
                    {
                        BlueFightCells = Record.BlueFightCells.Select(entry => Cells[entry]).ToArray();
                        RedFightCells = Record.RedFightCells.Select(entry => Cells[entry]).ToArray();
                    }
        ...
        /*public void UpdateFightPlacements(byte nbFighters = 0)
        {
            ...
            if (Record.BlueFightCells.Length == 0 || Record.RedFightCells.Length == 0)
            {
                if (1 == 1)
                {
                    PlacementPattern placementPattern = PlacementManager.Instance.m_patterns.First();
                    Point centerPoint = placementPattern.Center;
                    Cell centerCell = this.Cells[MapPoint.CoordToCellId(centerPoint.X, centerPoint.Y)];
                    foreach (Point point in placementPattern.Blues)
                    {
                        m_bluePlacement.Add(this.Cells[MapPoint.CoordToCellId(point.X, point.Y)]);
                    }
                    foreach (Point point in placementPattern.Reds)
                    {
                        m_redPlacement.Add(this.Cells[MapPoint.CoordToCellId(point.X, point.Y)]);
                    }
                }
                else
                {
        ...
        {
            BlueFightCells = bluePlacement;
            RedFightCells = redPlacement;
        }
        ...
        private readonly ConcurrentDictionary<int, RolePlayActor> m_actorsMap =
            new ConcurrentDictionary<int, RolePlayActor>();
        ...
        private readonly Dictionary<Cell, List<CellTrigger>>
            m_cellsTriggers = new Dictionary<Cell, List<CellTrigger>>();
        ...
        public override Cell[] Cells => Record.Cells;

        protected override IReadOnlyCollection<WorldObject> Objects => Actors;

        public IReadOnlyCollection<RolePlayActor> Actors => m_actors.AsReadOnly();

        public MapCellsInformationProvider CellsInfoProvider { get; private set; }

        public SubArea SubArea { get; internal set; }

        public Area Area => SubArea.Area;
        ...
        public ReadOnlyCollection<SpawningPoolBase> SpawningPools => m_spawningPools.AsReadOnly();

        public uint RelativeId => Record.RelativeId;

        public int MapType => Record.MapType;

        public Point Position => Record.Position.Pos;

        public bool Outdoor => Record.Outdoor;

        public int Capabilities => Record.Position != null ? Record.Position.Capabilities : 0xFFFF;

        public int TopNeighbourId
        {
            get => Record.TopNeighbourId;
            set => Record.TopNeighbourId = value;
        }

        public Map TopNeighbour
        {
            get => TopNeighbourId != -1
                ? m_topNeighbour ?? (m_topNeighbour = World.Instance.GetMap(TopNeighbourId))
                : null;
            set
            {
        ...
        {
            get => Record.BottomNeighbourId;
            set => Record.BottomNeighbourId = value;
        }

        public Map BottomNeighbour
        {
            get => BottomNeighbourId != -1
                ? m_bottomNeighbour ?? (m_bottomNeighbour = World.Instance.GetMap(BottomNeighbourId))
                : null;
            set
            {
        ...
        {
            get => Record.RightNeighbourId;
            set => Record.RightNeighbourId = value;
        }

        public Map RightNeighbour
        {
            get => RightNeighbourId != -1
                ? m_rightNeighbour ?? (m_rightNeighbour = World.Instance.GetMap(RightNeighbourId))
                : null;
            set
            {
        ...
        {
            get => Record.ShadowBonusOnEntities;
            set => Record.ShadowBonusOnEntities = value;
        }

        public bool UseLowpassFilter
        {
            get => Record.UseLowpassFilter;
            set => Record.UseLowpassFilter = value;
        }

        public bool UseReverb
        {
            get => Record.UseReverb;
            set => Record.UseReverb = value;
        }

        public int PresetId => Record.PresetId;
        ...
        public InteractiveObject Zaap
        {
            get
            {
                ...
                if (interactives != null && interactives.Count() > 0)
                    return interactives.FirstOrDefault(x =>
                        x.Template != null && x.Template.Type == InteractiveTypeEnum.TYPE_ZAAP);
                return null;
            }
        }
        ...
        public InteractiveObject Zaapi
        {
            get
            {
                ...
                if (interactives != null && interactives.Count() > 0)
                    return interactives.FirstOrDefault(x =>
                        x.Template != null && x.Template.Type == InteractiveTypeEnum.TYPE_ZAAPI);
                return null;
            }
        }
        ...
        protected TimedTimerEntry DroppedItemsCleaner { get; set; }

        public JobTemplate[] AvailableJobs { get; private set; }

        public bool IsInstantiated => Record.IsInstantiated;

        #endregion

        #region Restrictions

        public bool AllowChallenge => (Capabilities & 1) != 0;

        public bool AllowAggression => (Capabilities & 2) != 0;

        public bool AllowTeleportTo => (Capabilities & 4) != 0;

        public bool AllowTeleportFrom => (Capabilities & 8) != 0;

        public bool AllowExchangesBetweenPlayers => (Capabilities & 16) != 0;

        public bool AllowHumanVendor => (Capabilities & 32) != 0;

        public bool AllowCollector => (Capabilities & 64) != 0;

        public bool AllowSoulCapture => (Capabilities & 128) != 0;

        public bool AllowSoulSummon => (Capabilities & 256) != 0;

        public bool AllowTavernRegen => (Capabilities & 512) != 0;

        public bool AllowTombMode => (Capabilities & 1024) != 0;

        public bool AllowTeleportEverywhere => (Capabilities & 2048) != 0;

        public bool AllowFightChallenges => (Capabilities & 4096) != 0;
        ...
        public void UnSpawnNpc(Npc npc)
        {
            if (GetActor<Npc>(npc.Id) != npc)
                throw new Exception(
                    string.Format("Npc with id {0} not found, cannot unspawn an unexistant npc", npc.Id));
        ...
        public bool UseInteractiveObject(Character character, int interactiveId, int skillId)
        {
            ...
            var adjacentCells = interactiveObject.Position.Point.GetAdjacentCells(true).Where(x => x.IsInMap())
            ...
            if (delay > 0)
            {
                ...
                Area.CallDelayed(delay + InteractiveUseAdditionalDelay,
        ...
        {
            return BlueFightCells.Length > 0 && RedFightCells.Length > 0;
        }
        ...
        {
            foreach (var pool in SpawningPools.ToArray()) RemoveSpawningPool(pool);
        }

        public void EnableClassicalMonsterSpawns()
        {
            if (BlueFightCells == null || RedFightCells == null)
                return;
            if (!CanSpawnMonsters())
                return;
            ...
            foreach (var pool in pools) pool.StartAutoSpawn();
        }

        public void DisableClassicalMonsterSpawns()
        {
            foreach (var actor in GetActors<MonsterGroup>().Where(actor =>
                    actor.GetMonsters().All(entry => MonsterSpawns.Any(spawn => spawn.MonsterId == entry.Template.Id)))
                .ToArray()) Leave(actor);

            foreach (var spawningPool in SpawningPools.OfType<ClassicalSpawningPool>()
                .Where(spawningPool => spawningPool.AutoSpawnEnabled)) spawningPool.StopAutoSpawn();
        }
        ...
        public MonsterGroup GenerateRandomMonsterGroup(int length)
        {
            ...
            var group = new MonsterGroup(GetNextContextualId(),
                new ObjectPosition(this, GetRandomFreeCellAnyTimeWalkable(), GetRandomDirection()));
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                foreach (var spawn in MonsterSpawns)
                {
                    ...
                    monster = MonsterManager.Instance.GetMonsterGrade(spawn.MonsterId,
                        SubArea.RollMonsterGrade(spawn.MinGrade, spawn.MaxGrade));
        ...
        {
            var group = new MonsterGroup(GetNextContextualId(),
                new ObjectPosition(this, GetRandomFreeCell(), GetRandomDirection()));

            foreach (var monster in monsterGroup.GetMonsters()) group.AddMonster(new Monster(monster.Grade, group));
        ...
        {
            foreach (var actor in Actors.Where(x =>
                x is IAutoMovedEntity && ((IAutoMovedEntity) x).NextMoveDate <= DateTime.Now))
            {
                ...
                var dest = circle.GetCells(actor.Cell, this)
                    .Where(entry => entry.Walkable && !entry.NonWalkableDuringRP && entry.MapChangeData == 0)
        ...
        public Cell[] BlueFightCells { get; set; }

        public Cell[] RedFightCells { get; set; }

        public Cell[] GetBlueFightPlacement()
        {
            return BlueFightCells == null ? new Cell[0] : BlueFightCells;
        }

        public Cell[] GetRedFightPlacement()
        {
            return RedFightCells == null ? new Cell[0] : RedFightCells;
        }
        ...
        public void Enter(RolePlayActor actor)
        {
            ...
            {
                if (!Clients.Contains((actor as Character).Client)) Clients.Add((actor as Character).Client);

                if (IsInstantiated)
                    (actor as Character).SendServerMessage(
                        "Cette carte est soumise à une instance temporelle, Xélor vous restreind le fait de pouvoir intéragir avec d'autres joueurs !");
            }
        ...
        public void Leave(WorldObjectItem objectItem)
        {
            ...
            if (m_objectItems.Remove(objectItem)) OnObjectItemLeave(objectItem);
        }
        ...
        {
            ForEach(x => { ContextRoleplayHandler.SendObjectGroundAddedMessage(x.Client, objectItem); });
        ...
        {
            ForEach(x => { ContextRoleplayHandler.SendObjectGroundRemovedMessage(x.Client, objectItem); });
        ...
        private void OnEnter(RolePlayActor actor)
        {
            ...
            if (actor is IAutoMovedEntity)
            {
                ...
                if (m_autoMoveTimer == null) // call every (max+min)/2/10 to have an average 5% accuracy
                    m_autoMoveTimer = Area.CallPeriodically(
                        (AutoMoveActorMaxInverval + AutoMoveActorMinInverval) / 20 * 1000,
                        MoveRandomlyActors);
            }
        ...
        private void OnLeave(RolePlayActor actor)
        {
            ...
            if (actor == TaxCollector) TaxCollector = null;
        ...
        private void OnActorStopMoving(ContextActor actor, Path path, bool canceled)
        {
            ...
            if (ExecuteTrigger(CellTriggerType.END_MOVE_ON, actor.Cell, character) ||
                ExecuteTrigger(CellTriggerType.MOVE_ON, actor.Cell, character))
                return;
        }
        ...
        /// </summary>
        public WorldClientCollection Clients => GetAllCharacters().ToClients();
        ...
        public Cell GetRandomWalkableCell(Predicate<Cell> predicate)
        {
            ...
            if (cells.Count() < 1) cells = Cells.Where(x => x.Walkable);
        ...
        public Cell GetFirstFreeCellInDirectionNear(FightActor actor, Cell goal)
        {
            ...
            var cells = positiongoal.GetCellsInLine(actor.Position.Point).FirstOrDefault(x =>
                actor.Map.IsCellWalkable(actor.Map.GetCell(x.CellId)) && x.CellId != positiongoal.CellId &&
                actor.Fight.Fighters.All(y => y.Position.Point.CellId != x.CellId));
        ...
        public IEnumerable<MapObstacle> GetMapObstacles()
        {
            ...
            foreach (var skill in GetInteractiveObjects().SelectMany(x => x.GetSkills())
                .Where(skill => skill is SkillAnimate)) mapObstacles.AddRange(((SkillAnimate) skill).Obstacles);

            return mapObstacles.OrderByDescending(x => x.State == (sbyte) MapObstacleStateEnum.OBSTACLE_OPENED)
                .DistinctBy(x => x.ObstacleCellId).ToList();
        }
        ...
        public void MoveCharactersToWalkableCell()
        {
            foreach (var character in GetAllCharacters())
            {
                ...
                var cell = GetCell(character.Position.Point.GetCellInDirection(DirectionsEnum.DIRECTION_SOUTH_WEST, 1)
                    .CellId);

                if (!cell.Walkable)
                {
                    var adjacents = character.Position.Point.GetAdjacentCells(entry => GetCell(entry).Walkable)
                        .ToArray();
        ...
        {
            return cell.Walkable || GetMapObstacles().Any(x =>
                x.ObstacleCellId == cell.Id && x.State == (sbyte) MapObstacleStateEnum.OBSTACLE_OPENED);
        }
        ...
        public Cell GetRandomAdjacentFreeCell(MapPoint cell, bool actorFree = false)
        {
            ...
            {
                var excludedCells = GetActors<RolePlayActor>().Select(entry => entry.Cell.Id);
                var cells = cell.GetAdjacentCells(entry =>
                    CellsInfoProvider.IsCellWalkable(entry) && !excludedCells.Contains(entry));
        ...
        {
            return SpawningPools.Any(x => x is DungeonSpawningPool);
        }
        ...
        public bool HasSentCIDM { get; set; }

        public MapComplementaryInformationsDataMessage GetMapComplementaryInformationsDataMessage(Character character)
        {
            var interractives = m_interactives.Where(entry => entry.Value.CanBeSee(character))
                .Select(entry => entry.Value.GetInteractiveElementWithBonus(character)).ToArray();
            ...
            try
            {
                if (HavenBagManager.Instance.IsHavenBagMap(Record.Id))
                {
                    var mcifihm = new MapComplementaryInformationsDataInHavenBagMessage(
                        ...
                        m_actors.Where(entry => entry.CanBeSee(character)).Select(entry =>
                                entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations)
                            .ToArray(),
                        m_interactives.Where(entry => entry.Value.CanBeSee(character))
                            .Select(entry => entry.Value.GetInteractiveElementWithBonus(character)).ToArray(),
                        m_interactives.Where(entry => entry.Value.CanBeSee(character)).Where(x => x.Value.Animated)
                            .Select(entry => entry.Value.GetStatedElement()).ToArray(),
            ...
            catch (Exception ex)
            {
                return new MapComplementaryInformationsWithCoordsMessage(
                    ...
                    new HouseInformations[0],
                    m_actors.Where(entry => entry.CanBeSee(character)).Select(entry =>
                        entry.GetGameContextActorInformations(character) as GameRolePlayActorInformations).ToArray(),
                    interractives,
                    m_interactives.Where(entry => entry.Value.CanBeSee(character)).Where(x => x.Value.Animated)
                        .Select(entry => entry.Value.GetStatedElement()).ToArray(),
                    GetMapObstacles().ToArray(),
