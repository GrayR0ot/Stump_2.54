...
namespace Stump.Server.WorldServer.Game.Parties
{
    ...
    {
        public PartyTeleport(short dungeonId, int inviterId, IEnumerable<int> invalidIds, Party party, Map mapId,
            int cellId)
        {
            DungeonId = dungeonId;
            InviterId = inviterId;
        ...
        public IEnumerable<int> InvalidersIds { get; set; }

        public void Open()
        {
            Party.Members.ToClients()
                .Send(new TeleportBuddiesRequestedMessage(DungeonId, InviterId,
                    InvalidersIds.Select(i => (long) i).ToList()));
            Party.Members.Where(x => x.Id != InviterId && !InvalidersIds.Contains(x.Id)).ToClients()
                .Send(new TeleportToBuddyOfferMessage((ushort) DungeonId, (ulong) InviterId, 900));
