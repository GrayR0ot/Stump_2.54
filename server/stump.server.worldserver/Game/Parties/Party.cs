...
namespace Stump.Server.WorldServer.Game.Parties
{
    public class Party
    {
        ...
        public PartyTeleport TeleportRequest { get; set; }

        public WorldClientCollection Clients { get; } = new WorldClientCollection();

        public virtual PartyTypeEnum Type => PartyTypeEnum.PARTY_TYPE_CLASSICAL;

        public virtual int MembersLimit => MaxMemberCount;

        public bool IsFull => m_members.Count >= MembersLimit;

        public int GroupLevelSum { get; private set; }

        public int GroupLevelAverage { get; private set; }
        ...
        public IEnumerable<Character> Members => m_members;

        public IEnumerable<Character> Guests => m_guests;
        ...
        public bool RestrictFightToParty { get; private set; }

        public IdolInventory IdolInventory { get; }
        ...
        public virtual bool CanInvite(Character character, out PartyJoinErrorEnum error, Character inviter = null,
            bool send = true)
        {
        ...
        {
            foreach (var guest in m_guests.ToArray()) guest.DenyAllInvitations(this);
        }
        ...
        public void RemoveMember(Character character)
        {
            lock (m_memberLocker)
            {
                ...
                else if (Leader == character) ChangeLeader(m_members.First());
            }
        }
        ...
        public virtual void Kick(Character member)
        {
            lock (m_memberLocker)
            {
                ...
                else if (Leader == member) ChangeLeader(m_members.First());
            }
        }
        ...
        public void ForEach(Action<Character> action)
        {
            lock (m_memberLocker)
            {
                foreach (var character in Members) action(character);
            }
        }
        ...
        public void ForEach(Action<Character> action, Character except)
        {
            lock (m_memberLocker)
            {
                foreach (var character in Members.Where(character => character != except)) action(character);
            }
        }
            ...
            if (character.Area.Id == 68)
                character.Area.AddMessage(() =>
                {
                    ...
                    if (character.Fight is FightAgression)
                        ...
                        PartyHandler.SendPartyMemberInBreachFightMessage(clients, this, character,
                            PartyFightReasonEnum.MONSTER_ATTACK, character.Fight);
                });
            else
                character.Area.AddMessage(() =>
                {
        ...
        {
            foreach (var member in Members) UnBindEvents(member);
        }
        ...
        {
            Clients.Remove(groupGuest.Client);
        ...
        {
            Clients.Add(groupMember.Client);
        ...
        {
            Clients.Remove(groupMember.Client);
