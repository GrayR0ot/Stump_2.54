...
namespace Stump.Server.WorldServer.Game.Arena
{
    public class ArenaPreFight
    {
        ...
        public ArenaRecord Arena { get; }

        public ArenaPreFightTeam DefendersTeam { get; }

        public ArenaPreFightTeam ChallengersTeam { get; }
        ...
        private void OnMemberRemoved(ArenaPreFightTeam arg1, ArenaWaitingCharacter arg2)
        {
            ...
            Clients.Remove(arg2.Character.Client);
        ...
        private void OnMemberAdded(ArenaPreFightTeam arg1, ArenaWaitingCharacter arg2)
        {
            ...
            Clients.Add(arg2.Character.Client);
        }
        ...
        private void OnFightDenied(ArenaWaitingCharacter obj)
        {
            ...
            {
                ContextHandler.SendGameRolePlayArenaFighterStatusMessage(Clients, Id, obj.Character, false);
                obj.Character.ToggleArenaWaitTime();

                // %1 a refusé le combat en Kolizéum.
                BasicHandler.SendTextInformationMessage(Clients, TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 275,
                    obj.Character.Name);
                ...
                if (obj.Character.ArenaParty != null)
                {
                    foreach (var character in obj.Team.Members.ToArray())
                    {
                        ...
                        character.Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE,
                            273);
                    }
                ...
                foreach (var character in DefendersTeam.Members.Concat(ChallengersTeam.Members)
                    .Where(character => character.Character.ArenaPopup != null))
        ...
        private void OnReadyChanged(ArenaWaitingCharacter character, bool ready)
        {
            ArenaManager.Instance.ArenaTaskPool.ExecuteInContext(() =>
            {
                ContextHandler.SendGameRolePlayArenaFighterStatusMessage(Clients, Id, character.Character, ready);
        ...
        private void OnFightLeft(RolePlayActor rolePlayActor, Map map)
        {
            ...
            if (Interlocked.Decrement(ref m_readyPlayersCount) <= 0) m_fight.Map.Area.AddMessage(PrepareFight);
        }
