...
namespace Stump.Server.WorldServer.Game.Arena
{
    ...
    public class VersusLeagueData
    {
        ...
        public LeaguesEnum Type { get; set; }
    ...
    {
        [Variable] public static int MaxPlayersPerFights = 1;

        [Variable] public static int ArenaMinLevel = 50;

        [Variable] public static int ArenaMaxLevelDifference = 100;
        ...
        ///     is seconds
        /// </summary>
        [Variable] public static int ArenaMatchmakingInterval = 10;
        ...
        [Variable] public static int[] ArenaMapsId =
        {
            177475586,
            177474562,
            177473538,
        ...
        public ItemTemplate TokenItemTemplate => m_tokenTemplate ??
                                                 (m_tokenTemplate =
                                                     ItemManager.Instance.TryGetTemplate(
                                                         (int) ItemIdEnum.KOLIZETON_12736));
        ...
        private void InitializeLeaguesDatas()
        {
            ...
            var currentrank = 1;
            var savedgoldfix = new Tuple<int, int>(0, 0);
        ...
        public ArenaLeague GetNextLeague(ArenaLeague league)
        {
            return Leagues.FirstOrDefault(x => x.LeagueId == league.NextLeagueId);
        }

        public ArenaLeague GetLeague(int leagueId)
        {
            return Leagues.FirstOrDefault(x => x.LeagueId == leagueId);
        }

        public List<ArenaLeague> GetLeaguesByType(LeaguesEnum type)
        {
            return Leagues.Where(x => x.Type == type).ToList();
        }
    }
}