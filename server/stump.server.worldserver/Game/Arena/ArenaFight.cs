...
namespace Stump.Server.WorldServer.Game.Arena
{
    public class ArenaFight : Fight<ArenaTeam, ArenaTeam>
    {
        ...
        {
            var challengersRank =
                (int) ChallengersTeam.GetAllFightersWithLeavers().OfType<CharacterFighter>().Average(x =>
                    x.Character.ArenaMode == 1 ? x.Character.ArenaRank_1vs1 : x.Character.ArenaRank_3vs3);
            var defendersRank =
                (int) DefendersTeam.GetAllFightersWithLeavers().OfType<CharacterFighter>().Average(x =>
                    x.Character.ArenaMode == 1 ? x.Character.ArenaRank_1vs1 : x.Character.ArenaRank_3vs3);
            ...
            var results = (from fighter in GetFightersAndLeavers().OfType<CharacterFighter>()
                ...
                select new ArenaFightResult(fighter, outcome, fighter.Loot,
                    ArenaRankFormulas.AdjustRank(
                        ArenaMode == 1 ? fighter.Character.ArenaRank_1vs1 : fighter.Character.ArenaRank_3vs3,
            ...
            {
                var log_insertKoli = "INSERT INTO logs_koli (FightId, Duration, Team, isWinner, " +
                                     "AccountId, AccountName, CharacterId, " +
                                     "CharacterName, IPAddress, ClientKey, Date) " +
                                     "VALUES" +
                                     $"('{UniqueId.ToString()}', '{GetFightDuration().TotalSeconds}', '{Enum.GetName(typeof(TeamEnum), playerResult.Fighter.Team.Id)}'," +
        ...
        protected override IEnumerable<IFightResult> GenerateLeaverResults(CharacterFighter leaver,
            out IFightResult leaverResult)
        {
            ...
            foreach (var fighter in GetFightersAndLeavers().OfType<CharacterFighter>())
            {
                ...
                var result = new ArenaFightResult(fighter, outcome, new FightLoot(), fighter == leaver ? rankLoose : 0,
                    false);
        ...
        {
            var opposedTeamRank = (int) character.OpposedTeam.GetAllFightersWithLeavers().OfType<CharacterFighter>()
                .Average(x => x.Character.ArenaMode == 1 ? x.Character.ArenaRank_1vs1 : x.Character.ArenaRank_3vs3);

            return ArenaRankFormulas.AdjustRank(
                character.Character.ArenaMode == 1
                    ? character.Character.ArenaRank_1vs1
                    : character.Character.ArenaRank_3vs3, opposedTeamRank, false);
        }
