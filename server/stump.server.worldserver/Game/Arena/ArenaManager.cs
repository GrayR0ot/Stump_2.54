...
namespace Stump.Server.WorldServer.Game.Arena
{
    public class ArenaManager : DataManager<ArenaManager>
    {
        ...
        ///     in minutes
        /// </summary>
        [Variable] public static int ArenaWaitTime = 10;

        /// <summary>
        ///     Kolizeum MapId that show all Arena Fights
        /// </summary>
        [Variable] public static int KolizeumMapId = 81788928;
        ...
        public ItemTemplate TokenItemTemplate => m_tokenTemplate ??
                                                 (m_tokenTemplate =
                                                     ItemManager.Instance.TryGetTemplate(12736)); // Caliston

        public SelfRunningTaskPool ArenaTaskPool { get; } = new SelfRunningTaskPool(ArenaUpdateInterval, "Arena");

        public Dictionary<int, ArenaRecord> Arenas_3vs3 { get; private set; }

        public Dictionary<int, ArenaRecord> Arenas_1vs1 { get; private set; }

        [Initialization(InitializationPass.Fifth)]
        public override void Initialize()
        {
            Arenas_3vs3 = Database.Query<ArenaRecord>(ArenaRelator_3vs3.FetchQuery).ToDictionary(x => x.Id);
            Arenas_1vs1 = Database.Query<ArenaRecord>(ArenaRelator_1vs1.FetchQuery).ToDictionary(x => x.Id);
        ...
        {
            if (Arenas_3vs3.Count == 0 || Arenas_1vs1.Count == 0)
                return false;
        ...
        public void AddToQueue(ArenaParty party)
        {
            ...
            foreach (var character in party.Members.Where(x => x != party.Leader))
                BasicHandler.SendTextInformationMessage(character.Client,
                    TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 272, party.Leader.Name);
        ...
        public void ComputeMatchmaking()
        {
            ...
            while ((current = queue.FirstOrDefault()) != null)
            {
                ...
                if (current.Character != null)
                    matchs = queue.Where(x =>
                        x.IsCompatibleWith(current, current.Character?.ArenaMode == 1 ? true : false)).ToList();
                else
        ...
        private void StartFight(IEnumerable<ArenaQueueMember> team1, IEnumerable<ArenaQueueMember> team2)
        {
            ...
            {
                var first = team1.SelectMany(x => x.EnumerateCharacters()).ToArray()[0];
                var second = team2.SelectMany(x => x.EnumerateCharacters()).ToArray()[0];

                arena = Arenas_1vs1.RandomElementOrDefault().Value;
                first.CharacterToSeekName = second.Account.Nickname;
                second.CharacterToSeekName = first.Account.Nickname;
            }
            else
            {
                arena = Arenas_3vs3.RandomElementOrDefault().Value;
            }
            ...
            var challengersParty = preFight.ChallengersTeam.Members
                                       .Select(x => x.Character.GetParty(PartyTypeEnum.PARTY_TYPE_ARENA))
                                       .FirstOrDefault() ??
                                   PartyManager.Instance.Create(PartyTypeEnum.PARTY_TYPE_ARENA);
            var defendersParty = preFight.DefendersTeam.Members
                                     .Select(x => x.Character.GetParty(PartyTypeEnum.PARTY_TYPE_ARENA))
                                     .FirstOrDefault() ??
                                 PartyManager.Instance.Create(PartyTypeEnum.PARTY_TYPE_ARENA);
            ...
            foreach (var character in preFight.ChallengersTeam.Members.Select(x => x.Character)
                .Where(character => !challengersParty.IsInGroup(character)))
            ...
            foreach (var character in preFight.DefendersTeam.Members.Select(x => x.Character)
                .Where(character => !defendersParty.IsInGroup(character)))
