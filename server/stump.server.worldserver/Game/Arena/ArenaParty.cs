...
namespace Stump.Server.WorldServer.Game.Arena
{
    public class ArenaParty : Party
    {
        ...
        public override PartyTypeEnum Type => PartyTypeEnum.PARTY_TYPE_ARENA;

        public override int MembersLimit => MaxArenaMemberCount;

        public int GroupRankAverage { get; private set; }

        public override bool CanInvite(Character character, out PartyJoinErrorEnum error, Character inviter = null,
            bool send = true)
        {
            if (Members.Any())
            {
                ...
                if (Math.Max(character.Level, upper) - Math.Min(character.Level, lower) >
                    ArenaManager.ArenaMaxLevelDifference)
                {
