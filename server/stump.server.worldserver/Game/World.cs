...
namespace Stump.Server.WorldServer.Game
{
    public class World : DataManager<World>
    {
        ...
        private readonly ConcurrentDictionary<int, Character> m_charactersById =
            new ConcurrentDictionary<int, Character>(Environment.ProcessorCount,
                ClientManager.MaxConcurrentConnections);

        private readonly ConcurrentDictionary<string, Character> m_charactersByName =
            new ConcurrentDictionary<string, Character>(Environment.ProcessorCount,
                ClientManager.MaxConcurrentConnections, StringComparer.OrdinalIgnoreCase);
        ...
        private Dictionary<int, WorldMapGraveyardRecord> m_graveyards = new Dictionary<int, WorldMapGraveyardRecord>();
        ...
        public int CharacterCount => m_characterCount;

        public object SaveLock { get; } = new object();
        ...
        public void Save()
        {
            lock (SaveLock)
            {
                ...
                foreach (var client in clients)
                    try
                    {
                        if (client.Character != null) client.Character.SaveNow();
                    }
                    catch (Exception ex)
                    {
        ...
        {
            foreach (var area in m_areas) area.Value.Stop(wait);
        }
        ...
        public void Resume()
        {
            ...
            foreach (var pausedArea in m_pausedAreas) pausedArea.Start();
        ...
        public void SendAnnounceAllPlayersShutDown() // 
        {
            var Joueurco =
                Instance.GetCharacters()
                    .ToList(); // Le serveur vérifie les personnages qui sont connecté dans la Base de donnée
            foreach (var character in Joueurco)
                character.SendSystemMessage(3,
                    true); // Le serveur envoie  à tout les joueurs connecté un message de redémarrage serveur
        }

        public void SendAnnounceAllPlayersEvent() // Nom au hasard (Mettre un nom reconnaissable quand même...)
        {
            var connectedcharacter =
                Instance.GetCharacters()
                    .ToList(); // Le serveur vérifie les personnages qui sont connecté dans la Base de donnée
            foreach (var character in connectedcharacter)
                character.DisplayNotification(
                    "<br>UN EVENT VA COMMENCER</br> Prépare toi à gagner ! "); // Affichage temporaire (Notification)
        }
        ...
        {
            if (m_spacesLoaded) UnSetLinks();

            m_spacesLoaded = true;

            logger.Info("Load maps...");
            m_maps = Database.Query<MapRecord, MapPositionRecord, MapRecord>
                    (new MapRecordRelator().Map, MapRecordRelator.FetchQuery)
            ...
            m_areas = Database.Query<AreaRecord>(AreaRecordRelator.FetchQuery)
                .ToDictionary(entry => entry.Id, entry => new Area(entry));

            logger.Info("Load super areas...");
            m_superAreas = Database.Query<SuperAreaRecord>(SuperAreaRecordRelator.FetchQuery)
                .ToDictionary(entry => entry.Id, entry => new SuperArea(entry));

            logger.Info("Load graveyards...");
            m_graveyards = Database.Query<WorldMapGraveyardRecord>(WorldMapGraveyardRelator.FetchQuery)
        ...
        {
            if (m_spacesSpawned) UnSpawnSpaces();
        ...
        private void SetLinks()
        {
            ...
            {
                SubArea subArea;
                if (m_subAreas.TryGetValue(map.Record.Position.SubAreaId, out subArea)) subArea.AddMap(map);
            }

            foreach (var subArea in m_subAreas.Values)
            {
                Area area;
                if (m_areas.TryGetValue(subArea.Record.AreaId, out area)) area.AddSubArea(subArea);
            }

            foreach (var area in m_areas.Values)
            {
                SuperArea superArea;
                if (m_superAreas.TryGetValue(area.Record.SuperAreaId, out superArea)) superArea.AddArea(area);
            }
        }
        ...
        private void UnSetLinks()
        {
            ...
            {
                SubArea subArea;
                if (m_subAreas.TryGetValue(map.Record.Position.SubAreaId, out subArea)) subArea.RemoveMap(map);
            }

            foreach (var subArea in m_subAreas.Values)
            {
                Area area;
                if (m_areas.TryGetValue(subArea.Record.AreaId, out area)) area.RemoveSubArea(subArea);
            }

            foreach (var area in m_areas.Values)
            {
                SuperArea superArea;
                if (m_superAreas.TryGetValue(area.Record.SuperAreaId, out superArea)) superArea.RemoveArea(area);
            }
        }
        ...
        public void UnSpawnNpcs()
        {
            ...
            {
                var npcsToDelete = new List<Npc>();
                var npcs = current.Actors.Where(x => x is Npc);
                foreach (var npc in npcs) npcsToDelete.Add(npc as Npc);
                foreach (var npc in npcsToDelete) current.UnSpawnNpc(npc);
            }
        }
        ...
        public void UnSpawnSpaces()
        {
            ...
            {
                foreach (var interactive in map.GetInteractiveObjects().ToArray()) map.UnSpawnInteractive(interactive);

                foreach (var pool in map.SpawningPools.ToArray()) map.RemoveSpawningPool(pool);

                foreach (var trigger in map.GetTriggers().ToArray()) map.RemoveTrigger(trigger);
            }
        ...
        public void SpawnInteractives()
        {
            foreach (var interactive in InteractiveManager.Instance.GetInteractiveSpawns())
            {
                ...
                if (map == null)
                {
                    logger.Error("Cannot spawn interactive id={0} : map {1} doesn't exist", interactive.Id,
                        interactive.MapId);
                    continue;
                }
        ...
        public void UnSpawnInteractives()
        {
            foreach (var map in m_maps.Values)
            {
                ...
                foreach (var interactive in interactives) map.UnSpawnInteractive(interactive);
            }
        }
        ...
        public void SpawnCellTriggers()
        {
            foreach (var trigger in CellTriggerManager.Instance.GetCellTriggers()
                .Select(cellTrigger => cellTrigger.GenerateTrigger())) trigger.Position.Map.AddTrigger(trigger);
        }
        ...
        public void UnSpawnCellTriggers()
        {
            foreach (var map in m_maps.Values)
            {
                ...
                foreach (var trigger in triggers) map.RemoveTrigger(trigger);
            }
        }
        ...
        private void SpawnMonsters()
        {
            foreach (var spawn in MonsterManager.Instance.GetMonsterSpawns())
            {
                ...
                else if (spawn.SubArea != null) spawn.SubArea.AddMonsterSpawn(spawn);
            }
        ...
        {
            foreach (var merchant in from spawn in MerchantManager.Instance.GetMerchantSpawns()
        ...
        {
            return m_maps.Values.FirstOrDefault(entry =>
                entry.Position.X == x && entry.Position.Y == y && entry.Outdoor == outdoor);
        }
        ...
        {
            return m_maps.Values
                .Where(entry => entry.Position.X == x && entry.Position.Y == y && entry.Outdoor == outdoor).ToArray();
        }
        ...
        public void Enter(Character character)
        {
            ...
            if (m_charactersById.TryAdd(character.Id, character) &&
                m_charactersByName.TryAdd(character.Name, character))
            {
        ...
        {
            Character dummy;
            if (m_charactersById.TryRemove(character.Id, out dummy) &&
                m_charactersByName.TryRemove(character.Name, out dummy))
            {
