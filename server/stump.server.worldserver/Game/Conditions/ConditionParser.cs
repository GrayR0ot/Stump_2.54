...
namespace Stump.Server.WorldServer.Game.Conditions
{
    ...
    {
        private readonly Dictionary<int, int> m_parentheses = new Dictionary<int, int>();
                ...
                if (op != null)
                {
                    if (i + 1 >= String.Length)
                        throw new Exception(string.Format(
                            "Cannot parse {0} :  Right Expression of bool operator index {1} is empty", String, i));

                    var leftStr = String.Substring(0, i);

                    if (string.IsNullOrEmpty(leftStr))
                        throw new Exception(string.Format(
                            "Cannot parse {0} : Left Expression of bool operator index {1} is empty", String, i));
            ...
            while (index < String.Length)
            {
                ...
                if (op != null)
                {
                    if (index + 1 >= String.Length)
                        throw new Exception(string.Format(
                            "Cannot parse {0} :  Right Expression of comparaison operator index {1} is empty", String,
                            index));
                    ...
                    if (string.IsNullOrEmpty(leftStr))
                        throw new Exception(string.Format(
                            "Cannot parse {0} : Left Expression of comparaison operator index {1} is empty", String,
                            index));
        ...
        private void ParseParentheses()
        {
            ...
            for (var i = 0; i < String.Length; i++)
            {
                if (String[i] == '(') openIndexes.Push(i);

                if (String[i] == ')' && openIndexes.Count > 0)
                    m_parentheses.Add(openIndexes.Pop(), i);
                else if (String[i] == ')' && openIndexes.Count <= 0)
                    throw new Exception(string.Format(
                        "Cannot evaluate {0} : Parenthese at index {1} is not binded to an open parenthese", String,
                        i));
            }

            if (openIndexes.Count > 0)
                throw new Exception(string.Format("Cannot evaluate {0} : Parenthese at index {1} is not closed", String,
                    openIndexes.Pop()));
        }
    }
}