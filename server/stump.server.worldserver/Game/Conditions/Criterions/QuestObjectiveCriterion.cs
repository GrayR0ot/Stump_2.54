...
namespace Stump.Server.WorldServer.Game.Conditions.Criterions
{
    public class QuestObjectiveCriterion : Criterion
    {
        ...
        {
            return character.Quests.Where(x =>
                    x.CurrentStep.Objectives.Any(y => y.ObjectiveRecord.ObjectiveId == QuestObjectiveId && y.Finished))
                .Count() > 0;
        }
        ...
        public override void Build()
        {
            ...
            if (!int.TryParse(Literal, out questObjectiveId))
                throw new Exception(
                    string.Format("Cannot build QuestActiveCriterion, {0} is not a valid quest objective id", Literal));
