    ...
    public class HasManyItemsCriterion : Criterion
    {
        ...
        public override void Build()
        {
            ...
            if (!short.TryParse(condition[0], out item))
                throw new Exception($"Cannot build HasManyItemsCriterion, {Literal} is not a valid item id");
            if (!byte.TryParse(condition[1], out quantity))
                throw new Exception($"Cannot build HasManyItemsCriterion, {Literal} is not a valid quantity");
            Item = item;
            Quantity = quantity;
        }
