        ...
        public override void Build()
        {
            ...
            if (split.Length == 1)
            {
                ...
                if (!int.TryParse(Literal, out count))
                    throw new Exception(string.Format(
                        "Cannot build MapCharactersCriterion, {0} is not a valid characters count", Literal));
            ...
            else if (split.Length == 2)
            {
                ...
                if (!int.TryParse(split[0], out mapId))
                    throw new Exception(string.Format("Cannot build MapCharactersCriterion, {0} is not a valid map id",
                        split[0]));
                ...
                if (!int.TryParse(split[1], out count))
                    throw new Exception(string.Format(
                        "Cannot build MapCharactersCriterion, {0} is not a valid characters count", split[1]));
            ...
            {
                throw new Exception(string.Format(
                    "Cannot build MapCharactersCriterion, {0} is mal formatted : 'id,count' or 'count'", split[1]));
            }
        }
