...
namespace Stump.Server.WorldServer.Game.Breeds
{
    public class BreedManager : DataManager<BreedManager>
    {
        ...
        /// </summary>
        [Variable] public static readonly List<PlayableBreedEnum> AvailableBreeds = new List<PlayableBreedEnum>
        {
        ...
        public uint AvailableBreedsFlags
        {
            get
            {
                return (uint) AvailableBreeds.Aggregate(0,
                    (current, breedEnum) => current | (1 << ((int) breedEnum - 1)));
            }
        }
        ...
        public override void Initialize()
        {
            ...
            var index = 1;
            var dq = Database
                .Query<Breed, BreedItem, BreedSpell, Breed>(new BreedRelator().Map, BreedRelator.FetchQuery)
        ...
        public static void ChangeBreed(Character character, PlayableBreedEnum breed)
        {
            ...
            {
                foreach (var shortcut in character.Shortcuts.SpellsShortcuts
                    .Where(x => x.Value.SpellId == breedSpell.Spell).ToArray())
            ...
            foreach (var spell in character.Breed.Spells)
            {
                if (spell.ObtainLevel > character.Level)
                {
                    ...
                    if (character.Spells.HasSpell(spell.Spell, true)) character.Spells.UnLearnSpell(spell.Spell);
                }
                else if (spell.ObtainLevel <= character.Level && !character.Spells.HasSpell(spell.Spell, true))
                {
                    ...
                    character.Shortcuts.AddSpellShortcut(
                        character.Shortcuts.GetNextFreeSlot(ShortcutBarEnum.SPELL_SHORTCUT_BAR),
                        (short) spell.Spell);
                }
                ...
                if (spell != null)
                    if (spell.Spell != 0)
                    {
                        Console.WriteLine("SpellID: " + spell.Spell);
                        if (SpellManager.Instance.GetSpellLevels(spell.Spell) != null)
                        {
                            var count = SpellManager.Instance.GetSpellLevels(spell.Spell).Count();
                            foreach (var spelllevel in SpellManager.Instance.GetSpellLevels(spell.Spell)
                                .OrderByDescending(x => x.MinPlayerLevel))
                            {
