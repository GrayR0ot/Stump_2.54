...
namespace Stump.Server.WorldServer.Game.Dialogs.Merchants
{
    public class MerchantShopDialog : IShopDialog
    {
        ...
        public bool BuyItem(uint itemGuid, uint quantity)
        {
            ...
            Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 21, newItem.Stack,
                newItem.Template.Id);
        ...
        {
            return Character.Inventory.Kamas >= (ulong) (item.Price * amount) || !Merchant.CanBeSee(Character);
        }
    }
}