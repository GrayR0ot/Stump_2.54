...
namespace Stump.Server.WorldServer.Game.Exchanges.Paddock
{
    public class PaddockExchanger : Exchanger
    {
        ...
        {
            if (equip && Character.HasEquippedMount()) EquipToInventory(Character.EquippedMount.Id);
        ...
        public bool PaddockToEquip(int mountId)
        {
            ...
            {
                Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_ERROR, 227,
                    Mount.RequiredLevel);
                return false;
            }
        ...
        {
            if (Character.HasEquippedMount()) EquipToInventory(Character.EquippedMount.Id);
