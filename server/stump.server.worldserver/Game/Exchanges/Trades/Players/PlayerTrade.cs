    ...
    public class PlayerTrade : Trade<PlayerTrader, PlayerTrader>
    {
        ...
        protected override void Apply()
        {
            ...
            FirstTrader.Character.Inventory.SetKamas(
                FirstTrader.Character.Inventory.Kamas + (SecondTrader.Kamas - FirstTrader.Kamas));
            SecondTrader.Character.Inventory.SetKamas(
                SecondTrader.Character.Inventory.Kamas + (FirstTrader.Kamas - SecondTrader.Kamas));
        ...
        protected override void OnTraderItemMoved(Trader trader, TradeItem item, bool modified, int difference)
        {
            ...
            if (item.Stack == 0)
            {
                InventoryHandler.SendExchangeObjectRemovedMessage(FirstTrader.Character.Client, trader != FirstTrader,
                    item.Guid);
                InventoryHandler.SendExchangeObjectRemovedMessage(SecondTrader.Character.Client, trader != SecondTrader,
                    item.Guid);
            }
            else if (modified)
            {
                InventoryHandler.SendExchangeObjectModifiedMessage(FirstTrader.Character.Client, trader != FirstTrader,
                    item);
                InventoryHandler.SendExchangeObjectModifiedMessage(SecondTrader.Character.Client,
                    trader != SecondTrader, item);
            }
            else
            {
                InventoryHandler.SendExchangeObjectAddedMessage(FirstTrader.Character.Client, trader != FirstTrader,
                    item);
                InventoryHandler.SendExchangeObjectAddedMessage(SecondTrader.Character.Client, trader != SecondTrader,
                    item);
            }
        }
