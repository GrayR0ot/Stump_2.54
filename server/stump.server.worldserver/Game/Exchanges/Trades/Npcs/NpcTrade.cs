...
namespace Stump.Server.WorldServer.Game.Exchanges.Trades.Npcs
{
    ...
    {
        public NpcTrade(Character character, Npc npc, List<List<ItemList>> itemsToTake,
            List<List<ItemList>> itemsToReceive)
        {
        ...
        public List<List<ItemList>> ItemsToReceive { get; set; }

        public int Kamas { get; set; }

        public int RateItem { get; set; }

        public List<List<ItemList>> ItemsToTake { get; set; }
        ...
        private void NpcPutAllItem(List<ItemList> recipe, int quantity)
        {
            foreach (var item in recipe)
            {
                ...
                if (itm == null) continue; // UNE COUILLE
        ...
        private bool RecipeIsCorrect(List<ItemList> itemList, out int quantity)
        {
            ...
            foreach (var i in itemList)
            {
                ...
                double cf = item.Stack / i.Quantiy;

                if (cf < 1)
                    return false;

                var qt = (int) Math.Truncate(cf);
        ...
        protected override void OnTraderItemMoved(Trader trader, TradeItem item, bool modified, int difference)
        {
            ...
            if (SecondTrader.Items.Count > 0 && trader != SecondTrader) SecondTrader.Clear();
            ...
            {
                InventoryHandler.SendExchangeObjectModifiedMessage(FirstTrader.Character.Client, trader != FirstTrader,
                    item);
            }
            else
            {
            ...
            {
                var index = ItemsToTake.IndexOf(goodRecipe);

                foreach (var itm in ItemsToReceive[index])
                    SecondTrader.AddItem(itm.Item, (uint) (itm.Quantiy * quantity));
            }
        }
