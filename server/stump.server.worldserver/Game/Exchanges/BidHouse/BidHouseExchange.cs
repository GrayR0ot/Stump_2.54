...
namespace Stump.Server.WorldServer.Game.Exchanges.BidHouse
{
    public class BidHouseExchange : IExchange
    {
        ...
        public IEnumerable<int> Types { get; protected set; }

        public int MaxItemLevel { get; protected set; }

        public bool Buy { get; protected set; }

        public int CurrentViewedItem { get; protected set; }
        ...
        public SellerBuyerDescriptor GetBuyerDescriptor()
        {
            ...
            foreach (var type in Types) types_.Add((uint) type);
            var quants_ = new List<uint>();
            foreach (var quant in BidHouseManager.Quantities) quants_.Add((uint) quant);
            var boostedseller = Character.Level * 50;
            return new SellerBuyerDescriptor(quants_.ToArray(), types_.ToArray(), BidHouseManager.TaxPercent,
                BidHouseManager.TaxModificationPercent, (byte) MaxItemLevel, boostedseller, -1,
                (ushort) BidHouseManager.UnsoldDelay);
        }
        ...
        public void Open()
        {
            ...
            else
            {
                ...
                InventoryHandler.SendExchangeStartedBidSellerMessage(Character.Client, this,
                    items.Select(x => x.GetObjectItemToSellInBid()));
            }
