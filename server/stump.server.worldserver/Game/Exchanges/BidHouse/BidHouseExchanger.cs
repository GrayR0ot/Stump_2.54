...
namespace Stump.Server.WorldServer.Game.Exchanges.BidHouse
{
    public class BidHouseExchanger : Exchanger
    {
        ...
        public bool MovePricedItem(uint id, int quantity, long price)
        {
            ...
            if (BidHouseManager.Instance.GetBidHouseItems(Character.Account.Id, ((BidHouseExchange) Dialog).Types)
            ...
            var tax = (ulong) Math.Round(price * BidHouseManager.TaxPercent / 100);
        ...
        public bool ModifyItem(uint id, long price)
        {
            ...
            tax = (ulong) (diff < 0
                ? (int) Math.Round(Math.Abs(diff) * BidHouseManager.TaxPercent / 100)
                : (int) Math.Round(Math.Abs(price) * BidHouseManager.TaxModificationPercent / 100));
