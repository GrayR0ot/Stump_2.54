...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft
{
    public abstract class CraftDialog : BaseCraftDialog
    {
        ...
        public bool ChangeRecipe(CraftingActor actor, RecipeRecord recipe)
        {
            ...
            var valid = true;
            for (var i = 0; valid && i < recipe.Ingredients.Length; i++)
            {
        ...
        public bool Craft()
        {
            ...
            if (recipe.ResultLevel > Job.Level && Job.Id != (int) JobEnum.BASE)
            {
                InventoryHandler.SendExchangeCraftResultWithObjectIdMessage(Clients,
                    ExchangeCraftResultEnum.CRAFT_FAILED, recipe.ItemTemplate);
                return false;
            }
            ...
            if (xp > 0)
                Job.Experience += xp;
            if (Job.Id == 65) Job.Experience = lvlmax;
            ...
            if (!ItemManager.Instance.HasToBeGenerated(recipe.ItemTemplate))
            {
                ...
                InventoryHandler.SendExchangeCraftInformationObjectMessage(Crafter.Character.Map.Clients, createdItem,
                    Receiver.Character, ExchangeCraftResultEnum.CRAFT_SUCCESS);
            ...
            else
            {
                ...
                {
                    var createdItem =
                        Receiver.Character.Inventory.AddItem(recipe.ItemTemplate, keyPair.Key, keyPair.Value);
                    InventoryHandler.SendExchangeCraftResultWithObjectDescMessage(Clients,
                        ExchangeCraftResultEnum.CRAFT_SUCCESS, createdItem, keyPair.Value);
                    InventoryHandler.SendExchangeCraftInformationObjectMessage(Crafter.Character.Map.Clients,
                        createdItem, Receiver.Character, ExchangeCraftResultEnum.CRAFT_SUCCESS);
