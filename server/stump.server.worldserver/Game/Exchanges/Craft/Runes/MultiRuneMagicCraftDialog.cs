...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft.Runes
{
    ...
    {
        public MultiRuneMagicCraftDialog(Character crafter, Character customer, InteractiveObject interactive,
            Skill skill)
        ...
        public override void Open()
        {
            ...
            InventoryHandler.SendExchangeStartOkMulticraftCustomerMessage(Receiver.Character.Client,
                Skill.SkillTemplate, Job);

            ContextRoleplayHandler.SendJobExperienceOtherPlayerUpdateMessage(Receiver.Character.Client,
                Crafter.Character, Job);
        ...
        private void OnReady(Trader trader, bool isready)
        {
            ...
            if (Crafter.ReadyToApply && Receiver.ReadyToApply)
            {
                ...
                if (Receiver.Kamas > 0)
                {
                    Crafter.Character.Inventory.AddKamas(
                        (ulong) -(long) Receiver.Character.Inventory.SubKamas(Receiver.Kamas));
                    Receiver.SetKamas(0);
                }
        ...
        protected override void OnItemMoved(Trader trader, TradeItem item, bool modified, int difference)
        {
            if (trader == Crafter)
            {
                ...
                {
                    InventoryHandler.SendExchangeObjectRemovedMessage(Crafter.Character.Client, Crafter != trader,
                        item.Guid);
                    InventoryHandler.SendExchangeObjectRemovedMessage(Receiver.Character.Client, Receiver != trader,
                        item.Guid);
                }
                else
                {
                    InventoryHandler.SendExchangeObjectModifiedMessage(Crafter.Character.Client, Crafter != trader,
                        item);
            ...
            else
            {
                if (!modified && item.Stack > 0)
                {
                    InventoryHandler.SendExchangeObjectPutInBagMessage(Crafter.Character.Client, Crafter != trader,
                        item);
                    InventoryHandler.SendExchangeObjectPutInBagMessage(Receiver.Character.Client, Receiver != trader,
                        item);
                }
                else if (item.Stack <= 0)
                {
                    InventoryHandler.SendExchangeObjectRemovedFromBagMessage(Crafter.Character.Client,
                        Crafter != trader, item.Guid);
                    InventoryHandler.SendExchangeObjectRemovedFromBagMessage(Receiver.Character.Client,
                        Receiver != trader, item.Guid);
                }
                else
                {
                    InventoryHandler.SendExchangeObjectModifiedInBagMessage(Crafter.Character.Client, Crafter != trader,
                        item);
                    InventoryHandler.SendExchangeObjectModifiedInBagMessage(Receiver.Character.Client,
                        Receiver != trader, item);
                }
            }
        }
        ...
        {
            InventoryHandler.SendExchangeCraftResultMagicWithObjectDescMessage(Crafter.Character.Client, result,
                ItemToImprove.PlayerItem, ItemEffects, poolStatus);
            InventoryHandler.SendExchangeCraftResultMagicWithObjectDescMessage(Receiver.Character.Client, result,
                ItemToImprove.PlayerItem, ItemEffects, poolStatus);

            InventoryHandler.SendExchangeCraftInformationObjectMessage(Crafter.Character.Client,
                ItemToImprove.PlayerItem, ItemToImprove.Owner, (ExchangeCraftResultEnum) result);
            InventoryHandler.SendExchangeCraftInformationObjectMessage(Receiver.Character.Client,
                ItemToImprove.PlayerItem, ItemToImprove.Owner, (ExchangeCraftResultEnum) result);
