...
namespace Stump.Server.WorldServer.Game.Exchanges.Craft.Runes
{
    ...
    {
        [Variable] public static int DecraftHistoryLimit = 0;

        [Variable] public static double DecraftPercentile = 0.05; //base 0.5

        private readonly List<int> m_craftedItemsIds = new List<int>();

        private readonly Dictionary<ItemTypeEnum, LimitedOccurenceCounter<ItemTemplate>> m_decraftHistory =
            new Dictionary<ItemTypeEnum, LimitedOccurenceCounter<ItemTemplate>>();
        ...
        public override void Initialize()
        {
            ...
            foreach (var item in ItemManager.Instance.GetTemplates()
                .Where(x => x.TypeId == (int) ItemTypeEnum.RUNE_DE_FORGEMAGIE))
            ...
            m_historyRecords = Database.Fetch<DecraftItemRecord>(DecraftItemRelator.FetchQuery)
        ...
        {
            Console.WriteLine("Decraft " + item.Id + " item");
            LimitedOccurenceCounter<ItemTemplate> counter;
        ...
        {
            LimitedOccurenceCounter<ItemTemplate> counter;
            if (m_decraftHistory.TryGetValue(type, out counter)) return counter.GetPercentile(DecraftPercentile);
