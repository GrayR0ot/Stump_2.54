...
namespace Stump.Server.WorldServer.Game.Exchanges.TaxCollector
{
    public class TaxCollectorExchange : IExchange
    {
        ...
        public TaxCollectorNpc TaxCollector { get; }

        public Character Character { get; }

        private TimedTimerEntry Timer { get; set; }
        ...
        public ExchangeGuildTaxCollectorGetMessage GetExchangeGuildTaxCollector()
        {
            return new ExchangeGuildTaxCollectorGetMessage(
                ...
                (short) TaxCollector.Position.Map.Position.X, (short) TaxCollector.Position.Map.Position.Y,
                TaxCollector.Position.Map.Id,
                (ushort) TaxCollector.Position.Map.SubArea.Id, Character.Name, (uint) TaxCollector.Record.CallerId,
                TaxCollector.Record.CallerName, TaxCollector.GatheredExperience,
                (ushort) m_collector.RecoltedItems.Values.Sum(x => x.Template.RealWeight * x.Stack),
                m_collector.RecoltedItems.Values.Select(x => x.GetObjectItemGenericQuantity()).ToArray());
        }
        ...
        public void Close()
        {
            ...
            TaxCollectorHandler.SendGetExchangeGuildTaxCollectorMessage(TaxCollector.Guild.Clients,
                GetExchangeGuildTaxCollector());
