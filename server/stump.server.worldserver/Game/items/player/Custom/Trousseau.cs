...
namespace Stump.Server.WorldServer.Game.Items.Player.Custom
{
    ...
    public class Trousseau : BasePlayerItem
    {
        ...
        public override uint UseItem(int amount = 0, Cell targetCell = null, Character target = null)
        {
            ...
            Dictionary<Map, int> displayedDest = new Dictionary<Map, int>();
            foreach (var item in destinations)
            {
                if (Owner.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(item.Key.getKey())))
                {
                    displayedDest.Add(World.Instance.GetMap(item.Key.getDungeon()), item.Value);
                }
            }
            ...
            //DungsDialog s = new DungsDialog(Owner, destinations, true);
            s.Open();
