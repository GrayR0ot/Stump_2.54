...
namespace Stump.Server.WorldServer.Game.Quests
{
    public class QuestStep
    {
        public QuestStep(Quest quest, QuestStepTemplate template)
        {
            Quest = quest;
            Template = template;
            Objectives = template.Objectives.Where(x => !x.IsTriggeredByObjective)
                .Select(x => x.GenerateObjective(Quest.Owner)).ToList();
        ...
        public QuestStep(Quest quest, QuestStepTemplate template, List<QuestObjectiveStatus> status)
        {
            Quest = quest;
            Template = template;
            Objectives = status.Where(w => QuestManager.Instance.GetObjectiveTemplate(w.ObjectiveId).StepId == Id)
                .Select(x =>
                    QuestManager.Instance.GetObjectiveTemplate(x.ObjectiveId).GenerateObjective(Quest.Owner, x))
                .ToList();
            Rewards = template.Rewards.Select(x => new QuestReward(x)).ToArray();
        ...
        public QuestStepTemplate Template { get; set; }
        ...
        private void OnObjectiveCompleted(QuestObjective obj)
        {
            ...
            {
                var objective = QuestManager.Instance.GetObjectiveTemplate(obj.Template.TriggeredByObjectiveId)
                    .GenerateObjective(Quest.Owner);
        ...
        {
            foreach (var reward in Rewards) reward.GiveReward(Quest.Owner);
        }

        public QuestActiveInformations GetQuestActiveInformations()
        {
            return new QuestActiveDetailedInformations(Quest.Id, (ushort) Id,
                Objectives.Select(x => x.GetQuestObjectiveInformations()).ToArray());
        }
        ...
        private double CaclulateWonXp()
        {
            if (Quest.Owner.Level > Template.OptimalLevel)
            {
                ...
                var fixeOptimalLevelExperienceReward =
                    GetFixeExperienceReward(Template.OptimalLevel, Template.Duration, Template.XpRatio);
                var fixeLevelExperienceReward =
                    GetFixeExperienceReward((int) rewardLevel, Template.Duration, Template.XpRatio);
