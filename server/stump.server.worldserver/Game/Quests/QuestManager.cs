...
namespace Stump.Server.WorldServer.Game.Quests
{
    public class QuestManager : DataManager<QuestManager>
    {
        ...
        public override void Initialize()
        {
            ...
            m_questsRewards = Database.Query<QuestRewardTemplate>(QuestRewardRelator.FetchQuery)
                .ToDictionary(x => x.Id);
            m_questsObjectives = Database.Query<QuestObjectiveTemplate>(QuestObjectiveRelator.FetchQuery)
                .ToDictionary(x => x.Id);
            foreach (var quest in m_quests.Values) quest.Steps = quest.StepIds.Select(x => m_questsSteps[x]).ToArray();
            ...
            foreach (var objective in m_questsObjectives.Values)
                objective.ObjectivesToTrigger = m_questsObjectives.Values
                    .Where(x => x.TriggeredByObjectiveId == objective.Id).ToArray();
        }
        ...
        {
            return m_questsSteps.TryGetValue(id, out var template) ? template : null;
        }
        ...
        {
            return m_questsObjectives.TryGetValue(id, out var template) ? template : null;
        }

        public List<QuestObjectiveStatus> GetStatusByOwnerId(int ownerId)
        {
            return Database
                .Query<QuestObjectiveStatus>(string.Format(QuestRecordRelator.FetchObjectiveByOwner, ownerId)).ToList();
        }
    }
}