...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    ...
    {
        public TalkToObjective(Character character, QuestObjectiveTemplate template, bool finished, int npcId) : base(
            character, template, finished)
        {
            Npc = NpcManager.Instance.GetNpcTemplate(npcId);
        }

        public TalkToObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus finished,
            int npcId) : base(character, template, finished)
        {
            Npc = NpcManager.Instance.GetNpcTemplate(npcId);
        }
        ...
        private void CharacterInteractingWith(Character character, Npc npc, NpcActionTypeEnum actionType,
            NpcAction npcAction)
        {
