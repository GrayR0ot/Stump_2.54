...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    ...
    {
        public DiscoverMapObjective(Character character, QuestObjectiveTemplate template, bool finished, int mapId) :
            base(character, template, finished)
        {
            Map = World.Instance.GetMap(mapId);
        }

        public DiscoverMapObjective(Character character, QuestObjectiveTemplate template, QuestObjectiveStatus finished,
            int mapId) : base(character, template, finished)
        {
            Map = World.Instance.GetMap(mapId);
        }
