...
namespace Stump.Server.WorldServer.Game.Quests.Objectives
{
    public class DefeatMonstersObjective : QuestObjective
    {
        ...
        public DefeatMonstersObjective(Character character, QuestObjectiveTemplate template, bool finished,
        ...
        public DefeatMonstersObjective(Character character, QuestObjectiveTemplate template,
            QuestObjectiveStatus finished, int monsterId, int amount)
            : base(character, template, finished)
        {
            Monster = MonsterManager.Instance.GetTemplate(monsterId);
            Amount = amount;
        ...
        private void OnFightEnded(Character character, CharacterFighter fighter)
        {
            ...
            if (Character.Fighter != null && character.Fighter.Fight.Winners != character.Fighter.Team)
                return;
            var count = fighter.OpposedTeam.Fighters.OfType<MonsterFighter>()
                .Count(x => x.IsDead() && x.Monster.Template == Monster);
