...
namespace Stump.Server.WorldServer.Game.Shortcuts
{
    public class ShortcutBar
    {
        ...
        public IReadOnlyDictionary<int, SpellShortcut> SpellsShortcuts
        {
            get
            {
                if (m_CustomspellShortcuts.Count > 0)
        ...
        public IReadOnlyDictionary<int, ItemShortcut> ItemsShortcuts =>
            new ReadOnlyDictionary<int, ItemShortcut>(m_itemShortcuts);

        public IReadOnlyDictionary<int, PresetShortcut> PresetShortcuts =>
            new ReadOnlyDictionary<int, PresetShortcut>(m_presetShortcuts);

        internal void Load()
        {
            var database = WorldServer.Instance.DBAccessor.Database;

            m_spellShortcuts = database.Query<SpellShortcut>(string.Format(SpellShortcutRelator.FetchByOwner, Owner.Id))
                .DistinctBy(x => x.Slot).ToDictionary(x => x.Slot);
            m_itemShortcuts = database.Query<ItemShortcut>(string.Format(ItemShortcutRelator.FetchByOwner, Owner.Id))
                .DistinctBy(x => x.Slot).ToDictionary(x => x.Slot);
            m_presetShortcuts = database
                .Query<PresetShortcut>(string.Format(PresetShortcutRelator.FetchByOwner, Owner.Id))
                .DistinctBy(x => x.Slot).ToDictionary(x => x.Slot);
        }
        ...
        public void AddSpellShortcut(int slot, short spellId, bool Custom = false)
        {
            ...
            if (m_CustomspellShortcuts.Count < 1)
                ShortcutHandler.SendShortcutBarRefreshMessage(Owner.Client, ShortcutBarEnum.SPELL_SHORTCUT_BAR,
                    shortcut);
        }

        public void SwapSpellShortcuts(short previousspellId, short newspellId)
        {
            var previouslist = GetShortcuts(ShortcutBarEnum.SPELL_SHORTCUT_BAR)
                .Where(x => (x as SpellShortcut).SpellId == previousspellId);
        ...
        public SpellShortcut GetSpellShortcut(int slot)
        {
            ...
            if (m_CustomspellShortcuts.Count > 0)
                return m_CustomspellShortcuts.TryGetValue(slot, out shortcut) ? shortcut : null;
        ...
        public void Save()
        {
            lock (m_locker)
            {
                ...
                foreach (var shortcut in m_itemShortcuts
                    .Where(shortcut => shortcut.Value.IsDirty || shortcut.Value.IsNew)
                    .ToArray()) database.Save(shortcut.Value);

                foreach (var shortcut in m_spellShortcuts
                    .Where(shortcut => shortcut.Value.IsDirty || shortcut.Value.IsNew)
                    .ToArray()) database.Save(shortcut.Value);

                foreach (var shortcut in m_presetShortcuts
                    .Where(shortcut => shortcut.Value.IsDirty || shortcut.Value.IsNew)
                    .ToArray()) database.Save(shortcut.Value);
