...
namespace Stump.Server.WorldServer.Game.Accounts.Startup
{
    public class StartupActionItem
    {
        ...
        public StartupActionItemRecord Record => m_record;

        public ItemTemplate ItemTemplate
        {
            get => m_itemTemplate ?? (m_itemTemplate = ItemManager.Instance.TryGetTemplate(m_record.ItemTemplate));
            set
            {
        ...
        {
            return new ObjectItemInformationWithQuantity((ushort) ItemTemplate.Id,
                ItemTemplate.Effects.Select(entry => entry.GetObjectEffect()).ToArray(), Amount);
        }
    }
}