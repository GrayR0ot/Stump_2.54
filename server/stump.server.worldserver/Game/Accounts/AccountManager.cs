...
namespace Stump.Server.WorldServer.Game.Accounts
{
    ...
    {
        [Variable(true)] public static readonly int AccountBlockMaxDelay = 20; // in seconds

        public static UserGroup DefaultUserGroup = new UserGroup(new UserGroupData
            {Id = 0, IsGameMaster = false, Name = "Default", Role = RoleEnum.Player});

        private readonly ConcurrentDictionary<int, Tuple<Character, DateTime>> m_blockedAccount =
            new ConcurrentDictionary<int, Tuple<Character, DateTime>>();
        ...
        {
            if (!IPCAccessor.Instance.IsConnected) throw new Exception("IPC not connected");
