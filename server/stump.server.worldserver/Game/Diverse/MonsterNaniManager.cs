...
namespace Stump.Server.WorldServer.Game.Diverse
{
    public class MonsterNani
    {
        ...
        public MonsterNaniRecord Record { get; set; }
    ...
    public class MonsterNaniManager : DataManager<MonsterNaniManager>
    {
        ...
        private readonly SelfRunningTaskPool m_NaniTaskPool =
            new SelfRunningTaskPool(NaniUpdateInterval, "MonstersNani");
        ...
        {
            monsterNaniRecords = Database.Query<MonsterNaniRecord>(MonsterNaniRelator.FetchQuery)
                .Select(x => new MonsterNani(x)).ToList();
        ...
        public void CheckMonstersSpawns(bool Silent = false)
        {
            foreach (var nani in monsterNaniRecords)
            {
                ...
                if (nani.Spawned)
                {
                    if (!Silent)
                        World.Instance.SendAnnounce(
                            "'<b>" + monster.Template.Name + "</b>' est disponible dans la zone : <b>" + nani.ZoneName +
                            "</b>.", Color.OrangeRed);
                    continue;
                }
                ...
                if (!Silent)
                    World.Instance.SendAnnounce(
                        "'<b>" + monster.Template.Name + "</b>' est disponible dans la zone : <b>" +
                        map.SubArea.Record.Name + "</b>.", Color.OrangeRed);
            }
        }
        ...
        {
            var r = new Random();
            var pickedareaIndex = r.Next(0, areas.Count() - 1);
            ...
            var pickedMap = r.Next(0, AreaMaps.Count() - 1);
        ...
        {
            var mn = monsterNaniRecords.FirstOrDefault(x =>
                x.Record.MonsterGradeId == Nani.Id || x.Record.MonsterGradeId == Nani.GradeId);
            if (mn == null) return;
            mn.Spawned = false;
        }
    }
