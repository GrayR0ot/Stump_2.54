...
namespace Stump.Server.WorldServer.Game.Spells
{
    public class SpellInventory : IEnumerable<CharacterSpell>
    {
        ...
        internal void LoadSpells()
        {
            ...
            //where variant.VariantLevel <= Owner.Level
                       ...
                       learnableSpell.Spell))))
            {
                ...
                {
                    if (spellRecord.SpellId == 0)
                        spellRecord.Active = true;
            ...
            //foreach (var spellRecord in spellsToLearnVariant.Select (learnableSpell => SpellManager.Instance.CreateSpellRecord (Owner.Record,
            //        SpellManager.Instance.GetSpellTemplate (
            //            learnableSpell.VariantId)))) {
            //    if (!m_spells.ContainsKey (spellRecord.SpellId)) {
            //        m_spells.Add (spellRecord.SpellId, new CharacterSpell (spellRecord));
            //        database.Insert (spellRecord);
            //    }
        ...
        public IEnumerable<CharacterSpell> GetSpells(bool WithoutCustom = false)
        {
            ...
            return m_spells.Values.Where(x => x.Record.Active == true || x.Record.SpellId == 0);
        }
