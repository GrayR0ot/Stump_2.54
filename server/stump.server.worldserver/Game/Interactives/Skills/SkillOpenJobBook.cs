...
{
    [Discriminator("JobBook", typeof(Skill), typeof(int), typeof(InteractiveCustomSkillRecord),
    ...
    public class SkillOpenJobBook : CustomSkill
    {
        ...
        {
            get => m_jobsCSV ?? (m_jobsCSV = Record.GetParameter<string>(0));
            set
            {
        ...
        public JobTemplate[] Jobs
        {
            get
            {
                return m_jobs ?? (m_jobs = JobsCSV.FromCSV<int>(",").Select(x => JobManager.Instance.GetJobTemplate(x))
                    .ToArray());
            }
            set
            {
