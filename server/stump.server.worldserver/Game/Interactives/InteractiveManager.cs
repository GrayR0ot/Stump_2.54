...
namespace Stump.Server.WorldServer.Game.Interactives
{
    public class InteractiveManager : DataManager<InteractiveManager>
    {
        ...
        {
            m_interactivesTemplates = Database
                .Query<InteractiveTemplate, InteractiveSkillTemplate, InteractiveTemplateSkills,
                        InteractiveCustomSkillRecord, InteractiveTemplate>
                    (new InteractiveTemplateRelator().Map, InteractiveTemplateRelator.FetchQuery)
                .ToDictionary(entry => entry.Id);

            m_interactivesSpawns = Database
                .Query<InteractiveSpawn, InteractiveSpawnSkills, InteractiveCustomSkillRecord, InteractiveSpawn>
                    (new InteractiveSpawnRelator().Map, InteractiveSpawnRelator.FetchQuery)
                .ToDictionary(entry => entry.Id);
            m_skillsTemplates = m_interactivesTemplates.SelectMany(x => x.Value.TemplateSkills).DistinctBy(x => x.Id)
                .ToDictionary(entry => entry.Id);
            m_interactivesCustomSkills =
                Database.Query<InteractiveCustomSkillRecord>(InteractiveSkillRelator.FetchQuery)
                    .ToDictionary(entry => entry.Id);
        ...
        private void GenerateInteractivePlot()
        {
            foreach (var trigger in CellTriggerManager.Instance.GetCellTriggers())
            {
                ...
                var interactive = m_interactivesSpawns.Values.FirstOrDefault(x => x.MapId == trigger.MapId &&
                    x.CellId == trigger.CellId
        ...
        public void AddInteractiveSpawn(InteractiveSpawn spawn, InteractiveCustomSkillRecord skill,
            InteractiveSpawnSkills spawnSkill)
        {
        ...
        {
            return m_defaultSkillTemplate ?? (m_defaultSkillTemplate = GetSkillTemplate(DEFAULT_SKILL_TEMPLATE));
        }
    }
}