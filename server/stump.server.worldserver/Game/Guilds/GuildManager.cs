...
namespace Stump.Server.WorldServer.Game.Guilds
{
    public class GuildManager : DataManager<GuildManager>, ISaveable
    {
        ...
        private ObjectValidator<GuildInformations[]> m_cachedGuilds;
        private ObjectValidator<GuildVersatileInformations[]> m_cachedGuildsVersatile;
        ...
        public void Save()
        {
            lock (m_lock)
            {
                foreach (var guild in m_guilds.Values.Where(guild => guild.IsDirty)) guild.Save(Database);
        ...
        public override void Initialize()
        {
            ...
            m_guildsMembers = Database.Fetch<GuildMemberRecord, CharacterRecord, GuildMemberRecord>(
                new GuildMemberRelator().Map,
            ...
            m_cachedGuildsVersatile =
                new ObjectValidator<GuildVersatileInformations[]>(BuildCachedGuildsVersatile, TimeSpan.FromMinutes(15));
        ...
        {
            return m_guilds.Select(x => x.Value).ToList();
        }
        ...
        public Guild TryGetGuild(string name)
        {
            ...
            {
                return m_guilds
                    .FirstOrDefault(x => string.Equals(x.Value.Name, name, StringComparison.OrdinalIgnoreCase)).Value;
            }
        }
        ...
        {
            var guildalogemme =
                character.Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(ItemIdEnum.GUILDALOGEMME_1575));
            ...
            if (!Regex.IsMatch(name, "^\\b[A-Z][A-Za-zéèà\\s-']{4,30}\\b", RegexOptions.Compiled) ||
                Regex.IsMatch(name, "^\\s\\s$"))
        ...
        private GuildInformations[] BuildCachedGuilds()
        {
            return m_guilds.Values.Select(x => x.GetGuildInformations()).ToArray();
        }

        private GuildVersatileInformations[] BuildCachedGuildsVersatile()
        {
            return m_guilds.Values.Select(x => x.GetGuildVersatileInformations()).ToArray();
        }

        public GuildInformations[] GetCachedGuilds()
        {
            return m_cachedGuilds;
        }

        public GuildVersatileInformations[] GetCachedGuildsVersatile()
        {
            return m_cachedGuildsVersatile;
        }
    }
}