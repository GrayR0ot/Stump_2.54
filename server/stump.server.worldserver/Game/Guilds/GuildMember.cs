...
namespace Stump.Server.WorldServer.Game.Guilds
{
    public class GuildMember
    {
        ...
        public GuildMemberRecord Record { get; }
        ...
        public short RankId
        {
            get => Record.RankId >= 0 && Record.RankId <= 35 ? Record.RankId : (short) 0;
            set
            {
                Record.RankId = value;
                IsDirty = true;
        ...
        public CharacterMinimalInformations GetCharacterMinimalInformations()
        {
            return new CharacterMinimalInformations((ulong) Id, Name,
                ExperienceManager.Instance.GetCharacterLevel(Experience));
        }

        public CharacterMinimalGuildPublicInformations GetCharacterMinimalGuildPublicInformations()
        {
            return new CharacterMinimalGuildPublicInformations((ulong) Id, Name,
                ExperienceManager.Instance.GetCharacterLevel(Experience), (uint) RankId);
        }

        public NetworkGuildMember GetNetworkGuildMember()
        {
            var hvb = HavenBagManager.Instance.GetHavenBagByOwner(Character);
            if (hvb == null && IsConnected)
                return new NetworkGuildMember((ulong) Id, Character.Name, Character.Level,
                    Character.Sex == SexTypeEnum.SEX_FEMALE, false, (sbyte) Character.Breed.Id, (ushort) RankId,
                    (ulong) GivenExperience, (sbyte) GivenPercent, (uint) Rights,
                    Character.IsInFight() ? (sbyte) 2 : (sbyte) 1,
                    (sbyte) Character.AlignmentSide, (ushort) DateTime.Now.Hour, (ushort) Character.SmileyMoodId,
                    Record.AccountId, 0, Character.Status);
            if (IsConnected)
                return new NetworkGuildMember((ulong) Id, Character.Name, Character.Level,
                    Character.Sex == SexTypeEnum.SEX_FEMALE, hvb.GuildAllowed, (sbyte) Character.Breed.Id,
                    (ushort) RankId,
                    (ulong) GivenExperience, (sbyte) GivenPercent, (uint) Rights,
                    Character.IsInFight() ? (sbyte) 2 : (sbyte) 1,
                    (sbyte) Character.AlignmentSide, (ushort) DateTime.Now.Hour, (ushort) Character.SmileyMoodId,
            ...
            return new NetworkGuildMember((ulong) Id, Name,
                ...
                (sbyte) AlignementSide,
                LastConnection != null ? (ushort) (DateTime.Now - LastConnection.Value).TotalHours : (ushort) 0, 0,
        ...
        {
            return Rights.HasFlag(GuildRightsBitEnum.GUILD_RIGHT_BOSS) ||
                   Rights.HasFlag(GuildRightsBitEnum.GUILD_RIGHT_MANAGE_RIGHTS) &&
                   right != GuildRightsBitEnum.GUILD_RIGHT_BOSS || Rights.HasFlag(right);
        }
