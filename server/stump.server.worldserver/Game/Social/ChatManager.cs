...
namespace Stump.Server.WorldServer.Game.Social
{
    public class ChatManager : DataManager<ChatManager>, ISaveable
    {
        ...
        /// </summary>
        [Variable] public static readonly string CommandPrefix = ".";

        /// <summary>
        ///     Minimal role level to access the admin chat
        /// </summary>
        [Variable] public static readonly RoleEnum AdministratorChatMinAccess = RoleEnum.GameMaster;

        /// <summary>
        ///     In milliseconds
        /// </summary>
        [Variable] public static int AntiFloodTimeBetweenTwoMessages = 500;

        /// <summary>
        ///     In seconds
        /// </summary>
        [Variable] public static int AntiFloodTimeBetweenTwoGlobalMessages = 30;

        /// <summary>
        ///     Amount of messages allowed in a given time
        /// </summary>
        [Variable] public static int AntiFloodAllowedMessages = 8;

        /// <summary>
        ///     Time in seconds
        /// </summary>
        [Variable] public static int AntiFloodAllowedMessagesResetTime = 10;

        /// <summary>
        ///     Time in seconds
        /// </summary>
        [Variable] public static int AntiFloodMuteTime = 5;
        ...
        public void HandleChat(WorldClient client, ChatActivableChannelsEnum channel, string message,
            IEnumerable<ObjectItem> objectItems = null)
        {
            if (!ChatHandlers.ContainsKey(channel))
                return;

            if (message.StartsWith(CommandPrefix) &&
                (message.Length < CommandPrefix.Length * 2 ||
                 message.Substring(CommandPrefix.Length, CommandPrefix.Length) !=
                 CommandPrefix)) // ignore processing command whenever there is the preffix twice
            {
                ...
                if (client.Character.Account.UserGroupId > (int) RoleEnum.Player)
                    ...
                    {
                        var Buffer = "";
                        var CheminFichier = @".\logsmoderators\logs.txt";

                        var Fichier = new FileInfo(CheminFichier);

                        if (Fichier.Exists) // on verifie ke le fichier existe
                        {
                            var Lecture = new StreamReader(CheminFichier, Encoding.Default); // on ouvre le fichier
                        ...
                        {
                            var Ecriture =
                                new StreamWriter(CheminFichier, false,
                                    Encoding.Default); // le boolean à false permet d'écraser le fichier existant
                            Ecriture.Write("" + client.Character.Name + " |+| " + message + " |+| " + DateTime.Now +
                        ...
                        {
                            var Ecriture =
                                new StreamWriter(CheminFichier, true,
                                    Encoding
                                        .Default); // le boolean à false permet d'ajouter un ligne sans écraser le fichier
                            Ecriture.Write("" + client.Character.Name + " |+| " + message + " |+| " + DateTime.Now +
                                           " |+| " + client.IP +
                ...
                WorldServer.Instance.CommandManager.HandleCommand(new TriggerChat(
                    new StringStream(UnescapeChatCommand(message)),
                    client.Character));
            }
            else
            {
                ...
                if (client.Character.Account.UserGroupId >= (int) RoleEnum.Player)
                    ...
                    {
                        var Buffer = "";
                        var CheminFichier = @".\logsplayers\logs.txt";
                ...
                {
                    if (client.Character.ChatHistory.RegisterAndCheckFlood(
                        new ChatEntry(message, channel, DateTime.Now)))
        ...
        private static void SendChatServerMessage(IPacketReceiver client, Character sender,
            ChatActivableChannelsEnum channel, string message)
        {
        ...
        private static void SendChatServerMessage(IPacketReceiver client, INamedActor sender,
            ChatActivableChannelsEnum channel, string message)
        {
            ChatHandler.SendChatServerMessage(client, sender, channel, message);
        }

        private static void SendChatServerWithObjectMessage(IPacketReceiver client, INamedActor sender,
            ChatActivableChannelsEnum channel, string message, IEnumerable<ObjectItem> objectItems)
        {
        ...
        public void SayGlobal(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            var clients = client.Character.Map.GetAllCharacters()
                .Where(x => HavenBagManager.Instance.CanBeSeenInHavenBag(x, client.Character)).Select(v => v.Client);
            ...
            if (objectItems != null)
                ...
                clients.ForEach(x =>
                    SendChatServerMessage(x, client.Character, ChatActivableChannelsEnum.CHANNEL_GLOBAL, msg));
        }

        public void SayGlobal(NamedActor actor, string msg)
        {
            SendChatServerMessage(actor.CharacterContainer.Clients, actor, ChatActivableChannelsEnum.CHANNEL_GLOBAL,
                msg);
        }
        ...
        public void SayAdministrators(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            World.Instance.ForEachCharacter(entry =>
            {
                ...
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_ADMIN, msg, objectItems);
                else
        ...
        public void SayParty(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            client.Character.Party.ForEach(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_PARTY, msg, objectItems);
                else
        ...
        public void SayArena(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            client.Character.ArenaParty.ForEach(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_ARENA, msg, objectItems);
                else
        ...
        public void SayGuild(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            if (objectItems != null)
                SendChatServerWithObjectMessage(client.Character.Guild.Clients, client.Character,
                    ChatActivableChannelsEnum.CHANNEL_GUILD, msg, objectItems);
            else
                SendChatServerMessage(client.Character.Guild.Clients, client.Character,
                    ChatActivableChannelsEnum.CHANNEL_GUILD, msg);
        }
        ...
        public void SayTeam(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            foreach (var fighter in client.Character.Fighter.Team.GetAllFighters<CharacterFighter>())
                if (objectItems != null)
                    SendChatServerWithObjectMessage(fighter.Character.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_TEAM, msg, objectItems);
                else
                    SendChatServerMessage(fighter.Character.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_TEAM, msg);
        }
        ...
        public void SaySales(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            World.Instance.ForEachCharacter(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_SALES, msg, objectItems);
                else
        ...
        public void SayMonde(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            World.Instance.ForEachCharacter(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_MONDE, msg, objectItems);
                else
        ...
        public void SayVip(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            ...
            World.Instance.ForEachCharacter(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character,
                        ChatActivableChannelsEnum.CHANNEL_VIP, msg, objectItems);
                else
