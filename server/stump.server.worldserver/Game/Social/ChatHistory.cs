...
namespace Stump.Server.WorldServer.Game.Social
{
    public class ChatHistory
    {
        ...
        public ReadOnlyCollection<ChatEntry> Entries => m_entries.AsReadOnly();

        public bool RegisterAndCheckFlood(ChatEntry entry)
        {
            m_entries.Insert(0, entry);

            while (m_entries.Count >= MaxChatEntries) m_entries.Remove(m_entries.Last());
            ...
            if (m_entries.Count > 1 && ChatManager.IsGlobalChannel(entry.Channel))
            {
                ...
                if (entryIndex >= 0 &&
                    (entry.Date - m_entries[entryIndex].Date).TotalSeconds <
                    ChatManager.AntiFloodTimeBetweenTwoGlobalMessages)
                {
                    ...
                    Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 115,
                        ChatManager.AntiFloodTimeBetweenTwoGlobalMessages -
                        (int) (entry.Date - m_entries[entryIndex].Date).TotalSeconds);
