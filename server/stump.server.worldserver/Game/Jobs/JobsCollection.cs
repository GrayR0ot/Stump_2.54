...
namespace Stump.Server.WorldServer.Game.Jobs
{
    public class JobsCollection : IEnumerable<Job>
    {
        ...
        {
            return m_jobs.Values.GetEnumerator();
        }
        ...
        {
            m_jobs = JobManager.Instance.GetCharacterJobs(Owner.Id)
                .ToDictionary(x => x.TemplateId, x => new Job(Owner, x));

            // in order to free space we avoid storing job record with no experience in databse
            foreach (var job in JobManager.Instance.EnumerateJobTemplates().Where(x => !m_jobs.ContainsKey(x.Id))
                .ToArray())
