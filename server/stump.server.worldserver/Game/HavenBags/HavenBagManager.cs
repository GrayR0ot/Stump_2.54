    ...
    public class HavenBagManager : DataManager<HavenBagManager>, ISaveable
    {
        ...
        public void Save()
        {
            lock (m_lock)
            {
                ...
                {
                    foreach (var record in m_newhavenBags) Database.Insert(record);
                    m_newhavenBags.Clear();
                }

                if (m_deletehavenBags.Count > 0)
                {
                    foreach (var record in m_deletehavenBags) Database.Delete(record);
                ...
                {
                    foreach (var record in m_newhavenBagFurnitures) Database.Insert(record);
                    m_newhavenBagFurnitures.Clear();
                }

                if (m_deletehavenBagFurnitures.Count > 0)
                {
                    foreach (var record in m_deletehavenBagFurnitures) Database.Delete(record);
        ...
        public void HavenBagFurnituresUpdate(WorldClient client, HavenBagFurnituresRequestMessage message)
        {
            ...
            if (message.FunitureIds.Count() < 1) return;
            ...
            var havenbagfurnituresx = m_havenBagFurnitures.Where(x => x.HavenBagId == havenbag.HavenBagId)
                .Select(v => v.HavenBagFurnitureInformation).ToArray();
            foreach (var cli in World.Instance.GetCharacters()
                .Where(x => x.Record.IsInHavenBag && x.Record.HavenBagOwnerId == client.Character.Id)
                .Select(c => c.Client))
        ...
        {
            if (character.Record.HavenBagsCSV == null || character.Record.HavenBagsCSV.Length < 1 ||
                !character.Record.HavenBags.Contains(ThemeId))
            {
                var newcsv = character.Record.HavenBagsCSV + ";" + ThemeId;
        ...
        public void UpdateHavenBagTheme(WorldClient client, ChangeThemeRequestMessage message)
        {
            ...
            World.Instance.GetCharacters()
                .Where(x => x.Record.IsInHavenBag && x.Record.HavenBagOwnerId == client.Character.Id &&
                            x != client.Character).ToList().ForEach(c => ExitHavenBag(c.Client));
        ...
        public void UpdateHavenBagPermissions(WorldClient client, HavenBagPermissionsUpdateRequestMessage message)
        {
            ...
            if (havenbag != null)
                ...
                {
                    case 0:
                        havenbag.GuildAllowed = false;
                        havenbag.FriendsAllowed = false;
                        break;
                    case 1:
                        havenbag.GuildAllowed = false;
                        havenbag.FriendsAllowed = true;
                        break;
                    case 2:
                        havenbag.GuildAllowed = true;
                        havenbag.FriendsAllowed = false;
                        break;
                    case 3:
                        havenbag.GuildAllowed = true;
        ...
        {
            var n0 = !AllowedFriends && !AllowedGuild;
            var n1 = AllowedFriends && !AllowedGuild;
            var n2 = !AllowedFriends && AllowedGuild;
            var n3 = AllowedFriends && AllowedGuild;
        ...
        {
            if (!client.Character.Record.IsInHavenBag &&
                !GetHavenBagThemes().Any(x => x.MapId == client.Character.Map.Id)) return;
            client.Character.Record.IsInHavenBag = false;
            client.Character.Teleport(new ObjectPosition(
                World.Instance.GetMap(client.Character.Record.MapBeforeHavenBagId),
                World.Instance.GetMap(client.Character.Record.MapBeforeHavenBagId)
                    .GetCell(client.Character.Record.CellBeforeHavenBagId), DirectionsEnum.DIRECTION_SOUTH_EAST));
        }
        ...
        public bool EnterHavenBag(WorldClient client, int HavenBagOwnerId, bool Force = false)
        {
            if (client.Character.Id == HavenBagOwnerId)
            {
                ...
                var havenBagMapId = m_havenBagThemes.FirstOrDefault(x => x.Id == havenbag.ThemeId).MapId;
                client.Character.Teleport(new ObjectPosition(World.Instance.GetMap(havenBagMapId),
                    (short) m_havenBagThemes.FirstOrDefault(x => x.Id == havenbag.ThemeId).CellId,
            ...
            else
            {
                ...
                var Friend = client.Character.FriendsBook.IsFriend(owner.Account.Id) && havenbag.FriendsAllowed;
                var Guild = client.Character.Guild != null && owner.Guild != null &&
                            client.Character.Guild == owner.Guild && havenbag.GuildAllowed;
        ...
        {
            if (character == null) return null;
            return World.Instance.GetCharacter(m_havenBags
                .FirstOrDefault(x => x.OwnerId == character.Record.HavenBagOwnerId).OwnerId);
        }
