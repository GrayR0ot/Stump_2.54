...
namespace Stump.Server.WorldServer.Game.Idols
{
    ...
    {
        [Variable] private const int MaxActiveIdols = 6;

        public IdolInventory(Character owner)
        {
            Owner = owner;
            ActiveIdols = owner.Record.Idols.Select(x => IdolManager.Instance.CreatePlayerIdol(Owner, x))
        ...
        public bool IsPartyIdols => Party != null;

        public Character Owner { get; private set; }

        private List<PlayerIdol> ActiveIdols { get; }
        ...
        public bool HasIdol(int templateId)
        {
            return ActiveIdols.Any(x => x.Template.Id == templateId);
        }
        ...
        public void Add(short idolId)
        {
            ...
            owner.SendServerMessage("L'idole ajoutée a un score réel de = " + idol.Score);
            IdolHandler.SendIdolSelectedMessage(IsPartyIdols ? Party.Clients : Owner.Client, true, IsPartyIdols,
                (short) idol.Id);

            if (!owner.IsInFight() || owner.Fight.State != FightState.Placement)
                return;

            IdolHandler.SendIdolFightPreparationUpdate(Owner.Fight.Clients,
                ActiveIdols.Select(x => x.GetNetworkIdol()));
        }
        ...
        public bool Remove(PlayerIdol idol)
        {
            ...
            IdolHandler.SendIdolSelectedMessage(IsPartyIdols ? Party.Clients : Owner.Client, false, IsPartyIdols,
                (short) idol.Id);
        ...
        {
            var idolItems = Party.Members
                .SelectMany(x => x.Inventory.GetItems(y => y.Template.TypeId == (uint) ItemTypeEnum.IDOLE)).ToArray();
            ...
            foreach (var idolItem in idolItems)
            {
                ...
                partyIdol = new PartyIdol((ushort) template.Id, (ushort) template.ExperienceBonus,
                    (ushort) template.DropBonus, ownerIds.Select(x => (ulong) x).ToArray());
        ...
        private bool CanUseIdol(PlayerIdol idol, FightPvM fight)
        {
            if (!idol.Template.IdolSpellId.HasValue)
                return false;
            if (ActiveIdols.Count(x => x.Id == idol.Id) > 1)
                return false;
            if (fight.MonsterTeam.Fighters.OfType<MonsterFighter>()
                .Any(x => idol.Template.IncompatibleMonsters.Contains(x.Monster.Template.Id)))
                return false;
            if (fight.MonsterTeam.Fighters.OfType<MonsterFighter>().Any(x => x.Monster.Template.AllIdolsDisabled))
                return false;
            if (idol.Template.GroupOnly &&
                (fight.PlayerTeam.Fighters.Count < 4 || fight.MonsterTeam.Fighters.Count < 4))
                return false;
            if (!idol.Owner.Inventory.HasItem(idol.Template.IdolItem))
                return false;
            if (!idol.Owner.IsInFight() || idol.Owner.Fight.Id != fight.Id)
                return false;
