...
namespace Stump.Server.WorldServer.Game.Dungeon
{
    public class DungeonItemsManager : DataManager<DungeonItemsManager>
    {
        ...
        {
            m_dungeonItemsTemplate = Database.Query<DungeonItemsRecord>(DungeonItemsRelator.FetchQuery)
                .ToDictionary(entry => entry.ItemId);
        }

        public List<ItemTemplate> GetItems()
        {
            var m_dungeonItemsTemplates = new List<ItemTemplate>();
