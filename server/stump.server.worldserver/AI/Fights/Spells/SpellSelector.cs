...
namespace Stump.Server.WorldServer.AI.Fights.Spells
{
    public class SpellSelector
    {
        ...
        public List<SpellCastImpact> Possibilities { get; private set; }

        public Dictionary<SpellCategory, int> Priorities { get; set; }
        ...
        public bool CanReach(TargetCell target, Spell spell, out Cell castCell)
        {
            ...
            if (-diff <= Fighter.MP)
            {
                castCell = m_environment.GetCellToCastSpell(target, spell, spell.CurrentSpellLevel.CastTestLos,
                    nearFirst);
        ...
        {
            return cells.SelectMany(x =>
                spell.CurrentSpellLevel.Effects.SelectMany(y =>
                    ExpandZone(x, y.ZoneShape, (int) y.ZoneMinSize, (int) y.ZoneSize))).Distinct().ToArray();
        }
        ...
        private IEnumerable<TargetCell> ExpandZone(Cell center, SpellShapeEnum shape, int minRange, int maxRange)
        {
            switch (shape)
            {
                case SpellShapeEnum.X:
                    return new CrossSet(new MapPoint(center), maxRange, minRange).EnumerateValidPoints()
                        .Select(x => new TargetCell(Fighter.Map.Cells[x.CellId]));
                case SpellShapeEnum.C:
                    return new LozengeSet(new MapPoint(center), maxRange, minRange).EnumerateValidPoints()
                        .Select(x => new TargetCell(Fighter.Map.Cells[x.CellId]));
                case SpellShapeEnum.L:
                    return new LineSet(
                            new MapPoint(center).GetCellInDirection(DirectionsEnum.DIRECTION_NORTH_EAST, minRange),
                            maxRange, DirectionsEnum.DIRECTION_NORTH_EAST).EnumerateValidPoints()
                        .Select(x =>
                            new TargetCell(Fighter.Map.Cells[x.CellId], DirectionFlagEnum.DIRECTION_SOUTH_WEST))
                        .Union(new LineSet(
                            new MapPoint(center).GetCellInDirection(DirectionsEnum.DIRECTION_SOUTH_EAST, minRange),
                            maxRange, DirectionsEnum.DIRECTION_SOUTH_EAST).EnumerateValidPoints().Select(x =>
                            new TargetCell(Fighter.Map.Cells[x.CellId], DirectionFlagEnum.DIRECTION_NORTH_WEST))).Union(
                            new LineSet(
                                new MapPoint(center).GetCellInDirection(DirectionsEnum.DIRECTION_SOUTH_WEST, minRange),
                                maxRange, DirectionsEnum.DIRECTION_SOUTH_WEST).EnumerateValidPoints().Select(x =>
                                new TargetCell(Fighter.Map.Cells[x.CellId], DirectionFlagEnum.DIRECTION_NORTH_EAST)))
                        .Union(
                            new LineSet(
                                new MapPoint(center).GetCellInDirection(DirectionsEnum.DIRECTION_NORTH_WEST, minRange),
                                maxRange, DirectionsEnum.DIRECTION_NORTH_WEST).EnumerateValidPoints().Select(x =>
                                new TargetCell(Fighter.Map.Cells[x.CellId], DirectionFlagEnum.DIRECTION_SOUTH_EAST)));
        ...
        public void AnalysePossibilities()
        {
            ...
            foreach (var spell in Fighter.Spells.Values)
            {
                ...
                if ((category & SpellCategory.Summoning) != 0 &&
                    ((category & SpellCategory.Healing) == 0 ||
                     Fighter.Team.Fighters.Any(x => x.IsDead())) && // revive effect and an ally is dead
                    (Fighter.CanSummon() || (category & SpellCategory.Healing) != 0 &&
                        Fighter.Team.Fighters.Any(x => x.IsDead()))) // can summon or is revive spell
                {
                    var adjacentCell = Fighter.GetCastZoneSet(spell.CurrentSpellLevel, Fighter.Position.Point)
                        .EnumerateValidPoints().OrderBy(x => x.ManhattanDistanceTo(Fighter.Position.Point))
                ...
                {
                    var cells =
                        ExpandCellsZone(Fighter.Fight.Fighters
                            .Where(fighter => fighter.IsAlive() && fighter.IsVisibleFor(Fighter))
                    ...
                    foreach (var target in cells)
                    {
                        ...
                        if (Fighter.CanCastSpell(new SpellCastInformations(Fighter, spell, target.Cell)
                            {CastCell = cell}) != SpellCastResult.OK)
                            continue;
                ...
                {
                    if (cast.IsSummoningSpell && Fighter is SummonedMonster) continue;
        ...
        public AISpellCastPossibility FindFirstSpellCast()
        {
            ...
            foreach (var priority in Priorities.OrderByDescending(x => x.Value))
            {
                ...
                foreach (var possibleCast in Possibilities.OrderByDescending(x => x,
                    new SpellCastComparer(this, priority.Key)))
                {
                    ...
                    if (possibleCast.IsSummoningSpell)
                    {
                        ...
                        {
                            Target = new TargetCell(possibleCast.SummonCell), CastCell = Fighter.Cell,
                            AffectedCells = new[] {possibleCast.SummonCell}
                        };
            ...
            if (casts.Count > 1)
            {
                ...
                if (casts[1].Spell.CurrentSpellLevel.ApCost <=
                    Fighter.AP - max * casts[0].Spell.CurrentSpellLevel.ApCost &&
                    casts[0].MoveBefore != null)
                {
                    ...
                    var pathfinder = new Pathfinder(m_environment.CellInformationProvider);
                    var path = pathfinder.FindPath(casts[1].MoveBefore.EndCell.Id,
                        casts[0].MoveBefore != null ? casts[0].MoveBefore.EndCell.Id : Fighter.Cell.Id, false);
        ...
        public int MaxConsecutiveSpellCast(Spell spell, int ap)
        {
            ...
            if ((category & SpellCategory.Summoning) != 0) return 1;
        ...
        public SpellTarget ComputeSpellImpact(Spell spell, Cell targetCell, Cell castCell)
        {
            ...
            foreach (var handler in cast.GetEffectHandlers())
            {
                ...
                foreach (var target in handler.GetAffectedActors())
                    if (target != Fighter ||
                        handler.AffectedCells
                            .Contains(castCell)) // we take in account the movement of the caster before the spell cast
        ...
        private void CumulEffects(SpellEffectHandler handler, ref SpellTarget spellImpact, FightActor target,
            Spell spell)
        {
            ...
            if (Fighter is SummonedTurret) isFriend = category == SpellCategory.Healing;
            ...
            if (target is SummonedFighter)
                chanceToHappen /=
                    2; // It's much better to hit non-summoned foes => effect on summons (except allies summon for Osa) is divided by 2. 
            ...
            if ((category & SpellCategory.DamagesNeutral) > 0)
                AdjustDamage(result, min, max, SpellCategory.DamagesNeutral, chanceToHappen,
                    Fighter.Stats.GetTotal(PlayerFields.NeutralDamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.PhysicalDamage),
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonusPercent) +
                    Fighter.Stats.GetTotal(PlayerFields.Strength),
            ...
            if ((category & SpellCategory.DamagesFire) > 0)
                AdjustDamage(result, min, max, SpellCategory.DamagesNeutral, chanceToHappen,
                    Fighter.Stats.GetTotal(PlayerFields.FireDamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonus) + Fighter.Stats.GetTotal(PlayerFields.MagicDamage),
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonusPercent) +
            ...
            if ((category & SpellCategory.DamagesAir) > 0)
                AdjustDamage(result, min, max, SpellCategory.DamagesNeutral, chanceToHappen,
                    Fighter.Stats.GetTotal(PlayerFields.AirDamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonus) + Fighter.Stats.GetTotal(PlayerFields.MagicDamage),
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonusPercent) +
                    Fighter.Stats.GetTotal(PlayerFields.Agility),
            ...
            if ((category & SpellCategory.DamagesWater) > 0)
                AdjustDamage(result, min, max, SpellCategory.DamagesNeutral, chanceToHappen,
                    Fighter.Stats.GetTotal(PlayerFields.WaterDamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonus) + Fighter.Stats.GetTotal(PlayerFields.MagicDamage),
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonusPercent) +
                    Fighter.Stats.GetTotal(PlayerFields.Chance),
            ...
            if ((category & SpellCategory.DamagesEarth) > 0)
                AdjustDamage(result, min, max, SpellCategory.DamagesNeutral, chanceToHappen,
                    Fighter.Stats.GetTotal(PlayerFields.EarthDamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonus) +
                    Fighter.Stats.GetTotal(PlayerFields.PhysicalDamage),
                    Fighter.Stats.GetTotal(PlayerFields.DamageBonusPercent) +
                    Fighter.Stats.GetTotal(PlayerFields.Strength),
            ...
            if ((category & SpellCategory.Healing) > 0)
            {
                ...
                else
                {
                    if (hptoHeal > 0)
                    {
                        AdjustDamage(result, (uint) Math.Min(effect.DiceNum, hptoHeal),
                            (uint) Math.Min(effect.DiceFace, hptoHeal), SpellCategory.Healing, chanceToHappen,
            ...
            if ((category & SpellCategory.Curse) > 0)
            {
                ...
                if (effect.EffectId ==
                    EffectsEnum.Effect_SkipTurn) // Let say this effect counts as 2 damage per level of the target
            ...
            if (!isFriend && (category & SpellCategory.Damages) > 0 &&
                result.MinDamage >
                target.LifePoints) // Enough damage to kill the target => affect an arbitrary 50% of max heal (with at least current health), so strong spells are not favored anymore. 
            {
                var ratio = Math.Max(target.MaxLifePoints / 2d, target.LifePoints) / result.MinDamage;
