...
namespace Stump.Server.WorldServer.AI.Fights.Brain.Custom.Summons
{
    ...
    public class BarrelBrain : Brain
    {
        ...
        private void OnTurnStarted(IFight fight, FightActor player)
        {
            ...
            var fighters = Fighter.Fight.GetAllFightersInLine(Fighter.Cell,
                (int) (11 + 2 * (1 - barrel.MonsterGrade.GradeId)), DirectionsEnum.DIRECTION_NORTH_EAST);
            fighters.AddRange(Fighter.Fight.GetAllFightersInLine(Fighter.Cell,
                (int) (11 + 2 * (1 - barrel.MonsterGrade.GradeId)), DirectionsEnum.DIRECTION_SOUTH_EAST));
            fighters.AddRange(Fighter.Fight.GetAllFightersInLine(Fighter.Cell,
                (int) (11 + 2 * (1 - barrel.MonsterGrade.GradeId)), DirectionsEnum.DIRECTION_SOUTH_WEST));
            fighters.AddRange(Fighter.Fight.GetAllFightersInLine(Fighter.Cell,
                (int) (11 + 2 * (1 - barrel.MonsterGrade.GradeId)), DirectionsEnum.DIRECTION_NORTH_WEST));

            var spell = new Spell(1674, (short) barrel.MonsterGrade.GradeId);

            foreach (var fighter in fighters)
            {
                if (barrel.Position.Point.IsAdjacentTo(fighter.Position.Point) ||
                    barrel.CanCastSpell(spell, fighter.Cell) != SpellCastResult.OK)
                    continue;

                if (barrel.IsFriendlyWith(fighter) && fighter.HasState((int) SpellStatesEnum.SAOUL_498))
                    barrel.CastSpell(spell, fighter.Cell);

                else if (!barrel.IsFriendlyWith(fighter)) barrel.CastSpell(spell, fighter.Cell);
            }
        }
    }
