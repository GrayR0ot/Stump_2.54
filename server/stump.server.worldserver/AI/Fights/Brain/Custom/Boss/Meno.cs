            ...
            if (castHandler.Spell.Template.Id == 6808 || castHandler.Spell.Template.Id == 6812)
                foreach (var entity in Fighter.Fight.GetAllFighters())
                {
                    if (entity is MonsterFighter)
                    {
                        var monster = entity as MonsterFighter;
                        monster.Invulnerable = true;
                        if (monster.LifePoints >= 6000) monster.InflictDirectDamage(1000);
                        if (monster.LifePoints < 3000) monster.Die();
                    }
                    ...
                    if (entity is SummonedMonster)
                    {
                        ...
                        if (summonedMonster.LifePoints >= 6000) summonedMonster.InflictDirectDamage(1000);
                        if (summonedMonster.LifePoints < 3000) summonedMonster.Die();
                    }
                }
        }
    }
