...
namespace Stump.Server.WorldServer.AI.Fights.Brain.Custom
{
    ...
    public class HamrackBrain : Brain
    {
        ...
        {
            var target = Environment.GetNearestFighter(x =>
                x.IsFriendlyWith(Fighter) && x != Fighter &&
                Fighter.Position.Point.SquareDistanceTo(x.Position.Point) < 3);
        ...
        private void OnGetAlive(FightActor fighter)
        {
            ...
            if (fighter is SummonedMonster && (fighter as SummonedMonster).Monster.MonsterId == 2955)
                fighter.CastAutoSpell(new Spell(2617, 1), fighter.Cell);
        }
    }
}