    ...
    public class AccountController : ApiController
    {
        ...
        public IHttpActionResult AddTokens(int accountId, int amount)
        {
            ...
            WorldServer.Instance.IOTaskPool.ExecuteInContext(() =>
            {
                ...
                var account = ClientManager.Instance.Clients.OfType<WorldClient>()
                ...
                if (account == null)
                {
                    ...
                    {
                        {"AcctId", accountId}, {"AcctName", ""}, {"CharacterId", 0}, {"CharacterName", ""},
                        {"Amount", amount}, {"Date", DateTime.Now.ToString(CultureInfo.InvariantCulture)}
                    };
                ...
                if (account.Character == null)
                {
                    ...
                    var documentAccount = new BsonDocument
                    {
                        {"AcctId", accountId}, {"AcctName", account.Account.Login}, {"CharacterId", 0},
                        {"CharacterName", ""}, {"Amount", amount},
                ...
                {
                    {"AcctId", accountId}, {"AcctName", account.Character.Account.Login},
                    {"CharacterId", account.Character.Id}, {"CharacterName", account.Character.Name},
                    {"Amount", amount}, {"Date", DateTime.Now.ToString(CultureInfo.InvariantCulture)}
                };
