...
namespace Stump.Server.WorldServer.Handlers.Friends
{
    public class FriendHandler : WorldHandlerContainer
    {
        ...
        {
            var friend =
                client.Character.FriendsBook.Friends.FirstOrDefault(entry => entry.Account.Id == message.AccountId);
        ...
        public static void HandleFriendWarnOnLevelGainStateMessage(WorldClient client,
        ...
        public static void SendIgnoredDeleteResultMessage(IPacketReceiver client, bool success, bool session,
            string name)
        {
        ...
        {
            client.Send(new IgnoredListMessage(ignoreds.Where(x => !x.Session)
                .Select(entry => entry.GetIgnoredInformations()).ToArray()));
        }
