...
namespace Stump.Server.WorldServer.Handlers.Interactives
{
    public class InteractiveHandler : WorldHandlerContainer
    {
        ...
        {
            client.Character.Map.UseInteractiveObject(client.Character, (int) message.ElemId,
                (int) message.SkillInstanceUid);
        }
        ...
        public static void HandleZaapRespawnSaveRequestMessage(WorldClient client,
            ZaapRespawnSaveRequestMessage message)
        {
        ...
        public static void SendInteractiveUsedMessage(IPacketReceiver client, Character user,
            InteractiveObject interactiveObject, Skill skill)
        {
            client.Send(new InteractiveUsedMessage((uint) user.Id, (uint) interactiveObject.Id,
                (ushort) skill.SkillTemplate.Id, (ushort) (skill.GetDuration(user, true) / 100), true));
        }
        ...
        public static void SendStatedElementUpdatedMessage(IPacketReceiver client, int elementId, short elementCellId,
            int state)
        {
            client.Send(
                new StatedElementUpdatedMessage(
                    new StatedElement(elementId, (ushort) elementCellId, (uint) state, true)));
        }
        ...
        public static void SendInteractiveElementUpdatedMessage(IPacketReceiver client, Character character,
        ...
        public static void SendInteractiveUseEndedMessage(IPacketReceiver client, InteractiveObject interactive,
            Skill skill)
        {
