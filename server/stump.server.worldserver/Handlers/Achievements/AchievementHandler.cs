...
namespace Stump.Server.WorldServer.Handlers.Achievements
{
    public class AchievementHandler : WorldHandlerContainer
    {
        ...
        public static void HandleDareRewardConsumeRequestMessage(WorldClient client,
            DareRewardConsumeRequestMessage message)
        {
            var Achievements = new List<AchievementTemplate>();
            foreach (var achiev in client.Character.Achievement.FinishedAchievements) Achievements.Add(achiev);
            foreach (var Achievement in Achievements)
            {
                ...
                if (achievement != null)
                {
                    int experience;
                    int guildExperience;
                    if (client.Character.Achievement.RewardAchievement(achievement, out experience,
                        out guildExperience))
                    {
        ...
        {
            client.Send(new AchievementDetailedListMessage(startedAchievements.ToArray(),
        ...
        public static void SendAchievementFinishedMessage(IPacketReceiver client, ushort id, ulong charId,
            ushort finishedLevel)
        {
