...
namespace Stump.Server.WorldServer.Handlers.Characters
{
    ...
    {
        [Variable] public static int MaxDayCharacterDeletion = 5;

        [WorldHandler(CharacterDeletionRequestMessage.Id, ShouldBeLogged = false, IsGamePacket = false)]
        public static void HandleCharacterDeletionRequestMessage(WorldClient client,
            CharacterDeletionRequestMessage message)
        {
            ...
            if (ExperienceManager.Instance.GetCharacterLevel(character.Experience) <= 20 ||
                ...
                && secretAnswerHash == (message.CharacterId + "~" + client.Account.SecretAnswer).GetMD5())
            {
                ...
                {
                    client.Send(
                        new CharacterDeletionErrorMessage((int) CharacterDeletionErrorEnum
                            .DEL_ERR_TOO_MANY_CHAR_DELETION));
                    return;
                }
            ...
            {
                client.Send(
                    new CharacterDeletionErrorMessage((int) CharacterDeletionErrorEnum.DEL_ERR_BAD_SECRET_ANSWER));
            }
        }
    }
}