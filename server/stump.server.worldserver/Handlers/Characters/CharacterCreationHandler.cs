...
namespace Stump.Server.WorldServer.Handlers.Characters
{
    ...
    {
        [Variable] public static bool EnableNameSuggestion = true;

        [WorldHandler(CharacterCreationRequestMessage.Id, ShouldBeLogged = false, IsGamePacket = false)]
        public static void HandleCharacterCreationRequestMessage(WorldClient client,
            CharacterCreationRequestMessage message)
        {
            ...
            CharacterManager.Instance.CreateCharacter(client, message.Name, message.Breed, message.Sex, message.Colors,
                message.CosmeticId,
        ...
        public static void HandleCharacterNameSuggestionRequestMessage(WorldClient client,
            CharacterNameSuggestionRequestMessage message)
        {
            if (!EnableNameSuggestion)
            {
                client.Send(
                    new CharacterNameSuggestionFailureMessage((sbyte) NicknameGeneratingFailureEnum
                        .NICKNAME_GENERATOR_UNAVAILABLE));
                return;
            }
        ...
        public static void SendCharacterCreationResultMessage(IPacketReceiver client,
            CharacterCreationResultEnum result)
        {
