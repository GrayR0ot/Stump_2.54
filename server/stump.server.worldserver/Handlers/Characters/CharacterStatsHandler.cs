...
namespace Stump.Server.WorldServer.Handlers.Characters
{
    public partial class CharacterHandler : WorldHandlerContainer
    {
        ...
        public static void SendCharacterLevelUpInformationMessage(IPacketReceiver client, Character character,
            ushort level)
        {
            client.Send(new CharacterLevelUpInformationMessage(level, character.Name, (ulong) character.Id));
        }

        public static void SendGameRolePlayPlayerLifeStatusMessage(IPacketReceiver client, PlayerLifeStatusEnum status,
            int phoenixMapId)
        {
