...
namespace Stump.Server.WorldServer.Handlers.Idols
{
    public class IdolHandler : WorldHandlerContainer
    {
        ...
        public static void HandleIdolPartyRegisterRequestMessage(WorldClient client,
            IdolPartyRegisterRequestMessage message)
        {
        ...
        public static void SendIdolListMessage(WorldClient client, IEnumerable<PlayerIdol> chosenIdols,
            IEnumerable<PlayerIdol> partyChosenIdols, IEnumerable<PartyIdol> partyIdols)
        {
            client.Send(new IdolListMessage(chosenIdols.Select(x => (ushort) x.Id).ToArray(),
                partyChosenIdols.Select(x => (ushort) x.Id).ToArray(), partyIdols.ToArray()));
        }
        ...
        public static void SendIdolSelectErrorMessage(WorldClient client, bool activate, bool party, short idolId,
            sbyte reason)
        {
