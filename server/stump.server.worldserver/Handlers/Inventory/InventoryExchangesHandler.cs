...
namespace Stump.Server.WorldServer.Handlers.Inventory
{
    public partial class InventoryHandler : WorldHandlerContainer
    {
        ...
        public static void HandleExchangePlayerMultiCraftRequestMessage(WorldClient client,
            ExchangePlayerMultiCraftRequestMessage message)
        {
            ...
            var interactives = client.Character.Map.GetInteractiveObjects().Where(x =>
                    x.GetSkills().Any(y => y.SkillTemplate?.Id == message.SkillId && y.IsEnabled(client.Character)))
                .ToArray();
        ...
        public static void HandleExchangeObjectMoveKamaMessage(WorldClient client,
            ExchangeObjectMoveKamaMessage message)
        {
            if (!client.Character.IsInExchange()) return;
            client.Character.Exchanger.SetKamas(message.Quantity);
        }

        [WorldHandler(ExchangeCraftPaymentModificationRequestMessage.Id)]
        public static void HandleExchangeCraftPaymentModificationRequestMessage(WorldClient client,
            ExchangeCraftPaymentModificationRequestMessage message)
        {
            if (!(client.Character.Dialoger is CraftCustomer)) return;
        ...
        public static void HandleExchangeBuyMessage(WorldClient client, ExchangeBuyMessage message)
        {
            ...
            if (dialog != null) dialog.BuyItem(message.ObjectToBuyId, message.Quantity);
        }
        ...
        {
            var dialog = client.Character.Dialog as IShopDialog;
            if (dialog != null) dialog.SellItem(message.ObjectToSellId, message.Quantity);
        }
        ...
        public static void HandleExchangeRequestOnShopStockMessage(WorldClient client,
            ExchangeRequestOnShopStockMessage message)
        {
            if (client.Character.IsBusy()) return;
        ...
        public static void HandleExchangeObjectMovePricedMessage(WorldClient client,
            ExchangeObjectMovePricedMessage message)
        {
            ...
            if (client.Character.Exchanger is CharacterMerchant)
                ((CharacterMerchant) client.Character.Exchanger).MovePricedItem(message.ObjectUID, message.Quantity,
                    (long) message.Price);
            else if (client.Character.Exchanger is BidHouseExchanger)
                ((BidHouseExchanger) client.Character.Exchanger).MovePricedItem(message.ObjectUID, message.Quantity,
                    (long) message.Price);
        }

        [WorldHandler(ExchangeObjectModifyPricedMessage.Id)]
        public static void HandleExchangeObjectModifyPricedMessage(WorldClient client,
            ExchangeObjectModifyPricedMessage message)
        {
            if (!client.Character.IsInExchange()) return;

            if (client.Character.Exchanger is CharacterMerchant)
            {
                ((CharacterMerchant) client.Character.Exchanger).ModifyItem(message.ObjectUID, message.Quantity,
                    (uint) message.Price);
            }
            else if (client.Character.Exchanger is BidHouseExchanger)
            {
        ...
        public static void HandleExchangeOnHumanVendorRequestMessage(WorldClient client,
            ExchangeOnHumanVendorRequestMessage message)
        {
            var merchant = client.Character.Map.GetActor<Merchant>((int) message.HumanVendorId);

            if (merchant == null || merchant.Cell.Id != message.HumanVendorCell) return;
        ...
        public static void HandleExchangeRequestOnTaxCollectorMessage(WorldClient client,
            ExchangeRequestOnTaxCollectorMessage message)
        {
            if (client.Character.Guild == null) return;

            var taxCollectorNpc = client.Character.Map.TaxCollector;
            if (taxCollectorNpc == null) return;
            ...
            if (!(string.Equals(taxCollectorNpc.Record.CallerName, client.Character.Name) &&
                  guildMember.HasRight(GuildRightsBitEnum.GUILD_RIGHT_COLLECT_MY_TAX_COLLECTOR) ||
                  guildMember.HasRight(GuildRightsBitEnum.GUILD_RIGHT_COLLECT)))
            {
                client.Send(new TaxCollectorErrorMessage((sbyte) TaxCollectorErrorReasonEnum.TAX_COLLECTOR_NO_RIGHTS));
                return;
            }

            if (taxCollectorNpc.IsBusy()) return;
        ...
        public static void HandleExchangeHandleMountStableMessage(WorldClient client,
            ExchangeHandleMountsMessage message)
        {
            if (!client.Character.IsInExchange()) return;
        ...
        public static void HandleExchangeBidHousePriceMessage(WorldClient client, ExchangeBidHousePriceMessage message)
        {
            ...
            SendExchangeBidPriceForSellerMessage(client, message.GenId,
                BidHouseManager.Instance.GetAveragePriceForItem(message.GenId), true,
                BidHouseManager.Instance.GetMinimalPricesForItem(message.GenId));
        }

        [WorldHandler(ExchangeBidHouseSearchMessage.Id)]
        public static void HandleExchangeBidHouseSearchMessage(WorldClient client,
            ExchangeBidHouseSearchMessage message)
        {
            var exchange = client.Character.Exchange as BidHouseExchange;
            if (exchange == null) return;
            ...
            var categories = BidHouseManager.Instance.GetBidHouseCategories(message.GenId, exchange.MaxItemLevel)
                .Select(x => x.GetBidExchangerObjectInfo()).ToArray();
        ...
        public static void HandleExchangeBidHouseBuyMessage(WorldClient client, ExchangeBidHouseBuyMessage message)
        {
            ...
            var item = category.GetItem(message.Qty, (long) message.Price);
            if (item == null)
            {
            ...
            if (result) client.Character.Inventory.SubKamas((ulong) item.Price);

            SendExchangeBidHouseBuyResultMessage(client, item.Guid, result);

            //%3 x {item,%1,%2} (%4 kamas)
            client.Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 252,
                item.Template.Id, item.Guid, item.Stack, item.Price);
        }

        [WorldHandler(ExchangeCraftCountRequestMessage.Id)]
        public static void HandleExchangeCraftCountRequestMessage(WorldClient client,
        ...
        public static void HandleExchangeSetCraftRecipeMessage(WorldClient client,
            ExchangeSetCraftRecipeMessage message)
        {
            if (!JobManager.Instance.Recipes.ContainsKey(message.ObjectGID)) return;

            var craftActor = client.Character.Dialoger as CraftingActor;

            if (craftActor == null) return;
        ...
        public static void HandleJobCrafterDirectoryListRequestMessage(WorldClient client,
            JobCrafterDirectoryListRequestMessage message)
        {
        ...
        public static void HandleExchangeObjectTransfertAllFromInvMessage(WorldClient client,
            ExchangeObjectTransfertAllFromInvMessage message)
        {
        ...
        public static void HandleExchangeObjectTransfertExistingFromInvMessage(WorldClient client,
            ExchangeObjectTransfertExistingFromInvMessage message)
        {
            var bank = client.Character.Dialog as BankDialog;
            var mount = client.Character.Dialog as MountDialog;
            if (bank != null)
        ...
        public static void HandleExchangeObjectTransfertListFromInvMessage(WorldClient client,
            ExchangeObjectTransfertListFromInvMessage message)
        {
            var bank = client.Character.Dialog as BankDialog;
            var mount = client.Character.Dialog as MountDialog;
            var trade = client.Character.Dialog as PlayerTrade;
        ...
        public static void HandleExchangeObjectTransfertAllToInvMessage(WorldClient client,
            ExchangeObjectTransfertAllToInvMessage message)
        {
            var bank = client.Character.Dialog as BankDialog;
            var mount = client.Character.Dialog as MountDialog;
            if (bank != null)
        ...
        public static void HandleExchangeObjectTransfertExistingToInvMessage(WorldClient client,
            ExchangeObjectTransfertExistingToInvMessage message)
        {
            var bank = client.Character.Dialog as BankDialog;
            var mount = client.Character.Dialog as MountDialog;
            if (bank != null)
        ...
        public static void HandleExchangeObjectTransfertListToInvMessage(WorldClient client,
            ExchangeObjectTransfertListToInvMessage message)
        {
            var bank = client.Character.Dialog as BankDialog;
            var mount = client.Character.Dialog as MountDialog;
            if (bank != null)
        ...
        public static void HandleExchangeObjectUseInWorkshopMessage(WorldClient client,
            ExchangeObjectUseInWorkshopMessage message)
        {
            (client.Character.Dialog as MultiRuneMagicCraftDialog)?.MoveItemFromBag(message.ObjectUID,
                message.Quantity);
        }

        [WorldHandler(ExchangeRequestOnMountStockMessage.Id)]
        public static void HandleExchangeRequestOnMountStockMessage(WorldClient client,
            ExchangeRequestOnMountStockMessage message)
        {
        ...
        public static void SendExchangeRequestedTradeMessage(IPacketReceiver client, ExchangeTypeEnum type,
            Character source,
            Character target)
        {
        ...
        public static void SendExchangeStartedWithStorageMessage(IPacketReceiver client, ExchangeTypeEnum type,
            int storageMaxSlot)
        {
        ...
        public static void SendExchangeStartedTaxCollectorShopMessage(IPacketReceiver client,
            TaxCollectorNpc taxCollector)
        {
            client.Send(new ExchangeStartedTaxCollectorShopMessage(
                taxCollector.Bag.Select(x => x.GetObjectItem()).ToArray(), (ulong) taxCollector.GatheredKamas));
        }

        public static void SendExchangeStartOkHumanVendorMessage(IPacketReceiver client, Merchant merchant)
        {
            client.Send(new ExchangeStartOkHumanVendorMessage(merchant.Id,
                merchant.Bag.Where(x => x.Stack > 0).Select(x => x.GetObjectItemToSellInHumanVendorShop()).ToArray()));
        }
        ...
        {
            client.Send(new ExchangeStartOkNpcShopMessage(dialog.Npc.Id,
                dialog.Token != null ? (ushort) dialog.Token.Id : (ushort) 0,
                dialog.Items.Select(entry => entry.GetNetworkItem() as ObjectItemToSellInNpcShop).ToArray()));
        }
        ...
        public static void SendExchangeStartOkMountMessage(IPacketReceiver client, IEnumerable<Mount> stabledMounts,
            IEnumerable<Mount> paddockedMounts)
        {
            client.Send(new ExchangeStartOkMountMessage(stabledMounts.Select(x => x.GetMountClientData()).ToArray(),
                paddockedMounts.Select(x => x.GetMountClientData()).ToArray()));
        }
        ...
        public static void SendExchangeStartedBidSellerMessage(IPacketReceiver client, BidHouseExchange exchange,
            IEnumerable<ObjectItemToSellInBid> items)
        {
            client.Send(new ExchangeStartedBidSellerMessage(exchange.GetBuyerDescriptor(), items.ToArray()));
        }

        public static void SendExchangeTypesExchangerDescriptionForUserMessage(IPacketReceiver client,
        ...
        public static void SendExchangeTypesItemsExchangerDescriptionForUserMessage(IPacketReceiver client,
            IEnumerable<BidExchangerObjectInfo> items)
        {
        ...
        public static void SendExchangeBidPriceForSellerMessage(IPacketReceiver client, ushort itemId, long average,
            bool allIdentical, IEnumerable<ulong> prices)
        {
        ...
        {
            client.Send(new ExchangeBidHouseInListAddedMessage(category.Id, category.TemplateId,
        ...
        {
            client.Send(new ExchangeBidHouseInListUpdatedMessage(category.Id, category.TemplateId,
                category.Effects.Select(x => x.GetObjectEffect()).ToArray(), category.GetPrices().ToArray()));
        }
        ...
        public static void SendExchangeOfflineSoldItemsMessage(IPacketReceiver client,
            ObjectItemQuantityPriceDateEffects[] merchantItems, ObjectItemQuantityPriceDateEffects[] bidHouseItems)
        {
        ...
        public static void SendExchangeCraftResultWithObjectIdMessage(IPacketReceiver client,
            ExchangeCraftResultEnum result, ItemTemplate item)
        {
            client.Send(new ExchangeCraftResultWithObjectIdMessage((sbyte) result, (ushort) item.Id));
        }

        public static void SendExchangeCraftResultWithObjectDescMessage(IPacketReceiver client,
            ExchangeCraftResultEnum result, BasePlayerItem createdItem, int amount)
        {
            client.Send(new ExchangeCraftResultWithObjectDescMessage((sbyte) result,
                new ObjectItemNotInContainer((ushort) createdItem.Template.Id,
                    createdItem.Effects.Select(x => x.GetObjectEffect()).ToArray(), (uint) createdItem.Guid,
                    (uint) amount)));
        }

        public static void SendExchangeCraftInformationObjectMessage(IPacketReceiver client, IItem item,
            Character owner, ExchangeCraftResultEnum result)
        {
            client.Send(new ExchangeCraftInformationObjectMessage((sbyte) result, (ushort) item.Template.Id,
                (ulong) owner.Id));
        }

        public static void SendExchangeOkMultiCraftMessage(IPacketReceiver client, Character initiator, Character other,
            ExchangeTypeEnum role)
        {
            client.Send(new ExchangeOkMultiCraftMessage((ulong) initiator.Id, (ulong) other.Id, (sbyte) role));
        }

        public static void SendExchangeStartOkMulticraftCrafterMessage(IPacketReceiver client,
            InteractiveSkillTemplate skillTemplate)
        {
            client.Send(new ExchangeStartOkMulticraftCrafterMessage((uint) skillTemplate.Id));
        }

        public static void SendExchangeStartOkMulticraftCustomerMessage(IPacketReceiver client,
            InteractiveSkillTemplate skillTemplate, Job job)
        {
        ...
        {
            client.Send(
                new JobCrafterDirectoryListMessage(entries.Select(x => x.GetJobCrafterDirectoryListEntry()).ToArray()));
        }
        ...
        public static void SendExchangeCraftResultMagicWithObjectDescMessage(IPacketReceiver client,
            CraftResultEnum craftResult, IItem item, IEnumerable<EffectBase> effects, MagicPoolStatus poolStatus)
        {
            client.Send(new ExchangeCraftResultMagicWithObjectDescMessage((sbyte) craftResult,
                new ObjectItemNotInContainer((ushort) item.Template.Id,
                    effects.Select(x => x.GetObjectEffect()).ToArray(), (uint) item.Guid, item.Stack),
                (sbyte) poolStatus));
        }

        public static void SendExchangeItemAutoCraftStopedMessage(IPacketReceiver client,
            ExchangeReplayStopReasonEnum reason)
        {
        ...
        public static void SendDecraftResultMessage(IPacketReceiver client,
            IEnumerable<DecraftedItemStackInfo> itemsInfo)
        {
