...
namespace Stump.Server.WorldServer.Handlers.Guilds
{
    public class GuildHandler : WorldHandlerContainer
    {
        ...
        public static void HandleGuildCharacsUpgradeRequestMessage(WorldClient client,
            GuildCharacsUpgradeRequestMessage message)
        {
            if (client.Character.Guild == null)
                return;
        ...
        public static void HandleGuildSpellUpgradeRequestMessage(WorldClient client,
            GuildSpellUpgradeRequestMessage message)
        {
            if (client.Character.Guild == null)
                return;
        ...
        public static void HandleGuildModificationValidMessage(WorldClient client,
            GuildModificationValidMessage message)
        {
        ...
        public static void HandleGuildModificationNameValidMessage(WorldClient client,
            GuildModificationNameValidMessage message)
        {
        ...
        public static void HandleGuildModificationEmblemValidMessage(WorldClient client,
            GuildModificationEmblemValidMessage message)
        {
        ...
        public static void HandleGuildChangeMemberParametersMessage(WorldClient client,
            GuildChangeMemberParametersMessage message)
        {
            if (client.Character.Guild == null)
                return;
        ...
        public static void HandleGuildInvitationMessage(WorldClient client, GuildInvitationMessage message)
        {
            ...
            {
                client.Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_ERROR, 55,
                    client.Character.Guild.MaxMembers);
                return;
            }
        ...
        public static void HandleGuildMemberSetWarnOnConnectionMessage(WorldClient client,
            GuildMemberSetWarnOnConnectionMessage message)
        {
        ...
        public static void HandleGuildBulletinSetRequestMessage(WorldClient client,
            GuildBulletinSetRequestMessage message)
        {
            if (client.Character.GuildMember == null)
            {
        ...
        {
            client.Send(new GuildInvitedMessage((ulong) recruter.Id, recruter.Name,
                recruter.Guild.GetBasicGuildInformations()));
        }

        public static void SendGuildInvitationStateRecrutedMessage(IPacketReceiver client,
            GuildInvitationStateEnum state)
        {
            client.Send(new GuildInvitationStateRecrutedMessage((sbyte) state));
        }

        public static void SendGuildInvitationStateRecruterMessage(IPacketReceiver client, Character recruted,
            GuildInvitationStateEnum state)
        {
        ...
        {
            client.Send(new GuildInformationsGeneralMessage(false, (byte) guild.Level,
                (ulong) guild.ExperienceLevelFloor, (ulong) guild.Experience,
                (ulong) guild.ExperienceNextLevelFloor, guild.CreationDate.GetUnixTimeStamp(),
                (ushort) guild.Members.Count(), (ushort) guild.Members.Count(x => x.IsConnected)));
        }

        public static void SendGuildInformationsMembersMessage(IPacketReceiver client, Guild guild)
        {
            client.Send(
                new GuildInformationsMembersMessage(guild.Members.Select(x => x.GetNetworkGuildMember()).ToArray()));
        }
        ...
        {
            client.Send(new GuildInfosUpgradeMessage((sbyte) guild.MaxTaxCollectors, (sbyte) guild.TaxCollectors.Count,
                (ushort) guild.TaxCollectorHealth, (ushort) guild.TaxCollectorDamageBonuses,
                (ushort) guild.TaxCollectorPods, (ushort) guild.TaxCollectorProspecting,
                (ushort) guild.TaxCollectorWisdom, (ushort) guild.Boost,
                Guild.TAX_COLLECTOR_SPELLS.Select(x => (ushort) x).ToArray(),
                guild.GetTaxCollectorSpellsLevels().Select(x => (short) x).ToArray()));
        }
        ...
        public static void SendGuildModificationStartedMessage(IPacketReceiver client, bool changeName,
            bool changeEmblem)
        {
        ...
        {
            client.Send(new GuildMotdMessage(guild.MotdContent, guild.MotdDate.GetUnixTimeStamp(),
                (uint?) guild.MotdMember?.Id ?? 0, guild.MotdMember?.Name ?? "Unknown"));
        }

        public static void SendGuildBulletinMessage(IPacketReceiver client, Guild guild)
        {
            client.Send(new GuildBulletinMessage(guild.BulletinContent, guild.BulletinDate.GetUnixTimeStamp(),
                (uint?) guild.BulletinMember?.Id ?? 0, guild.BulletinMember?.Name ?? "Unknown",
                guild.LastNotifiedDate.GetUnixTimeStamp()));
        }
