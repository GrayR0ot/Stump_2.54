...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler : WorldHandlerContainer
    {
        ...
        {
            if (sub)
                client.Send(new GameActionFightDispellableEffectMessage((ushort) ActionsEnum.ACTION_DEBOOST_SPELL_RANGE,
                    buff.Caster.Id, buff.GetAbstractFightDispellableEffect()));
            else
                client.Send(new GameActionFightDispellableEffectMessage((ushort) ActionsEnum.ACTION_BOOST_SPELL_RANGE,
                    buff.Caster.Id, buff.GetAbstractFightDispellableEffect()));
        }
        ...
        public static void ModifySpellCastTestLos(IPacketReceiver client, Buff buff)
        {
            client.Send(new GameActionFightDispellableEffectMessage(
                (ushort) ActionsEnum.ACTION_BOOST_SPELL_NOLINEOFSIGHT, buff.Caster.Id,
                buff.GetAbstractFightDispellableEffect()));
        }
        ...
        public static void HandleGameMapMovementRequestMessage(WorldClient client,
            GameMapMovementRequestMessage message)
        {
            ...
            {
                SendGameMapNoMovementMessage(client, (short) client.Character.Position.Point.X,
                    (short) client.Character.Position.Point.Y);
                return;
            }
        ...
        public static void HandleGameMapMovementConfirmMessage(WorldClient client,
            GameMapMovementConfirmMessage message)
        {
        ...
        {
            client.Send(
                new ShowCellSpectatorMessage(spectator.Character.Id, (ushort) cell.Id, spectator.Character.Name));
        }
        ...
        public static void SendGameMapMovementMessage(IPacketReceiver client, IEnumerable<short> movementsKey,
            ContextActor actor)
        {
            client.Send(new GameMapMovementMessage(movementsKey.ToArray(), 0, actor.Id));
        }

        public static void SendGameCautiousMapMovementMessage(IPacketReceiver client, IEnumerable<short> movementsKey,
            ContextActor actor)
        {
        ...
        {
            client.Send(new GameEntitiesDispositionMessage(actors
                .Select(entry => entry.GetIdentifiedEntityDispositionInformations()).ToArray()));
        }

        public static void SendGameActionFightActivateGlyphTrapMessage(IPacketReceiver client, MarkTrigger trigger,
            short action, FightActor source, bool active)
        {
        ...
        public static void HandleEnterHavenBagRequestMessage(WorldClient client, EnterHavenBagRequestMessage message)
        {
            ...
            var havenBag = World.Instance.GetMap(162793472);
            var astrub = World.Instance.GetMap(191105026);
            ...
            if (player.Map == havenBag)
            {
                var previousMap = World.Instance.GetMap(player.Record.MapId);
                client.Character.Teleport(previousMap, previousMap.GetCell(player.Record.CellId));
            }
            else
            {
                if (player.Map.IsDungeon())
                {
                    player.Record.MapId = astrub.Id;
                    player.Record.CellId = 260;
                }
                else
                {
                    player.Record.MapId = player.Map.Id;
                    player.Record.CellId = player.Cell.Id;
                }

                client.Character.Teleport(havenBag, havenBag.GetCell(257));
            }
        }

        [WorldHandler(HavenBagFurnituresRequestMessage.Id)]
        public static void HandleHavenBagFurnituresRequestMessage(WorldClient client,
            HavenBagFurnituresRequestMessage message)
        {
            HavenBagManager.Instance.HavenBagFurnituresUpdate(client, message);
        }
        ...
        public static void HandleEditHavenBagCancelRequestMessage(WorldClient client,
            EditHavenBagCancelRequestMessage message)
        {
            client.Send(new EditHavenBagFinishedMessage());
        }

        [WorldHandler(HavenBagPermissionsUpdateRequestMessage.Id)]
        public static void HandleHavenBagPermissionsUpdateRequestMessage(WorldClient client,
            HavenBagPermissionsUpdateRequestMessage message)
        {
            HavenBagManager.Instance.UpdateHavenBagPermissions(client, message);
        }

        [WorldHandler(TeleportHavenBagRequestMessage.Id)]
        public static void HandleTeleportHavenBagRequestMessage(WorldClient client,
            TeleportHavenBagRequestMessage message)
        {
            var character = World.Instance.GetCharacter((int) message.GuestId);
            if (character == null) return;
            if (character.IsInFight() || character.IsBusy()) return;

            var v = new HavenBagInvitationRequest(client.Character, character);
            HavenBagManager.Instance.AddInvitation(v);
        }

        [WorldHandler(TeleportHavenBagAnswerMessage.Id)]
        public static void HandleTeleportHavenBagAnswerMessage(WorldClient client,
            TeleportHavenBagAnswerMessage message)
        {
        ...
        {
            HavenBagManager.Instance.UpdateHavenBagTheme(client, message);
        }

        [WorldHandler(OpenHavenBagFurnitureSequenceRequestMessage.Id)]
        public static void HandleOpenHavenBagFurnitureSequenceRequestMessage(WorldClient client,
            OpenHavenBagFurnitureSequenceRequestMessage message)
        {
            client.Send(new EditHavenBagStartMessage());
        }

        [WorldHandler(CloseHavenBagFurnitureSequenceRequestMessage.Id)]
        public static void HandleCloseHavenBagFurnitureSequenceRequestMessage(WorldClient client,
            CloseHavenBagFurnitureSequenceRequestMessage message)
        {
            client.Send(new EditHavenBagFinishedMessage());
        }
