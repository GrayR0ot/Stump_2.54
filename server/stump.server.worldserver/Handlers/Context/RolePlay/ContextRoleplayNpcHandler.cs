...
namespace Stump.Server.WorldServer.Handlers.Context.RolePlay
{
    public partial class ContextRoleplayHandler : WorldHandlerContainer
    {
            ...
            NpcGenericActionRequestMessage message)
        {
            ...
            {
                var bidbuy = new BidHouseExchange(client.Character, (client.Character.Dialog as BidHouseExchange).Types,
                    (client.Character.Dialog as BidHouseExchange).MaxItemLevel,
                    !(client.Character.Dialog as BidHouseExchange).Buy);
                bidbuy.Open();
            }
            else
            {
        ...
        public static void SendNpcDialogQuestionMessage(IPacketReceiver client, NpcMessage message,
            IEnumerable<short> replies, params string[] parameters)
        {
            client.Send(new NpcDialogQuestionMessage((ushort) message.Id, parameters,
                replies.Select(x => (uint) x).ToArray()));
        }
    }
}