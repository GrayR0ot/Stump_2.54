    ...
    public partial class ContextRoleplayHandler : WorldHandlerContainer
    {
            ...
            GameRolePlayAttackMonsterRequestMessage message)
        {
            ...
            if (monster != null && (monster.Position.Cell == client.Character.Position.Cell ||
                                    monster.Position.Point.ManhattanDistanceTo(client.Character.Position.Cell) <= 2))
                monster.FightWith(client.Character);
        }

        [WorldHandler(GameFightPlacementSwapPositionsRequestMessage.Id)]
        public static void HandleGameFightPlacementSwapPositionsRequestMessage(WorldClient client,
            GameFightPlacementSwapPositionsRequestMessage message)
        {
        ...
        public static void HandleGameFightPlacementSwapPositionsAcceptMessage(WorldClient client,
            GameFightPlacementSwapPositionsAcceptMessage message)
        {
        ...
        public static void HandleGameFightPlacementSwapPositionsCancelMessage(WorldClient client,
            GameFightPlacementSwapPositionsCancelMessage message)
        {
            if (!client.Character.IsInRequest() || !(client.Character.RequestBox is SwapRequest))
                return;
        ...
        public static void SendGameRolePlayPlayerFightFriendlyRequestedMessage(IPacketReceiver client,
            Character requester,
            Character source,
            Character target)
        {
        ...
        public static void SendGameRolePlayArenaUpdatePlayerInfosMessage(WorldClient client, Character character)
        {
            ...
            client.Send(new GameRolePlayArenaUpdatePlayerInfosAllQueuesMessage(
                new ArenaRankInfos(ranking, rankingLeague, (ushort) character.ArenaDailyMatchsWon_3vs3,
                    (ushort) character.ArenaDailyMatchsCount_3vs3, 10), new ArenaRankInfos(), new ArenaRankInfos()));
        }

        public static void SendGameRolePlayAggressionMessage(IPacketReceiver client, Character challenger,
            Character defender)
        {
            client.Send(new GameRolePlayAggressionMessage((ulong) challenger.Id, (ulong) defender.Id));
        }

        public static void SendGameFightPlacementSwapPositionsMessage(IPacketReceiver client,
        ...
        public static void SendGameFightPlacementSwapPositionsOfferMessage(IPacketReceiver client, Character source,
            Character target)
        {
            client.Send(new GameFightPlacementSwapPositionsOfferMessage(source.Id, source.Fighter.Id,
                (ushort) source.Cell.Id, target.Fighter.Id, (ushort) target.Cell.Id));
        }

        public static void SendGameFightPlacementSwapPositionsCancelledMessage(IPacketReceiver client, Character source,
            Character canceller)
        {
