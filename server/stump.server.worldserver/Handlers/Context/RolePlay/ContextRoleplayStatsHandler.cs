...
namespace Stump.Server.WorldServer.Handlers.Context.RolePlay
{
    ...
    {
        private static readonly Dictionary<StatsBoostTypeEnum, PlayerFields> m_statsEnumRelations =
            new Dictionary<StatsBoostTypeEnum, PlayerFields>
            {
        ...
        public static void HandleStatsUpgradeRequestMessage(WorldClient client, StatsUpgradeRequestMessage message)
        {
            ...
            if (client.Character.IsInIncarnation) return;
            ...
            {
                SendStatsUpgradeResultMessage(client, StatsUpgradeResultEnum.NOT_ENOUGH_POINT,
                    (short) message.BoostPoint);
                return;
            }
            ...
            while (pts >= thresholds[index][1])
            {
                ...
                if (index < thresholds.Length - 1 &&
                    pts / (double) thresholds[index][1] > thresholds[index + 1][0] - actualPoints)
                {
        ...
        public static void SendStatsUpgradeResultMessage(IPacketReceiver client, StatsUpgradeResultEnum result,
            short usedpts)
        {
        ...
        public static void HandleResetCharacterStatsRequestMessage(WorldClient client,
            ResetCharacterStatsRequestMessage message)
        {
