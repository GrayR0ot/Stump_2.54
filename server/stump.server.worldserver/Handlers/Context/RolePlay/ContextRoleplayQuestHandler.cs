    ...
    public partial class ContextRoleplayHandler
    {
        ...
        public static void HandleQuestListRequestMessage(WorldClient client, QuestListRequestMessage message)
        {
            client.Send(new QuestListMessage(client.Character.Quests.Where(x => x.Finished).Select(x => x.Id).ToArray(),
                new ushort[client.Character.Quests.Select(x => x.Finished).Count()],
                client.Character.Quests.Where(x => !x.Finished).Select(x => x.GetQuestActiveInformations()).ToArray(),
                new ushort[0]));
        }

        public static void SendQuestListMessage(IPacketReceiver client)
        {
            client.Send(new QuestListMessage(new ushort[0], new ushort[0], new QuestActiveInformations[0],
                new ushort[0]));
        }

        [WorldHandler(QuestObjectiveValidationMessage.Id)]
        public static void HandleQuestObjectiveValidationMessage(WorldClient client,
            QuestObjectiveValidationMessage message)
        {
            ...
            var objective = quest.CurrentStep.Objectives.FirstOrDefault(x =>
                !x.Finished && x.ObjectiveRecord.ObjectiveId == message.ObjectiveId);
            ...
            if (quest.Id == 489 && objective.Template.Id == 3510)
                quest.CurrentStep.Objectives.FirstOrDefault(x => !x.Finished && x.ObjectiveRecord.ObjectiveId == 3511)
                    .CompleteObjective();
        }
        ...
        public static void SendRefreshMapQuestWithout(WorldClient client, int questId)
        {
            ...
            client.Send(new MapNpcsQuestStatusUpdateMessage(client.Character.Map.Id, new int[0], questflag,
                new[] {-20000}));
        }
