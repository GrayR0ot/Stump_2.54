...
namespace Stump.Server.WorldServer.Handlers.Context.RolePlay
{
    public partial class ContextRoleplayHandler : WorldHandlerContainer
    {
        ...
        public static void HandleJobCrafterDirectoryDefineSettingsMessage(WorldClient client,
            JobCrafterDirectoryDefineSettingsMessage message)
        {
        ...
        public static void HandleJobBookSubscribeRequestMessage(WorldClient client,
            JobBookSubscribeRequestMessage message)
        {
        ...
        public static void SendJobMultiCraftAvailableSkillsMessage(IPacketReceiver client, Character character,
            IEnumerable<InteractiveSkillTemplate> skills, bool enabled)
        {
            client.Send(new JobMultiCraftAvailableSkillsMessage(enabled, (ulong) character.Id,
                skills.Select(x => (ushort) x.Id).ToArray()));
        }

        public static void SendJobExperienceMultiUpdateMessage(IPacketReceiver client, Character character)
        {
            client.Send(new JobExperienceMultiUpdateMessage(character.Jobs.Where(x => x.Id != 1)
                .Select(x => x.GetJobExperience()).ToArray()));
        }
        ...
        public static void SendJobExperienceOtherPlayerUpdateMessage(IPacketReceiver client, Character character,
            Job job)
        {
        ...
        {
            client.Send(new JobDescriptionMessage(character.Jobs.Where(x => x.Id != 1)
                .Select(x => x.GetJobDescription()).ToArray()));
        }

        public static void SendJobCrafterDirectorySettingsMessage(IPacketReceiver client, Character character)
        {
            client.Send(
                new JobCrafterDirectorySettingsMessage(character.Jobs.Select(x => x.GetJobCrafterDirectorySettings())
                    .ToArray()));
        }
