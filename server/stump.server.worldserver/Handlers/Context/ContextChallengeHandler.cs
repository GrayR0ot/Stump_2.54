...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler : WorldHandlerContainer
    {
        ...
        public static void HandleChallengeTargetsListRequestMessage(WorldClient client,
            ChallengeTargetsListRequestMessage message)
        {
            ...
            SendChallengeTargetsListMessage(challenge.Fight.Clients, new[] {(double) challenge.Target.Id},
                new[] {challenge.Target.Cell.Id});
        }

        public static void SendChallengeInfoMessage(IPacketReceiver client, DefaultChallenge challenge)
        {
            client.Send(new ChallengeInfoMessage((ushort) challenge.Id,
                challenge.Target != null ? challenge.Target.Id : -1, (uint) challenge.Bonus, (uint) challenge.Bonus));
        }

        public static void SendChallengeResultMessage(IPacketReceiver client, DefaultChallenge challenge)
        {
            client.Send(new ChallengeResultMessage((ushort) challenge.Id,
                challenge.Status == ChallengeStatusEnum.SUCCESS));
        }

        public static void SendChallengeTargetsListMessage(IPacketReceiver client, IEnumerable<double> targetIds,
            IEnumerable<short> targetCells)
        {
