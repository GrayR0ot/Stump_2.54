...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler : WorldHandlerContainer
    {
            ...
            GameActionFightCastRequestMessage message)
        {
            ...
            else
            {
                var spell = fighter.IsSlaveTurn()
                    ? fighter.GetSlave().GetSpell(message.SpellId)
                    : fighter.GetSpell(message.SpellId);
        ...
        public static void HandleGameActionFightCastOnTargetRequestMessage(WorldClient client,
            GameActionFightCastOnTargetRequestMessage message)
        {
            if (!client.Character.IsFighting())
                return;
        ...
        public static void HandleGameFightPlacementPositionRequestMessage(WorldClient client,
            GameFightPlacementPositionRequestMessage message)
        {
            if (!client.Character.IsFighting())
                return;
            ...
            GameRolePlayPlayerFightRequestMessage message)
        {
            ...
            else // agression
            {
                ...
                {
                    foreach (var character in target.Map.GetAllCharacters()
                        .Where(x => x != target && x != client.Character))
                        ContextRoleplayHandler.SendGameRolePlayAggressionMessage(character.Client, client.Character,
                            target);
                    //<b>%1</b> agresse <b>%2</b>
        ...
        public static void HandleGameRolePlayPlayerFightFriendlyAnswerMessage(WorldClient client,
            GameRolePlayPlayerFightFriendlyAnswerMessage message)
        {
        ...
        {
            if (message.Option == (sbyte) FightOptionsEnum.FIGHT_OPTION_SET_TO_PARTY_ONLY &&
                client.Character.IsPartyLeader())
        ...
        public static void HandleGameFightSpectatePlayerRequestMessage(WorldClient client,
            GameFightSpectatePlayerRequestMessage message)
        {
            ...
            if (!client.Character.FriendsBook.IsFriend(player.Account.Id) &&
                !client.Character.Guild.Members.Any(x => x.Character?.Id == player.Id))
                return;
        ...
        {
            if (fight.FightType == FightTypeEnum.FIGHT_TYPE_BREACH)
            {
                var budget = fight.Winners == fight.ChallengersTeam ? 100 : 0;
                client.Send(new BreachGameFightEndMessage((int) fight.GetFightDuration().TotalMilliseconds, 1, 0,
                    results.ToArray(), fight.GetPartiesNameWithOutcome().ToArray(), budget));
            }
            else
            {
                client.Send(new GameFightEndMessage((int) fight.GetFightDuration().TotalMilliseconds, fight.Bonus, 0,
        ...
        public static void SendGameFightSpectatorJoinMessage(IPacketReceiver client, IFight fight)
        {
            client.Send(new GameFightSpectatorJoinMessage(false, false, false, fight.IsStarted,
                fight.IsStarted ? (short) 0 : (short) (fight.GetPlacementTimeLeft().TotalMilliseconds / 100),
                (sbyte) fight.FightType, fight.GetPartiesName().ToArray()));
        }
        ...
        {
            client.Send(new GameFightTurnResumeMessage(fighterPlaying.Id, (uint) fighterPlaying.TurnTime / 100,
                (uint) fighterPlaying.Fight.GetTurnTimeLeft().TotalMilliseconds / 100));
        }
        ...
        {
            client.Send(new GameFightTurnListMessage(fight.GetAliveFightersIds().Select(x => (double) x).ToArray(),
                fight.GetDeadFightersIds().Select(x => (double) x).ToArray()));
        }
        ...
        {
            client.Send(new GameFightRemoveTeamMemberMessage((ushort) fighter.Fight.Id, (sbyte) fighter.Team.Id,
                fighter.Id));
        }
        ...
        public static void SendGameFightPlacementPossiblePositionsMessage(IPacketReceiver client, IFight fight,
            sbyte team)
        {
        ...
        public static void SendGameFightOptionStateUpdateMessage(IPacketReceiver client, FightTeam team,
            FightOptionsEnum option, bool state)
        {
            client.Send(new GameFightOptionStateUpdateMessage((ushort) team.Fight.Id, (sbyte) team.Id, (sbyte) option,
                state));
        }

        public static void SendGameActionFightSpellCastMessage(IPacketReceiver client, ActionsEnum actionId,
            FightActor caster, FightActor target,
            Cell cell, FightSpellCastCriticalEnum critical, bool silentCast,
            Spell spell, short[] portals)
        {
            client.Send(new GameActionFightSpellCastMessage((ushort) actionId, caster.Id, silentCast, true,
                target == null ? 0 : target.Id, silentCast ? (short) -1 : cell.Id, (sbyte) critical,
        ...
        {
            client.Send(new GameActionFightSpellCastMessage((ushort) actionId, caster.Id, silentCast, false,
                target == null ? 0 : target.Id, cell.Id, (sbyte) critical,
        ...
        {
            client.Send(new GameActionFightNoSpellCastMessage(spell.Id == 0 ? 0 : spell.CurrentSpellLevel.Id));
        }

        public static void SendGameActionFightModifyEffectsDurationMessage(IPacketReceiver client, short actionId,
            FightActor source, FightActor target, short delta)
        {
            client.Send(
                new GameActionFightModifyEffectsDurationMessage((ushort) actionId, source.Id, target.Id, delta));
        }

        public static void SendGameActionFightDispellableEffectMessage(IPacketReceiver client, Buff buff,
            bool update = false)
        {
            client.Send(new GameActionFightDispellableEffectMessage(
                update ? (ushort) ActionsEnum.ACTION_CHARACTER_UPDATE_BOOST : (ushort) buff.GetActionId(),
                buff.Caster.Id, buff.GetAbstractFightDispellableEffect()));
        }

        public static void SendGameActionFightMarkCellsMessage(IPacketReceiver client, MarkTrigger trigger,
            bool visible = true)
        {
            ...
            client.Send(new GameActionFightMarkCellsMessage((ushort) action, trigger.Caster.Id,
                visible ? trigger.GetGameActionMark() : trigger.GetHiddenGameActionMark()));
        }
        ...
        public static void SendGameActionFightTriggerGlyphTrapMessage(IPacketReceiver client, MarkTrigger trigger,
            FightActor target, Spell triggeredSpell)
        {
            var action = trigger.Type == GameActionMarkTypeEnum.GLYPH
                ? ActionsEnum.ACTION_FIGHT_TRIGGER_GLYPH
                : ActionsEnum.ACTION_FIGHT_TRIGGER_TRAP;
            client.Send(new GameActionFightTriggerGlyphTrapMessage((ushort) action, trigger.Caster.Id, trigger.Id,
                (ushort) target.Cell.Id, target.Id, (ushort) triggeredSpell.Id));
        }
        ...
        {
            sbyte slotIndex = 0;
            client.Send(new SlaveSwitchContextMessage(actor.Summoner.Id, actor.Id,
        ...
        {
            sbyte slotIndex = 0;
            client.Send(new SlaveSwitchContextMessage(actor.Master.Id, actor.Id,
        ...
        public static void SendGameFightResumeMessage(IPacketReceiver client, CharacterFighter fighter)
        {
            ...
            if (slaves.Any())
                client.Send(new GameFightResumeWithSlavesMessage(
                    ...
                    slaves.Select(x => new GameFightResumeSlaveInfo(x.Id, x.SpellHistory.GetCooldowns(),
                        (sbyte) x.SummonedCount, (sbyte) x.BombsCount)).ToArray()));
            else
        ...
        {
            client.Send(
                new GameActionFightKillMessage((ushort) ActionsEnum.ACTION_CHARACTER_KILL, source.Id, target.Id));
        }
    }
}