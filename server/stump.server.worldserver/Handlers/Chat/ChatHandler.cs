    ...
    public partial class ChatHandler : WorldHandlerContainer
    {
        ...
        public static void HandleChatClientPrivateMessage(WorldClient client, ChatClientPrivateMessage message)
        {
            ...
            if (receiver.IsMuted())
            {
                ...
                sender.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 168, receiver.Name,
                    (int) receiver.GetMuteRemainingTime().TotalMinutes);
                return;
            }
            ...
            if (sender.Status.StatusId != (sbyte) PlayerStatusEnum.PLAYER_STATUS_AVAILABLE
                && sender.Status.StatusId != (sbyte) PlayerStatusEnum.PLAYER_STATUS_PRIVATE ||
                !sender.FriendsBook.IsFriend(receiver.Account.Id))
            ...
            //Send to receiver
            SendChatServerMessage(receiver.Client, sender, ChatActivableChannelsEnum.PSEUDO_CHANNEL_PRIVATE,
                message.Content);
            //Send a copy to sender
            SendChatServerCopyMessage(client, sender, receiver, ChatActivableChannelsEnum.PSEUDO_CHANNEL_PRIVATE,
                message.Content);

            if (receiver.Status.StatusId == (sbyte) PlayerStatusEnum.PLAYER_STATUS_AFK &&
                receiver.Status is PlayerStatusExtended)
                SendChatServerMessage(sender.Client, receiver, ChatActivableChannelsEnum.PSEUDO_CHANNEL_PRIVATE,
                    $"Réponse automatique:{((PlayerStatusExtended) receiver.Status).Message}");
        }

        [WorldHandler(ChatClientPrivateWithObjectMessage.Id)]
        public static void HandleChatClientPrivateWithObjectMessage(WorldClient client,
            ChatClientPrivateWithObjectMessage message)
        {
            ...
            {
                //Le principe de précaution vous a rendu muet pour %1 seconde(s).
                sender.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_ERROR, 123,
                    (int) sender.GetMuteRemainingTime().TotalSeconds);
                return;
            }
            ...
            if (receiver.IsMuted())
            {
                //Message automatique : Le joueur <b>%1</b> a été rendu muet pour ne pas avoir respecté les règles. <b>%1</b> ne pourra pas vous répondre avant <b>%2</b> minutes.
                sender.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 168, receiver.Name,
                    receiver.Name, (int) receiver.GetMuteRemainingTime().TotalMinutes);
                return;
            }
            ...
            //Send to receiver
            SendChatServerWithObjectMessage(receiver.Client, sender, ChatActivableChannelsEnum.PSEUDO_CHANNEL_PRIVATE,
                message.Content, "", message.Objects);
            //Send a copy to sender
            SendChatServerCopyWithObjectMessage(client, sender, receiver,
                ChatActivableChannelsEnum.PSEUDO_CHANNEL_PRIVATE, message.Content, message.Objects);
        ...
        public static void HandleChatClientMultiWithObjectMessage(WorldClient client,
        ...
        public static void SendChatServerWithObjectMessage(IPacketReceiver client, INamedActor sender,
            ChatActivableChannelsEnum channel, string content, string fingerprint, IEnumerable<ObjectItem> objectItems)
        {
            client.Send(new ChatServerWithObjectMessage((sbyte) channel, content, DateTime.Now.GetUnixTimeStamp(),
                fingerprint, sender.Id, sender.Name, "", 0, objectItems.ToArray()));
        }

        public static void SendChatServerMessage(IPacketReceiver client, string message)
        {
            SendChatServerMessage(client, ChatActivableChannelsEnum.PSEUDO_CHANNEL_INFO, message,
                DateTime.Now.GetUnixTimeStamp(), "", 0, "", 0);
        }

        public static void SendChatServerMessage(IPacketReceiver client, INamedActor sender,
        ...
        public static void SendChatServerMessage(IPacketReceiver client, Character sender,
            ChatActivableChannelsEnum channel, string message)
        {
            SendChatServerMessage(client, sender, channel, message, DateTime.Now.GetUnixTimeStamp(), "");
        }
        ...
        public static void SendChatServerMessage(IPacketReceiver client, Character sender,
            ChatActivableChannelsEnum channel, string message, int timestamp, string fingerprint)
        {
        ...
        public static void SendChatServerMessage(IPacketReceiver client, ChatActivableChannelsEnum channel,
            string message, int timestamp, string fingerprint,
        ...
        public static void SendChatServerCopyMessage(IPacketReceiver client, Character sender, Character receiver,
            ChatActivableChannelsEnum channel, string message)
        {
        ...
        public static void SendChatServerCopyWithObjectMessage(IPacketReceiver client, Character sender,
            Character receiver, ChatActivableChannelsEnum channel,
            string message, IEnumerable<ObjectItem> objectItems)
        {
            SendChatServerCopyWithObjectMessage(client, sender, receiver, channel, message,
                DateTime.Now.GetUnixTimeStamp(), "", objectItems);
        }

        public static void SendChatServerCopyWithObjectMessage(IPacketReceiver client, Character sender,
            Character receiver, ChatActivableChannelsEnum channel, string message,
