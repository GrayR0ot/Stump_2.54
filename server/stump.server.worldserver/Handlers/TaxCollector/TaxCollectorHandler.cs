...
namespace Stump.Server.WorldServer.Handlers.TaxCollector
{
    public class TaxCollectorHandler : WorldHandlerContainer
    {
        ...
        public static void HandleGuildFightLeaveRequestMessage(WorldClient client,
            GuildFightLeaveRequestMessage message)
        {
            if (client.Character.Guild == null)
                return;
        ...
        public static void SendTaxCollectorListMessage(IPacketReceiver client, Guild guild)
        {
            client.Send(new TaxCollectorListMessage(
                guild.TaxCollectors.Select(x => x.GetNetworkTaxCollector()).ToArray(),
        ...
        {
            client.Send(new TaxCollectorAttackedMessage((ushort) taxCollector.FirstNameId,
                (ushort) taxCollector.LastNameId, (short) taxCollector.Map.Position.X,
                (short) taxCollector.Map.Position.Y,
                taxCollector.Map.Id, (ushort) taxCollector.Map.SubArea.Id,
                taxCollector.Guild.GetBasicGuildInformations()));
        }

        public static void SendGuildFightPlayersHelpersJoinMessage(IPacketReceiver client, TaxCollectorNpc taxCollector,
            Character character)
        {
            client.Send(new GuildFightPlayersHelpersJoinMessage(taxCollector.GlobalId,
                character.GetCharacterMinimalPlusLookInformations()));
        }

        public static void SendGuildFightPlayersHelpersLeaveMessage(IPacketReceiver client,
            TaxCollectorNpc taxCollector, Character character)
        {
            client.Send(new GuildFightPlayersHelpersLeaveMessage(taxCollector.GlobalId, (ulong) character.Id));
        }

        public static void SendGuildFightPlayersEnemyRemoveMessage(IPacketReceiver client, TaxCollectorNpc taxCollector,
            Character character)
        {
            client.Send(new GuildFightPlayersEnemyRemoveMessage(taxCollector.GlobalId, (ulong) character.Id));
        }

        public static void SendGuildFightPlayersEnemiesListMessage(IPacketReceiver client, TaxCollectorNpc taxCollector,
            IEnumerable<Character> characters)
        {
            client.Send(new GuildFightPlayersEnemiesListMessage(taxCollector.GlobalId,
                characters.Select(x => x.GetCharacterMinimalPlusLookInformations()).ToArray()));
        }

        public static void SendTaxCollectorAttackedResultMessage(IPacketReceiver client, bool deadOrAlive,
            TaxCollectorNpc taxCollector)
        {
            client.Send(new TaxCollectorAttackedResultMessage(deadOrAlive,
                taxCollector.GetTaxCollectorBasicInformations(), taxCollector.Guild.GetBasicGuildInformations()));
        }

        public static void SendGetExchangeGuildTaxCollectorMessage(IPacketReceiver client,
            ExchangeGuildTaxCollectorGetMessage exchangeGuildTaxCollectorGetMessage)
        {
            client.Send(exchangeGuildTaxCollectorGetMessage);
        }

        public static void SendTaxCollectorMovementMessage(IPacketReceiver client,
            TaxCollectorMovementTypeEnum movementType, TaxCollectorNpc taxCollector, int playerId, string name)
        {
            client.Send(new TaxCollectorMovementMessage((sbyte) movementType,
                taxCollector.GetTaxCollectorBasicInformations(), (ulong) playerId, name));
        }
