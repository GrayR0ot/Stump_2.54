...
namespace Stump.Server.WorldServer.Handlers.Approach
{
    public class ApproachHandler : WorldHandlerContainer
    {
            ...
            try
            {
                ...
                lock (ConnectionQueue.SyncRoot)
                {
                    foreach (var worldClient in ConnectionQueue)
                    {
                        ...
                        if (!worldClient.Connected) toRemove.Add(worldClient);
                    ...
                    foreach (var worldClient in toRemove) ConnectionQueue.Remove(worldClient);
                }
            }
            finally
            {
        ...
        {
            if (AccountManager.Instance.IsAccountBlocked(message.Account.Id, out var dummy))
            {
            ...
            var clients = WorldServer.Instance.FindClients(x => x.Account != null && x.Account.Id == ticketAccount.Id)
                .ToArray();
            ...
            SendServerSessionConstantsMessage(client,
                new ServerSessionConstantInteger((ushort) ServerConstantTypeEnum.TIME_BEFORE_DISCONNECTION,
                    BaseServer.Settings.InactivityDisconnectionTime * 1000 ?? -1),
            ...
            if (client.UserGroup.IsGameMaster)
                SendConsoleCommandsListMessage(client,
                    CommandManager.Instance.AvailableCommands.Where(x => client.UserGroup.IsCommandAvailable(x)));
        }
        ...
        public static void SendServerOptionalFeaturesMessage(IPacketReceiver client,
            params OptionalFeaturesEnum[] features)
        {
            client.Send(new ServerOptionalFeaturesMessage(features.Select(x => (sbyte) x).ToArray()));
        }

        public static void SendServerSessionConstantsMessage(IPacketReceiver client,
            params ServerSessionConstant[] constants)
        {
