...
namespace Stump.Server.WorldServer
{
    public class WorldServer : ServerBase<WorldServer>
    {
        ...
        /// </summary>
        [Variable] public static readonly string Host = "25.3.129.232";

        /// <summary>
        ///     Server port
        /// </summary>
        [Variable] public static readonly int Port = 3467;

        [Variable(true)] public static WorldServerData ServerInformation = new WorldServerData
        {
            Id = 1,
            Name = "Jiva",
            Address = "localhost",
        ...
        [Variable(true)] public static int AutoSaveInterval = 3 * 60;

        [Variable(true)] public static bool SaveMessage = true;
        ...
        public WorldVirtualConsole VirtualConsoleInterface { get; protected set; }

        public WorldPacketHandler HandlerManager { get; private set; }
        ...
        protected override void DisconnectAfkClient()
        {
            ...
            var afkClients = FindClients(client =>
                DateTime.Now.Subtract(client.LastActivity).TotalSeconds >=
                BaseServer.Settings.InactivityDisconnectionTime);

            foreach (var client in afkClients) client.DisconnectAfk();
        }

        public bool DisconnectClient(int accountId)
        {
            IEnumerable<WorldClient> clients =
        ...
        protected override void CheckScheduledShutdown()
        {
            ...
            if (diff < TimeSpan.FromHours(12))
            {
                ...
                else if (diff > TimeSpan.FromMinutes(30) && diff <= TimeSpan.FromHours(1) &&
                         announceDiff >= TimeSpan.FromMinutes(10))
                    AnnounceTimeBeforeShutdown(TimeSpan.FromMinutes(diff.TotalMinutes.RoundToNearest(30)), automatic);
                else if (diff > TimeSpan.FromMinutes(10) && diff <= TimeSpan.FromMinutes(30) &&
                         announceDiff >= TimeSpan.FromMinutes(5))
                    AnnounceTimeBeforeShutdown(TimeSpan.FromMinutes(diff.TotalMinutes.RoundToNearest(10)), automatic);
                else if (diff > TimeSpan.FromMinutes(5) && diff <= TimeSpan.FromMinutes(10) &&
                         announceDiff >= TimeSpan.FromMinutes(2))
                    AnnounceTimeBeforeShutdown(TimeSpan.FromMinutes(diff.TotalMinutes), automatic);
                else if (diff > TimeSpan.FromMinutes(1) && diff <= TimeSpan.FromMinutes(5) &&
                         announceDiff >= TimeSpan.FromMinutes(1))
                    AnnounceTimeBeforeShutdown(TimeSpan.FromMinutes(diff.TotalMinutes.RoundToNearest(1)), automatic);
                else if (diff > TimeSpan.FromSeconds(10) && diff <= TimeSpan.FromMinutes(1) &&
                         announceDiff >= TimeSpan.FromSeconds(10))
        ...
        {
            World.Instance.SendAnnounce(TextInformationTypeEnum.TEXT_INFORMATION_ERROR, 15,
                time.ToString(@"hh\h\ mm\m\ ss\s"));
        ...
        protected override void OnShutdown()
        {
            ...
            foreach (var client in ClientManager.Clients.ToArray()) client.Disconnect();
